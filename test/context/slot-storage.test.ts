...
it('reloads inside a shared lock even when both tabs previously cached the empty slot', async () => {
  let bytes: any = null, failing = false;
  const slot = {
    key: 'test', reliable: true, read: () => bytes && JSON.parse(bytes), write: (next: any) => {
...
it('an elected owner refuses a second writer and close permits explicit takeover', async () => {
  ...
  const manager = {
    request: async (name: any, options: any, fn: any) => {
  ...
  let bytes: any = null;
  const slot = { key: 'owner', read: () => bytes, write: (next: any) => { bytes = structuredClone(next); } };
...
it('closing refuses pending and future writes without re-acquiring storage ownership', async () => {
  let writes = 0;
  const storage = createSlotLedgerStorage({ read: () => null, write: () => { writes++; } }, { locks: null });
  const pending = storage.set('late', 1);
  await storage.close();
  await assert.rejects(pending, /closed/);
  await assert.rejects(storage.set('later', 2), /closed/);
  await assert.rejects(storage.get('later'), /closed/);
  await storage.close(); assert.equal(writes, 0);
});
