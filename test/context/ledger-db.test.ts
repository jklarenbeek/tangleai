 ...
 * and nothing from `@tangleai/context`, because the storage contract is the
 ...
 *
 * The adapter itself is `benchmark/lib/ledger-db.ts`, because the
...
import { createDbStorage } from '../../benchmark/lib/ledger-db.ts';
...
const memory = (id: any, text: any, extra = {}) =>
...
const opened: any[] = [];
async function storage(options: Record<string, any> = {}) {
...
describe('the db-backed ledger adapter — the four methods', function () {
  ...
  it('keys() answers sorted, under the prefix — and an adapter that does not is caught by that assertion', async function () {
    ...
    const shuffling = { ...adapter, keys: async (prefix: any) => (await adapter.keys(prefix)).reverse() };
...
describe('the db-backed ledger adapter — the ledger\'s own shapes, durable', function () {
  it('goal, progress, memories, skills, slots and listings all behave as they do in memory', async function () {
    ...
    assert.ok('progress' in progressed);
    assert.strictEqual(progressed.progress.length, 1);

    for (const record of CORPUS) await ledger.addMemory(record);
    assert.deepStrictEqual(withMember((await ledger.recall({ tags: ['greek'] })), 'map').map((m: any) => m.id),
    ...
    const skill = await ledger.addSkill({
      id: 'deploy', name: 'deploy', when: 'on green',
      instructions: 'run the gate'
    });
    assert.ok('id' in skill);
    assert.strictEqual(skill.id, 'deploy');
    assert.deepStrictEqual(withMember((await ledger.recallSkills()), 'map').map((s: any) => s.id), ['deploy']);
  ...
  it('five concurrent adds mint five ids and store five records', async function () {
    const adapter = await storage();
    const ledger = createLedger({ storage: adapter, now: clock() });
    const written = await Promise.all(Array.from({ length: 5 }, (_: any, i) =>
      ledger.addMemory({ text: `fact ${i}`, evidence: 'a source' })));
    const ids = written.map((record: any) => record.id);
    assert.strictEqual(new Set(ids).size, 5, 'five distinct ids');
    assert.strictEqual(withMember((await ledger.recall({})), 'length').length, 5, 'and five records durably stored');
  });
  ...
  it('snapshot and rollback restore the state the snapshot was taken over', async function () {
    ...
    assert.strictEqual(withMember((await ledger.recall({})), 'length').length, 2);
    await ledger.rollback(token);
    assert.deepStrictEqual(withMember((await ledger.recall({})), 'map').map((m: any) => m.id), ['before']);
  });
...
describe('the db-backed ledger adapter — rank, and the same answer without it', function () {
  ...
  async function seeded(extra: any[] = []) {
    const adapter = await storage();
    const ledger = createLedger({ storage: adapter, embedder: hash16(), now: clock() });
  ...
  it('the column answers what the sweep answers, to the record and the score', async function () {
    ...
    assert.deepStrictEqual(top.memories.map((m: any) => m.id), ['exact', 'close']);
  });
  ...
  it('a record with no vector is reported, not scored, on both paths', async function () {
    ...
    assert.ok('memories' in viaColumn);
    assert.ok('memories' in viaSweep);
    assert.deepStrictEqual(viaColumn.memories.map((m: any) => m.id),
      viaSweep.memories.map((m: any) => m.id));
    assert.ok('memories' in viaColumn);
    assert.ok(!viaColumn.memories.some((m: any) => m.id === 'late'),
  ...
  it('minScore filters and limit caps identically on both paths', async function () {
    ...
    assert.ok('memories' in sweptKept);
    assert.deepStrictEqual(kept.memories, sweptKept.memories);
    assert.ok('scores' in kept);
    assert.ok(kept.scores.every((s: number) => s >= 0.5));
    const none = await ledger.recall({ near: 'alpha beta gamma delta', minScore: 1.5 });
    assert.deepStrictEqual(none, {
      memories: [], scores: [], skipped: 0, via: 'adapter',
  ...
  it('a mixture of identities refuses in the ledger\'s own words, whichever path found it', async function () {
    ...
    assert.ok(typeof viaColumn === 'object' && viaColumn !== null && 'error' in viaColumn);
    ...
    assert.ok(typeof viaColumn === 'object' && viaColumn !== null && 'error' in viaColumn);
    assert.ok(typeof viaSweep === 'object' && viaSweep !== null && 'error' in viaSweep);
  ...
  it('a tag or a where narrows candidates the adapter knows nothing about, so it sweeps', async function () {
    ...
    assert.ok('via' in tagged);
    assert.strictEqual(tagged.via, 'sweep', 'the filter is the ledger\'s, so the read is too');
    assert.ok('memories' in tagged);
    assert.deepStrictEqual(tagged.memories.map((m: any) => m.id), ['exact', 'close', 'far']);
  });
  ...
  it('an adapter whose rank answers the wrong shape is refused, not trusted', async function () {
    ...
    const over = createLedger(({ storage: lying, embedder: hash16(), now: clock() } as any));
    ...
    assert.strictEqual(withMember((await ledger.recall({ near: 'alpha beta', limit: 2 })), 'via').via, 'adapter');
  });
  ...
  it('rank runs the k-nearest plan, and a sweep runs none', async function () {
    ...
    assert.deepStrictEqual((slots.stats() as { knn: Record<string, number>; }).knn, { queries: 0, rows: 0, candidates: 0, fullFetches: 0, diverted: 0 },
      'nothing has ranked yet');
    await ledger.recall({ near: 'alpha beta gamma delta', limit: 2 });
    const ranked = (slots.stats() as { knn: Record<string, number>; }).knn;
    ...
    assert.strictEqual((slots.stats() as { knn: Record<string, number>; }).knn.queries, 1, 'the swept ledger ran no plan at all');
  });
});
...
describe('the recipe and the documentation are one thing', function () {
  ...
  before(function () {
    const source = readFileSync(join(root, 'benchmark/lib/ledger-db.ts'), 'utf8');
  ...
  it('the context README carries the adapter verbatim', function () {
    const readme = readFileSync(join(root, 'packages/context/README.md'), 'utf8');
    assert.ok(readme.includes(listing),
      'packages/context/README.md must carry the listing between the markers, unedited');
  });

  it('the adapter imports @jarenjs/db and nothing from @tangleai/context', function () {
    assert.match(listing, /^import .* from '@jarenjs\/db';$/m);
    assert.doesNotMatch(listing, /^import .* from '@tangleai\/context/m,
...
it('two DB connections atomically mint ids and a failed transform rolls back entirely', async () => {
  ...
  await Promise.all(Array.from({ length: 12 }, (_: any, i) => [one, two][i % 2].addMemory({ text: String(i), evidence: 'source' })));
  assert.strictEqual((await one.listMemories()).length, 12);
  await assert.rejects(first.mutate('ai/', (current: any) => {
