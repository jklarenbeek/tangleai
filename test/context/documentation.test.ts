/**
 * @file The migrated AI documentation drift gate.
 *
...
import { LEDGER_SCHEMAS, createEnvironment } from '@tangleai/context';
import { PROGRAM_OPS } from '@tangleai/agents';

const ROOT = fileURLToPath(new URL('../../', import.meta.url));
const read = (rel: any) => fs.readFileSync(path.join(ROOT, rel), 'utf8');

const README = ['models', 'context', 'agents', 'jaren'].map(name => read(`packages/${name}/README.md`)).join('\n') + read('packages/jaren/docs/AUTHORING.md');

describe('AI package documentation matches the code', () => {
  ...
  it('every budget stop reason the loop can return is documented, and no other', () => {
    const source = read('packages/agents/src/agent.ts');
  ...
  it('every schema size the README quotes is the size of that artifact', () => {
    ...
    for (const [file, label] of Object.entries(sizes)) {
      const chars = JSON.stringify(JSON.parse(fs.readFileSync(new URL(import.meta.resolve(`@jarenjs/json/schemas/${file}`)), 'utf8'))).length;
