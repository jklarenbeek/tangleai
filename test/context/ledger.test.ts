...
import { createLedger as createLedgerDirect } from '@tangleai/context/ledger';
...
async function seedMemories(ledger: any) {
  await ledger.addMemory({
    id: 'm1', text: 'the deploy needs a WAL checkpoint',
    evidence: 'observed in run 41', tags: ['deploy', 'sqlite']
  });
  await ledger.addMemory({
    id: 'm2', text: 'the CSV reader hangs on short records',
    evidence: 'benchmark/csv.js timed out', tags: ['csv']
  });
  await ledger.addMemory({
    id: 'm3', text: 'WAL mode is on by default',
...
describe('ai ledger — writes are validated at the boundary', function () {
  it('rejects a memory with no evidence, in the shape the toolbox uses', async function () {
    const ledger = bare();
    const rejected = await ledger.addMemory(({ text: 'a guess', tags: [] } as any));
  ...
  it('rejects every kind the same way, and never throws', async function () {
    ...
    const cases = [
      ...
      ['memory', () => ledger.addMemory(({ text: 'x', evidence: 'y', tags: [1] } as any))],
      ['skill', () => ledger.addSkill(({ name: 'n', when: 'w' } as any))],
  ...
  it('rejects a timestamp that would sort wrong', async function () {
    ...
    assert.match(rejected.error, /invalid input for memory/);
    assert.ok(typeof rejected === 'object' && rejected !== null && 'errors' in rejected);
    assert.ok(rejected.errors.some((e: any) => e.keyword === 'pattern'),
  ...
  it('refuses an unknown member with the answer validate() gives, never by dropping it', async function () {
    ...
    assert.match(rejected.error, /invalid input for memory/);
    assert.ok(typeof rejected === 'object' && rejected !== null && 'errors' in rejected);
    assert.ok(rejected.errors.some((e: any) => e.keyword === 'additionalProperties' && e.instancePath === '/score'),
    ...
    assert.ok(typeof skill === 'object' && skill !== null && 'errors' in skill);
    assert.ok(skill.errors.some((e: any) => e.keyword === 'additionalProperties'));
  ...
  it('there is exactly one rejection-shape implementation', function () {
    ...
    const sources = ([['toolbox.ts', 'agents'], ['ledger.ts', 'context'], ['check.ts', 'models']] as const).map(([file, owner]) => [file,
      readFileSync(new URL(`../../packages/${owner}/src/${file}`, import.meta.url), 'utf8')]);
    for (const [file, source] of sources) {
      const builds = /error:\s*`invalid input for/.test(source);
      assert.strictEqual(builds, file === 'check.ts',
        `${file} ${builds ? 'builds its own' : 'does not build a'} rejection — only check.ts may`);
    }
  });
});
...
describe('ai ledger — the goal is singular', function () {
  ...
  it('records evidenced progress, and refuses it with no goal', async function () {
    const ledger = bare();
    assert.match(withMember((await ledger.recordProgress({ note: 'n', evidence: 'e' })), 'error').error,
      /no active goal/);
    ...
    assert.match(withMember((await ledger.recordProgress(({ note: 'trust me' } as any))), 'error').error,
  ...
  it('sets a terminal status without superseding', async function () {
    ...
    assert.strictEqual(withMember((await ledger.setGoalStatus('done')), 'status').status, 'done');
    assert.strictEqual((await ledger.getGoal())!.status, 'done');
    assert.deepStrictEqual(await ledger.listArchivedGoals(), []);
    assert.match(withMember((await ledger.setGoalStatus(('nonsense' as any))), 'error').error, /invalid input for goal/);
  });
});
...
describe('ai ledger — writes serialize and a minted id never collides', function () {
  ...
  const kinds = [
    ['memory',
      (ledger: any, n: any) => ledger.addMemory({ text: `fact ${n}`, evidence: `source ${n}` }),
      (ledger: any) => ledger.listMemories(),
      (ledger: any, id: any) => ledger.deleteMemory(id),
      (ledger: any, id: any) => ledger.getMemory(id)],
    ['skill',
      (ledger: any, n: any) => ledger.addSkill({ name: `skill ${n}`, when: 'w', instructions: 'i' }),
      (ledger: any) => ledger.listSkills(),
      (ledger: any, id: any) => ledger.deleteSkill(id),
      (ledger: any, id: any) => ledger.getSkill(id)],
  ] as const;
...
describe('ai ledger — retrieval, wired and unwired', function () {
  it('returns the same results with the seam wired and unwired', async function () {
    ...
    for (const tags of [['sqlite'], ['csv'], ['sqlite', 'csv'], ['absent']] as const) {
      const withSeam = await (await seedMemories(wired())).recall({ tags });
      const without = await (await seedMemories(bare())).recall({ tags });
      assert.deepStrictEqual(without.map((m: any) => m.id), withSeam.map((m: any) => m.id),
  ...
  it('orders by recency and honours a limit, on both paths', async function () {
    for (const make of [wired, bare]) {
      ...
      assert.deepStrictEqual((await ledger.recall()).map((m: any) => m.id), ['m3', 'm2', 'm1'],
        'newest first');
      assert.deepStrictEqual((await ledger.recall({ limit: 2 })).map((m: any) => m.id), ['m3', 'm2']);
      assert.deepStrictEqual((await ledger.recall({ tags: ['sqlite'] })).map((m: any) => m.id),
        ['m3', 'm1']);
    }
  });
  ...
  it('the wired seam evaluates a predicate the fallback cannot', async function () {
    ...
    assert.deepStrictEqual(hits.map((m: any) => m.id), ['m3', 'm1']);
    ...
    assert.deepStrictEqual(narrowed.map((m: any) => m.id), ['m1']);
  });
  ...
  it('retrieves skills the same way', async function () {
    ...
    await ledger.addSkill({
      id: 's2', name: 'measure', when: 'before a claim',
      instructions: 'run the benchmark', tools: []
    });
    assert.deepStrictEqual(withMember((await ledger.recallSkills()), 'map').map((s: any) => s.id), ['s2', 's1']);
    assert.deepStrictEqual(
      withMember((await ledger.recallSkills({ where: { $contains: ['$it.when', 'deploy'] } })), 'map')
        .map((s: any) => s.id), ['s1']);
  });
});
...
describe('ai ledger — slots address content without carrying it', function () {
  ...
  it('serializes a non-string value, and removes both keys on delete', async function () {
    const ledger = bare();
    await ledger.putSlot('rows', ([{ a: 1 }] as any));
...
describe('ai ledger — every mutation is reversible', function () {
  ...
  it('two ledgers over one storage mint distinct tokens, each restoring its own state', async function () {
    ...
    assert.deepStrictEqual((await a.listMemories()).map((m: any) => m.id).sort(), ['b1', 'base'],
      "b's snapshot restores b's state");
    assert.strictEqual(await a.rollback(tokenA), true);
    assert.deepStrictEqual((await b.listMemories()).map((m: any) => m.id), ['base'],
      "a's snapshot was not overwritten by b's");
  });

  it('answers an unknown token rather than throwing', async function () {
    assert.match(withMember((await bare().rollback('snap-999999')), 'error').error, /unknown snapshot/);
  });
});
...
describe('ai ledger — records and skills round-trip', function () {
  it('gets, lists and deletes memories and skills', async function () {
    ...
    assert.deepStrictEqual((await ledger.listMemories()).map((m: any) => m.id), ['m3', 'm1']);
