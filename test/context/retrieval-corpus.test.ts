 ...
 *
 * `benchmark/retrieval.ts` publishes the number every retrieval change
...
} from '../../scripts/generate-retrieval-corpus.ts';
import {
  KS, assertScorer, corpusAt, loadCorpus, loadLedger, questionTags, randomBand, randomFloor, runSize,
} from '../../benchmark/lib/retrieval.ts';

const ROOT = fileURLToPath(new URL('../../', import.meta.url));
const GENERATOR = path.join(ROOT, 'scripts/generate-retrieval-corpus.ts');
  ...
  it('is deterministic: two runs into a temp dir are byte-identical, and so is the committed fixture', () => {
    ...
    try {
      ...
      assert.ok(first.equals(readFileSync(FIXTURE)),
        'the committed fixture differs from the generator — run node scripts/generate-retrieval-corpus.ts --write and read the diff');
    }
    finally {
...
describe('the committed corpus', () => {
  ...
  const byId = new Map<string, { text: string; tags: string[]; }>(corpus.memories.map((m: any) => [m.id, m]));
  ...
  it('has unique ids, unique statements, and one gold memory per fact stating exactly that fact', () => {
    ...
    assert.strictEqual(new Set(corpus.memories.map((m: any) => m.text)).size, corpus.memories.length,
  ...
  it('asks every fact exactly once, with one, two or three gold memories per question', () => {
    ...
    const asked = corpus.questions.flatMap((q: any) => q.facts);
    assert.deepStrictEqual([...asked].sort(), corpus.facts.map((f: any) => f.id).sort());
    const counts = corpus.questions.map((q: any) => q.gold.length);
    for (const count of [1, 2, 3]) assert.ok(counts.includes(count), `no question with ${count} gold memories`);
    assert.ok(counts.every((c: any) => c >= 1 && c <= 3));
    for (const question of corpus.questions) {
      ...
      const subjects = question.facts.map((f: any) => corpus.facts.find((x: any) => x.id === f).text.split(' ').slice(0, 3).join(' '));
  ...
  it('spreads `at` over one synthetic year and every record passes the ledger schema', async () => {
    const years = new Set(corpus.memories.map((m: any) => m.at.slice(0, 4)));
    assert.deepStrictEqual([...years], ['2025']);
    const months = new Set(corpus.memories.map((m: any) => m.at.slice(5, 7)));
  ...
  it('names a tag in most questions and no tag in a quarter of them, and no phrase carries a tag word', () => {
    ...
    const untagged = corpus.questions.filter((q: any) => questionTags(q.text, (tags as any)).length === 0).length;
    ...
    for (const question of corpus.questions) {
      const found = questionTags(question.text, (tags as any));
    ...
    assert.deepStrictEqual(questionTags('About deploy and tests: what did we learn about the tag push?', (tags as any)),
      ['deploy', 'tests']);
    assert.deepStrictEqual(questionTags('What did we learn about the tag push?', (tags as any)), []);
  });
});
...
describe('the retrieval scorer', () => {
  it('scores the oracle at 1.000 everywhere and the random draw on its analytic floor, at 1 000 memories', async () => {
    ...
    const at = (key: any) => result.policies.find((p: any) => p.key === key);
  ...
  it('refuses a broken oracle and an off-floor random row, naming the row', () => {
    const questions = Array.from({ length: 50 }, (_: any, i) => ({ gold: [`m${i}`] }));
    const ok = (recall: any) => ({ recall: Object.fromEntries(KS.map((k) => [k, recall])), mrr: recall });
    const result = (oracle: any, random: any) => ({
      n: 1000, questions,
