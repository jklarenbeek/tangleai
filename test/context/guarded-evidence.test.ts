...
const envelope = () => ({
  version: 1, artifacts: [{ id: 'a', kind: 'file', locator: 'slot:source' }],
...
it('validates distinct referential failures deterministically and preserves legacy memories', async () => {
  ...
  assert.deepEqual(new Set(checked.errors.map((e: any) => e.code)), new Set([
    'EVIDENCE_DUPLICATE', 'EVIDENCE_ARTIFACT', 'EVIDENCE_REFERENCE', 'EVIDENCE_CRITICAL', 'EVIDENCE_HIDDEN',
  ]));
  assert.ok(checked.errors.every((e: any) => e.docPath.startsWith('/')));
  assert.equal(validateClaimEvidence(value, { artifacts: [] }).valid, false);
  const ledger = createLedger();
  assert.ok(withMember((await ledger.addMemory(({ text: 'bad', evidence: broken } as any))), 'error').error);
  const old = await ledger.addMemory({ text: 'old', evidence: 'legacy' });
  const fresh = await ledger.addMemory(({ text: 'new', evidence: value } as any));
  assert.ok('id' in old);
  assert.equal((await ledger.getMemory(old.id))!.evidence, 'legacy');
  assert.ok('id' in fresh);
...
it('claim refinement rejects invented artifacts before commit and rolls back a failed edit', async () => {
  let value = envelope(), saved: any, writes = 0;
  const refiner = createClaimRefiner({
    read: async () => value, artifacts: value.artifacts,
    validateProposal: () => true, apply: (_: any, proposed: any) => proposed,
    snapshot: async () => { saved = structuredClone(value); return 'checkpoint'; },
    restore: async () => { value = saved; },
    commit: async (next: any) => { writes++; value = next; if (next.claims[0].text === 'fail') throw new Error('disk'); return next; },
  });
...
it('ledger refinement cannot invent or alter a host-admitted artifact', async () => {
  ...
  const refiner = createRefiner(({ ledger, applyPatch: applyJSONPatch } as any));
...
it('exact-evidence suppression also compares versioned envelopes by their JSON bytes', async () => {
  const { createRefiner } = await import('@tangleai/agents');
  const { applyJSONPatch } = await import('@jarenjs/json');
  const ledger = createLedger(), refiner = createRefiner(({ ledger, applyPatch: applyJSONPatch, deduplicate: 'exact-evidence' } as any));
