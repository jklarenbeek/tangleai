...
import { loadDataset, normalizeDataset, parseJsonl, relevanceMetrics, scoreRelevance, sha256 } from '../../benchmark/lib/relevance.ts';
import { createRelevanceCache, meteredFetch, verifiedVector } from '../../benchmark/lib/relevance-cache.ts';
...
const temp = (t: any) => { const dir = mkdtempSync(join(tmpdir(), 'relevance-')); t.after(() => rmSync(dir, { recursive: true, force: true })); return dir; };
...
describe('labelled relevance and the data boundary', () => {
  it('scores unjudged prototype-like corpus ids as zero gain', () => {
    for (const id of ['constructor', '__proto__', 'toString']) {
      ...
      const result = relevanceMetrics(data.questions, [[id, 'a']] as const);
  ...
  it('pins graded recall, MRR, nDCG and no-answer by hand', async () => {
    ...
    assert.equal(relevanceMetrics(dataset.questions, [['b'], [], ['a']] as const).noAnswer.accuracy, 0);
    const scored = await scoreRelevance({ key: 'pinned', run: (q: any) => rankings[dataset.questions.indexOf(q)] }, dataset.questions);
    assert.equal(scored.mrr, result.mrr);
    assert.ok(!JSON.stringify(scored).includes(dataset.questions[0].text));
    assert.equal(relevanceMetrics([dataset.questions[2]], [[]] as const).mrr, null);
  });
  ...
  ] as const) it(`refuses ${label}`, () => assert.throws(() => normalizeDataset(manifest, [...docs], [...qs], [...rels]), pattern));
  it('refuses checksum tampering, escaping paths, malformed JSON and duplicate rankings', (t) => {
    ...
    assert.throws(() => relevanceMetrics([{ gold: ['a'] }], [['a', 'a']] as const), /distinct/);
...
describe('identity-bound embedding cache', () => {
  ...
  const embedder = { model: 'fixture', dims: 2, embed: async (texts: any) => texts.map(() => new Float32Array([1, 2])) };
  it('batches distinct texts, resumes, and verifies cached vector hashes', async (t) => {
    ...
    await assert.rejects(cache.embed(('abc' as any)), /text array/);
  ...
  it('keeps failed batches, excludes secrets, and resumes paid work', async (t) => {
    const directory = temp(t);
    let call = 0;
    const failing = { ...embedder, embed: async (texts: any) => { if (++call === 2) throw new Error('secret-token raw-text'); return embedder.embed(texts); } };
    ...
    assert.equal(resumed.history().filter((r: any) => r.outcome === 'failed').length, 1);
  });
  it('charges a hard request ceiling and refuses partial/invalid batches and Float32 overflow', async (t) => {
    ...
    assert.equal(cache.stats.embedded, 1);
    const broken = createRelevanceCache({ directory: temp(t), identity, embedder: { ...embedder, embed: async () => [[1, 2], [NaN, 2]] as const } });
    await assert.rejects(broken.embed(['a', 'b']), /failed/);
    assert.equal(broken.stats.embedded, 0);
    for (const v of [[1e100, 1], [0, 0], [1], ['1', 1]] as const) assert.throws(() => verifiedVector(v, 2), /invalid/);
  });
  it('refuses identity drift within one embedder instance', async (t) => {
    ...
    moving.embed = async (texts: any) => { moving.model = 'foreign'; return embedder.embed(texts); };
...
describe('storage candidate trust boundary', () => {
  ...
  async function setup(reply: any) {
    const storage = createMemoryStorage();
    const ledger = createLedger({ storage: { ...storage, rank: async () => reply }, embedder });
    for (const [id, embedding] of [['a', [1, 0]], ['b', [0, 1]]] as const)
      await ledger.addMemory(({ id, text: id, evidence: 'source', embedding, embeddedBy: { model: 'm', dims: 2 } } as any));
  ...
  it('re-scores unsorted hits, normalizes old exact metadata, and reports approximation', async () => {
    const reply: ReturnType<typeof valid> & { ranking?: { algorithm: string; exhaustive: boolean; candidateCount: number; }; } = valid();
    const { ledger } = await setup(reply);
    const old = await ledger.recall({ near: 'q', limit: 1 });
    assert.ok('memories' in old);
    assert.equal(old.memories[0].id, 'a');
    assert.ok('scores' in old);
    assert.deepEqual(old.scores, [1]);
    assert.ok('ranking' in old);
    ...
    assert.equal(withMember((await ledger.recall({ near: 'q' })), 'ranking').ranking.exhaustive, false);
  });
  for (const [name, mutate] of [
    ['foreign prefix', (r: any) => { r.hits[0].key = 'ai/state/skill/b'; }],
    ['duplicate hit', (r: any) => { r.hits.push(r.hits[0]); }],
    ['invalid score', (r: any) => { r.hits[0].score = NaN; }],
    ['negative skipped', (r: any) => { r.skipped = -1; }],
    ['malformed identity', (r: any) => { r.identities = [null]; }],
    ['lying count', (r: any) => { r.ranking = { algorithm: 'ann', exhaustive: false, candidateCount: 1 }; }],
    ['null metadata', (r: any) => { r.ranking = null; }],
  ] as const) it(`refuses ${name}`, async () => {
    const reply: ReturnType<typeof valid> & { ranking?: { algorithm: string; exhaustive: boolean; candidateCount: number; }; } = valid(); mutate(reply);
    const { ledger } = await setup(reply);
    assert.ok(withMember((await ledger.recall({ near: 'q' })), 'error').error);
  });
  it('verifies stored identity and vector even when the adapter lies, tolerating concurrent deletion', async () => {
    ...
    assert.match(withMember((await ledger.recall({ near: 'q' })), 'error').error, /identity mismatch/);
    await storage.set(prefix + 'a', { ...original, embedding: [null, 1] });
    assert.match(withMember((await ledger.recall({ near: 'q' })), 'error').error, /vector/);
    await storage.delete(prefix + 'a');
    assert.deepEqual(withMember((await ledger.recall({ near: 'q' })), 'memories').memories.map((m: any) => m.id), ['b']);
  });
});
