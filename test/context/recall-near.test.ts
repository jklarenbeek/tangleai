...
import { createLedger, sameIdentity, describeIdentity } from '@tangleai/context/ledger';
...
import { MEMORY_SCHEMA, SKILL_SCHEMA } from '@tangleai/context/schemas/ledger';
import { compileJsonQuery } from '@jarenjs/json/query';

/** A deterministic clock: every record's `at` is decided by the test. */
function clock(start: number = 0) {
  let tick = start;
...
async function embeddingOf(embedder: any, text: any) {
 ...
 */
function counting(inner: import('@tangleai/models/embed').Embedder & { dims: number; } = hash16()) {
...
const memory = (id: any, text: any, extra = {}) => ({ id, text, evidence: 'a source', tags: [], at: '2026-08-25T00:00:00.000Z', ...extra });
...
describe('ai ledger — a record carries its embedding with its identity', function () {
  it('a memory and a skill with embedding + embeddedBy round-trip as plain arrays', async function () {
    ...
    assert.ok('id' in plain);
    assert.strictEqual(plain.id, 'm2');
    assert.strictEqual(('embedding' in plain ? plain.embedding : undefined), undefined);
  });
  ...
  it('refuses one of the pair without the other, in the standard shape', async function () {
    ...
    assert.ok(typeof rejected === 'object' && rejected !== null && 'error' in rejected);
    assert.match(rejected.error, /invalid input for memory/);
    assert.ok(typeof rejected === 'object' && rejected !== null && 'errors' in rejected);
    assert.ok(Array.isArray(rejected.errors) && rejected.errors.length > 0);
    assert.ok('inputSchema' in rejected);
    ...
    assert.ok(typeof identityOnly === 'object' && identityOnly !== null && 'error' in identityOnly);
    assert.match(identityOnly.error, /invalid input for memory/);
    assert.strictEqual(await ledger.getMemory('m-id'), null);

    const skill = await ledger.addSkill(({ id: 's-lone', name: 'n', when: 'w', instructions: 'i', embedding: [1] } as any));
    assert.ok(typeof skill === 'object' && skill !== null && 'error' in skill);
    assert.match(skill.error, /invalid input for skill/);
    assert.ok('inputSchema' in skill);
  ...
  it('keeps the vector arithmetic out of ledger.js', function () {
    ...
    const source = readFileSync(new URL('../../packages/context/src/ledger.ts', import.meta.url), 'utf8');
...
describe('ai ledger — recall({ near }) refuses before it ranks', function () {
  it('without the seam it names the seam, and plain recall still works', async function () {
    ...
    assert.ok(typeof refused === 'object' && refused !== null && 'error' in refused);
    ...
    assert.deepStrictEqual(withMember((await ledger.recall({ tags: ['t'] })), 'map').map((m: any) => m.id), ['m1'],
      'the tag path is untouched by the missing seam');
    const skills = await ledger.recallSkills({ near: 'anything' });
    assert.ok(typeof skills === 'object' && skills !== null && 'error' in skills);
  ...
  it('refuses a `near` that is not a non-empty string', async function () {
    ...
    for (const near of ['', 42, null, ['a']]) {
      const refused = await ledger.recall({ near } as any);
      assert.ok(typeof refused === 'object' && refused !== null && 'error' in refused);
  ...
  it('refuses a mixture of identities naming the models found, never ranking a subset', async function () {
    ...
    assert.ok(typeof mixed === 'object' && mixed !== null && 'error' in mixed);
    ...
    assert.strictEqual('memories' in mixed ? mixed.memories : undefined, undefined, 'and answers no subset');
    ...
    assert.ok(typeof disagree === 'object' && disagree !== null && 'error' in disagree);
    ...
    assert.deepStrictEqual(narrowed, {
      memories: [], scores: [], skipped: 0, via: 'sweep',
  ...
  it('a `where` predicate still needs the compileQuery seam, with or without near', async function () {
    ...
    assert.ok('memories' in hits);
    assert.deepStrictEqual(hits.memories.map((m: any) => m.id), ['m1'], 'the predicate narrows the candidates');
  });
  ...
  it('a seam that throws or rejects at query time is content, never a crash', async function () {
    for (const embedder of [syncThrower(), rejecter()]) {
      ...
      assert.ok(typeof failed === 'object' && failed !== null && 'error' in failed);
...
describe('ai ledger — recall({ near }) ranks, breaks ties, and reports what it skipped', function () {
  ...
  async function seeded(extra: Record<string, any> = {}) {
    const embedder = hash16();
  ...
  it('ranks by cosine similarity, descending, and counts the un-embedded as skipped', async function () {
    ...
    assert.deepStrictEqual(result.memories.map((m: any) => m.id), ['exact', 'close', 'far']);
  ...
  it('composes with the tag filter, and `skipped` counts only what passed the filter', async function () {
    ...
    assert.ok('memories' in result);
    assert.deepStrictEqual(result.memories.map((m: any) => m.id), ['exact', 'close']);
    assert.ok('skipped' in result);
  ...
  it('`limit` caps after ranking and `minScore` filters after ranking', async function () {
    ...
    assert.ok('memories' in top);
    assert.deepStrictEqual(top.memories.map((m: any) => m.id), ['exact']);
    assert.ok('scores' in top);
    assert.strictEqual(top.scores.length, 1);
    assert.ok('skipped' in top);
    ...
    assert.ok('memories' in kept);
    assert.deepStrictEqual(kept.memories.map((m: any) => m.id), ['exact', 'close']);
    const nothing = await ledger.recall({ near: 'alpha beta gamma delta', minScore: 1.01 });
    assert.ok('memories' in nothing);
    assert.deepStrictEqual(nothing.memories, []);
    assert.ok('skipped' in nothing);
    ...
    assert.ok('memories' in one);
    assert.deepStrictEqual(one.memories.map((m: any) => m.id), ['exact']);
  });
  ...
  it('breaks a tie by recency, then by id — deterministically', async function () {
    ...
    const at = (tick: any) => new Date(Date.UTC(2026, 7, 25, 0, 0, tick)).toISOString();
    ...
    assert.ok('memories' in first);
    assert.deepStrictEqual(first.memories.map((m: any) => m.id), ['m-new', 'a-old', 'z-old'],
      'equal scores: newest first, then id ascending');
    assert.ok('scores' in first);
    assert.ok(first.scores.every((s: any) => s === first.scores[0]), 'the scores really are equal');
    const second = await ledger.recall({ near: 'identical text' });
    assert.ok('memories' in second);
    assert.ok('memories' in first);
    assert.deepStrictEqual(second.memories.map((m: any) => m.id), first.memories.map((m: any) => m.id));
  });

  it('recallSkills ranks the same way over skills', async function () {
    const embedder = hash16();
    const ledger = createLedger({ now: clock(), embedder });
    const skill = async (id: any, name: any, when: any, instructions: any) => ledger.addSkill(({
    ...
    assert.deepStrictEqual(result.skills.map((s: any) => s.id), ['deploy', 'measure']);
    assert.ok('scores' in result);
    assert.strictEqual(result.scores.length, 2);
    assert.ok('skipped' in result);
    assert.strictEqual(result.skipped, 1);
    assert.ok(!('memories' in result), 'skills answer as skills');
  });
  ...
  it('without `near` the answer is the array it always was', async function () {
    ...
    assert.deepStrictEqual(plain.map((m: any) => m.id), ['bare-1', 'close', 'exact'],
...
describe('ai ledger — embedMissing is the explicit sweep', function () {
  async function unembedded(ledger: any, n: number = 5) {
  ...
  it('embeds every un-embedded memory and skill, and a second run embeds zero', async function () {
    ...
    assert.ok('skipped' in ranked);
    assert.strictEqual(ranked.skipped, 0);
    assert.ok('memories' in ranked);
  ...
  it('a failing batch leaves its records un-embedded, counted in `remaining`, with the error surfaced once', async function () {
    ...
    assert.match((result.error as any), /HTTP 503/);
  ...
  it('a seam that throws synchronously loses nothing either', async function () {
    ...
    assert.match((result.error as any), /sync boom/);
  ...
  it('refuses to create a mixture of identities', async function () {
    ...
    assert.strictEqual(result.embedded, 0);
    assert.strictEqual(result.remaining, 4);
    assert.match((result.error as any), /^embedMissing: .*hash-trigram-32/, 'names what the ledger already holds');
    assert.match((result.error as any), /hash-trigram-16/, 'and the embedder that would have mixed with it');
  ...
  it('without the seam it refuses the same way recall does', async function () {
    ...
    const result = await ledger.embedMissing();
    assert.match((result.error as any), /^embedMissing: .*needs the embedder seam/);
...
describe('ai ledger — auto-embedding on write is opt-in', function () {
  it('is off by default: a write with a seam and no flag stays un-embedded and succeeds', async function () {
    ...
    assert.ok('id' in stored);
    assert.strictEqual(stored.id, 'm1');
    assert.strictEqual(('embedding' in stored ? stored.embedding : undefined), undefined);
    ...
    assert.deepStrictEqual(ranked, {
      memories: [], scores: [], skipped: 1, via: 'sweep',
      ranking: { algorithm: 'exact-cosine', exhaustive: true, candidateCount: 0 }
    },
  ...
  it('on, a write acquires its embedding inside the write, and keeps one it was handed', async function () {
    ...
    const rejected = await ledger.addMemory(({ text: 'no evidence' } as any));
    assert.ok(typeof rejected === 'object' && rejected !== null && 'error' in rejected);
  ...
  it('on, a seam failure stores the record un-embedded and says so — never a lost write', async function () {
    for (const embedder of [rejecter(), syncThrower()]) {
      ...
      assert.strictEqual(('error' in stored ? stored.error : undefined), undefined, 'and is not a rejection');
      assert.ok('embedError' in stored);
      assert.match((stored.embedError as any), /boom/, 'the seam failure is reported on the result');
      const read = await ledger.getMemory('m1');
      assert.strictEqual(read!.embedding, undefined, 'stored un-embedded');
      assert.strictEqual(('embedError' in read! ? read.embedError : undefined), undefined, 'the report is not part of the record');
      ...
      const ranked = await ledger.recall({ near: 'a fact' });
      assert.ok(typeof ranked === 'object' && ranked !== null && 'error' in ranked);
  ...
  it('refuses a contradictory or malformed configuration at construction', function () {
    ...
    assert.throws(() => createLedger(({ embedder: { model: 'x' } } as any)), (err) =>
      err instanceof AiError && err.code === 'AI0001' && /embedder/.test(err.message));
    assert.throws(() => createLedger(({ embedder: { embed: async () => [] } } as any)), (err) =>
