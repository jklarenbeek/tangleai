/**
 * @file The `docs/VECTORS-AND-RECALL.md` end-to-end vector example, executed.
 *
...
const HOWTO = fs.readFileSync(path.join(ROOT, 'docs/VECTORS-AND-RECALL.md'), 'utf8');
 ...
 * @param text @param heading @returns
 */
function topLevelSection(text: string, heading: string): string {
...
describe('the HOWTO vector example runs, and answers what it says it answers', () => {
  it('the section is present and carries every fence the program needs', () => {
    assert.ok(SECTION.length > 0, `docs/VECTORS-AND-RECALL.md no longer has a "${HEADING}" section`);
  ...
  it('the ranked recall answers the memories, scores and skip count the document prints', () => {
    const evidence = ran.recalled.memories.map((m: any) => m.evidence);
    const scores = ran.recalled.scores.map((s: number) => s.toFixed(3));
