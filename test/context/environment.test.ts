 ...
 * `environment-scale.test.ts` proves the consequence; this file proves
...
import { createEnvironment, chunkSlotName, environmentTools } from '@tangleai/context/environment';
...
function corpus(lines: number = 200) {
  const rows: any[] = [];
...
const env = (options: Record<string, any> = {}) => createEnvironment({ now: () => AT, ...options });
...
describe('environment replacement and bounds', () => {
  ...
  it('removes only stale indexed pieces after a shorter or empty ingestion', async () => {
    ...
    assert.deepStrictEqual((await environment.ledger.listSlots()).map((slot: any) => slot.name), ['corpus#given:0/note']);
  });
...
describe('ai — the environment\'s query seam (D3)', function () {
  const DATA = JSON.stringify(Array.from({ length: 50 },
    (unused: any, i) => ({ id: `REC${String(i).padStart(4, '0')}`, value: i * 7, tag: i % 2 ? 'odd' : 'even' })));
...
describe('ai — the environment as tools', function () {
  it('every operation is reachable through its tool, with the arguments it declares', async function () {
    ...
    assert.ok(rejected.errors.some((e: any) => e.message.includes('chars')));
  });
});
