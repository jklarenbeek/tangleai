...
import { checkpointProgress, validateCheckpoint, ledgerFootprint } from '@tangleai/context/retention';
import { archiveFootprint } from '@tangleai/context/archive';
import { retentionFixture, measureRetention } from '../../benchmark/lib/retention.ts';
const now = () => '2026-09-09T00:00:00Z';
const dump = (map: any) => JSON.stringify([...map].sort());
...
it('reconciles serialized bytes and measures referenced/unreferenced losses after restart', async () => {
  ...
  assert.equal(measured.bytes, measured.overhead + Object.values(measured.classes).reduce((n: any, c: any) => n + c.bytes, 0));
...
it('evicts deterministically with durable tombstones, protects evidence, and clears intentionally', async () => {
  ...
  assert.ok(!('error' in (await ledger.putSlot('new', 'new content', { kind: 'agent-round' }))));
  assert.equal(withMember((await ledger.readSlot('old'))!, 'status').status, 'evicted');
  ...
  assert.equal(withMember((await reopened.readSlot('old'))!, 'reason').reason, 'archive-budget');
  await ledger.addMemory({ text: 'requires new', evidence: 'new' });
  const before = dump(backing);
  assert.equal(withMember((await reopened.putSlot('refused', 'x', { kind: 'agent-round' })), 'code').code, 'ARCHIVE_BUDGET');
...
it('archive byte limits include report/tombstones and refusal publishes no partial batch', async () => {
  ...
  assert.ok(!('error' in (await bounded.putSlot('first', 'é'.repeat(500), { kind: 'agent-round' }))));
  const before = dump(backing);
  const tooSmall = createLedger({ storage, now, archiveLimits: { maxBytes: bytes - 1 } });
  assert.equal(withMember((await tooSmall.putSlot('first', 'é'.repeat(500), { kind: 'agent-round' })), 'code').code, 'ARCHIVE_BUDGET');
  assert.equal(dump(backing), before);
  assert.ok(withMember((await bounded.putArchive([{ name: 'a', text: 'a', kind: 'agent-round' }, { name: 'b', text: 'b', kind: 'agent-round' }])), 'error').error);
  assert.equal(dump(backing), before);
});
...
it('goal checkpoint preserves every source through restart and rejects invented evidence', async () => {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    assert.ok(!('error' in answer), 'error' in answer ? answer.error : undefined);
  }
  ...
  assert.ok(withMember((await ledger.composeGoal()), 'text').text!.length <= 6000);
  ...
  const before = dump(backing);
  assert.equal(withMember((await liar.recordProgress({ note: 'new', evidence: 'receipt' })), 'code').code, 'GOAL_CHECKPOINT');
  assert.equal(dump(backing), before);
  const tiny = createLedger({ ...options, goalLimits: { maxChars: 10 } });
  assert.equal(withMember((await tiny.composeGoal()), 'code').code, 'GOAL_BUDGET');
  assert.equal(withMember((await tiny.recordProgress({ note: 'new', evidence: 'receipt' })), 'code').code, 'GOAL_BUDGET');
  assert.equal(dump(backing), before);
...
it('retention requires atomic storage and quota failures leave all durable bytes unchanged', async () => {
  ...
  assert.equal(withMember((await legacy.putSlot('x', 'x', { kind: 'agent-round' })), 'code').code, 'ATOMIC_REQUIRED');
  assert.equal(withMember((await legacy.setGoal({ objective: 'x' })), 'code').code, 'ATOMIC_REQUIRED');
  const backing = new Map(), durable = createMemoryStorage(backing);
  let fail = false;
  const storage = {
    ...durable, mutate: (prefix: any, fn: any) => durable.mutate(prefix, (current) => {
...
it('committed retention curves validate and reproduce every seeded policy row', async () => {
  ...
  const read = (name: any) => JSON.parse(readFileSync(new URL(`../../benchmark/${name}`, import.meta.url), 'utf8'));
  ...
  assert.ok(decoding.rows.every((row: any) => row.schemaSent === true || row.outcome === 'failed'));
...
it('scoped lifecycle reads and clears never expose or remove sibling archives', async () => {
  const { createEnvironment } = await import('@tangleai/context');
...
it('immutable archives reject concurrent address collisions and named archives remain replaceable', async () => {
  ...
  assert.equal(outcomes.filter((outcome) => 'ok' in outcome && outcome.ok).length, 1);
  assert.equal(outcomes.filter((outcome) => 'code' in outcome && outcome.code === 'ARCHIVE_COLLISION').length, 1);
  const before = dump(backing);
  assert.equal(withMember((await a.putArchive([{ name: 'same-address', text: 'changed', kind: 'agent-round' }],
    { immutable: true })), 'code').code, 'ARCHIVE_COLLISION');
  assert.equal(dump(backing), before);
  assert.equal(withMember((await a.putArchive([{ name: 'same-address', text: 'changed', kind: 'agent-round' }])), 'ok').ok, true);
...
it('re-archiving identical content preserves timestamps and pinned protection', async () => {
  let tick = 0;
  const backing = new Map(), ledger = createLedger({
    storage: createMemoryStorage(backing),
  ...
  const before = dump(backing);
  assert.equal(withMember((await ledger.putArchive([{ name: 'other', text: 'other', kind: 'agent-round' }])), 'code').code, 'ARCHIVE_BUDGET');
  assert.equal(dump(backing), before);
  assert.equal(withMember((await ledger.putArchive([{ name: 'outside', text: 'not a round', kind: 'text' }])), 'code').code, 'ARCHIVE_INPUT');
  assert.equal(dump(backing), before);
});
...
it('archive limits do not make ordinary slot writes copy an unrelated corpus', async () => {
  ...
  let copied = 0;
  const storage = {
    ...base, mutate: (scope: any, fn: any) => base.mutate(scope, (current) => {
  ...
  assert.equal(withMember((await ledger.readSlot('corpus'))!, 'length').length, 100000);
});
