...
import { createSiteApp, parseHash } from './host-fixture.ts';
import { createSlotLedgerStorage } from '@tangleai/context/storage/slot';
import { validateFlowDocument } from '@jarenjs/studio/flow';
import { createStubHost, fire, serialize } from '../assistant/dom.stub.ts';

/** Build an SSE completion body from chat chunks + the [DONE] marker. */
function sseBody(chunks: any) {
  return chunks.map((c: any) => `data: ${JSON.stringify(c)}\n\n`).join('') + 'data: [DONE]\n\n';
}
const textTurn = (content: any) => sseBody([
  { choices: [{ delta: { role: 'assistant', content } }] },
  { choices: [{ delta: {}, finish_reason: 'stop' }] },
]);
const toolTurn = (name: any, args: any) => sseBody([
...
function mountSite({
...
}: { hash?: string; aiSettings?: any; aiChat?: any; aiLedger?: any; responses?: any[]; onFetch?: (request: any) => unknown; } = {}) {
  const { document, container } = createStubHost();

  let routeCb: any = null;
  ...
  const app = createSiteApp({
    ...
    listenHash: (cb: any) => { routeCb = cb; cb(parseHash(hash)); },
    navigate: (h: any) => routeCb(parseHash(h)),
    storage: { read: () => null, write: () => { } },
    aiStorage: { read: () => aiData, write: (data: any) => { aiData = JSON.parse(JSON.stringify(data)); } },
    aiChat: { read: () => chatData, write: (data: any) => { chatData = JSON.parse(JSON.stringify(data)); } },
    aiLedger: {
      read: () => ledgerData,
      write: (data: any) => { ledgerData = JSON.parse(JSON.stringify(data)); },
    },
    aiFetch: (url: any, init: any) => {
  ...
  return {
    app, container, go: (h: any) => routeCb(parseHash(h)), requests,
    settings: () => aiData, chat: () => chatData, ledger: () => ledgerData,
  };
}

/** Open the panel (which is also what reads the ledger) and let it settle. */
async function openPanel(site: any) {
  fire(find(site.container, (n: any) => n.attributes?.get('class') === 'ai-launch'), 'click');
  await new Promise((resolve) => setTimeout(resolve, 0));
  return serialize(site.container);
}

function find(node: any, pred: any): any {
...
async function settle(app: any, max: number = 50) {
  for (let i = 0; i < max; i++) {
...
describe('website — the AI assistant panel', function () {
  ...
  it('shows the settings form until configured, then hides it', function () {
    ...
    fire(find(configured.container, (n: any) => n.attributes?.get('class') === 'ai-launch'), 'click');
  ...
  it('drives the play surface: a tool call navigates, loads inputs and the reply renders', async function () {
    ...
    assert.deepStrictEqual(messages.map((m: any) => m.role), ['user', 'assistant']);
    ...
    assert.strictEqual(secondBody.messages.some((m: any) => m.role === 'tool'), true,
  ...
  it('surfaces a provider HTTP error without crashing', async function () {
    ...
    assert.deepStrictEqual(app.getState().ai.messages.map((m: any) => m.role), ['user']);
  });
  ...
  it('persists the transcript and restores it on the next boot', async function () {
    ...
    assert.deepStrictEqual(first.chat().messages.map((m: any) => m.role), ['user', 'assistant'],
    ...
    assert.deepStrictEqual(second.app.getState().ai.messages.map((m: any) => m.role),
      ['user', 'assistant']);
    fire(find(second.container, (n: any) => n.attributes?.get('class') === 'ai-launch'), 'click');
...
describe('website — the assistant ledger (a goal that outlives the tab)', function () {
  ...
  it('puts the objective in front of the model on every turn', async function () {
    ...
    assert.ok(site.chat().messages.every((m: any) => !String(m.content).includes('Port the CSV importer.')
      || m.role === 'user'));
  });
  ...
  it('says how many rounds a compacted session archived', async function () {
    ...
    const ledger = createLedger({
      storage: createSlotLedgerStorage({
        read: () => slot, write: (data: any) => { slot = data; },
      }),
    });
...
describe('website — WebMCP over @jarenjs/ai', function () {
  it('registers the generalized toolbox and its tools drive the site', async function () {
    ...
    const app = createSiteApp({
      ...
      debounceMs: 0,
      fetchJson: () => Promise.reject(new Error('404')),
      listenHash: (cb: any) => { routeCb = cb; cb(parseHash('#/play')); },
      navigate: (h: any) => routeCb(parseHash(h)),
      share: (h: any) => { shared.push(h); return `https://x/${h}`; },
      storage: {
        read: () => ({ experiments: { demo: { engine: 'path', inputs: { selector: '$..price', data: '{}' }, savedAt: 'x' } } }),
        write: () => { },
      },
      modelContext: { provideContext: ({ tools }: { tools?: any; }) => { registered = tools; } },
      onError: (err: any) => { throw err; },
    });
    await app.ready;
    const tool = (name: any) => registered.find((t: any) => t.name === name);
    ...
    assert.strictEqual((await validate.execute({ schema: { type: 'integer' }, data: 5 })).valid, true);
    assert.strictEqual((await validate.execute({ schema: { type: 'integer' }, data: 'no' })).valid, false);

    const run = tool('jaren_run_engine');
    const result = (await run.execute({ engine: 'path', inputs: { selector: '$.a', data: '{"a":42}' } }));
    ...
    assert.match((await run.execute({ engine: 'no-such', inputs: {} })).error, /invalid input/);

    // the catalogue lists validate + every engine, with input fields
    const engines = (await tool('jaren_list_engines').execute({}));
    ...
    const state = (await tool('jaren_get_state').execute({}));
    ...
    assert.deepStrictEqual((await tool('jaren_list_experiments').execute({})).experiments.map((e: any) => e.name), ['demo']);
    assert.deepStrictEqual((await tool('jaren_load_experiment').execute({ name: 'demo' })), { ok: true });
    assert.strictEqual(app.getState().play.source.selector, '$..price', 'the legacy experiment loaded into play');
    assert.match((await tool('jaren_load_experiment').execute({ name: 'nope' })).error, /no experiment/);

    // the share tool builds and copies a play-session link
    assert.strictEqual((await tool('jaren_share_link').execute({})).ok, true);
    assert.strictEqual(shared.length, 1);
    assert.match(shared[0], /^#\/play\?s=/);

    const nav = tool('jaren_navigate');
    const unsupported = (await nav.execute({ page: 'docs', params: { s: 'jslt' } }));
    assert.strictEqual(unsupported.ok, false);
    assert.match(unsupported.error, /not hosted/);
    assert.strictEqual(app.getState().route.page, 'play');

    // the example library: ready-to-run inputs per engine, by list or label
    const examples = tool('jaren_get_examples');
    const forPath = (await examples.execute({ engine: 'path' }));
    ...
    assert.deepStrictEqual((await examples.execute({ engine: 'path', label: forPath[0].label })), forPath[0]);
    const miss = (await examples.execute({ engine: 'path', label: 'nope' }));
    assert.match(miss.error, /no example/);
    assert.deepStrictEqual(miss.labels, forPath.map((e: any) => e.label));
    assert.match((await examples.execute({ engine: 'no-such' })).error, /invalid input/,
    ...
    assert.match((await tool('jaren_save_experiment').execute({ name: '' })).error, /invalid input/,
    ...
    const flowTemplates = (await tool('jaren_flow_get_templates').execute({}));
    assert.ok(flowTemplates.some((t: any) => t.name === 'review' && t.kind === 'fsm'));
    const review = (await tool('jaren_flow_get_templates').execute({ name: 'review' }));
    const wrote = (await tool('jaren_flow_write').execute({ kind: 'fsm', doc: review.doc }));
    ...
    const rejected = (await tool('jaren_flow_write').execute({ kind: 'fsm', doc: brokenFsm }));
    ...
    assert.strictEqual((await tool('jaren_flow_check').execute({ kind: 'fsm', doc: brokenFsm })).ok, false);
    assert.strictEqual((await tool('jaren_flow_check').execute({ kind: 'fsm', doc: review.doc })).ok, true);
    const patched = (await tool('jaren_flow_patch').execute({
    ...
    const enrich = (await tool('jaren_flow_get_templates').execute({ name: 'enrich' }));
    assert.strictEqual((await tool('jaren_flow_write').execute({ kind: 'dag', doc: enrich.doc })).ok, true);
    assert.strictEqual(app.getState().flow!.kind, 'dag');
    (await tool('jaren_navigate').execute({ page: 'flow' }));
    assert.strictEqual(app.getState().route.page, 'flow');
  });

  it('the public DAG gate resolves task names without requiring or invoking their runtime handlers', function () {
    assert.strictEqual(validateFlowDocument('dag', { $dag: '0.1', nodes: { input: { kind: 'input' }, task: { kind: 'task', run: 'never-called' }, out: { kind: 'output' } }, edges: [{ from: 'input', to: 'task' }, { from: 'task', to: 'out' }] }).valid, true);
  });
  ...
  it('the built-in storage/settings defaults boot and persist without env wiring', function () {
    ...
    const app = createSiteApp({
      ...
      schedule: (f: any) => f(),
      fetchJson: () => Promise.reject(new Error('404')),
      listenHash: (cb: any) => { routeCb = cb; cb(parseHash('#/')); },
      navigate: (h: any) => routeCb(parseHash(h)),
    });
...
describe('website — reasoning-model streaming', function () {
  const reasoningTurn = (thinking: any, content: any) => sseBody([
