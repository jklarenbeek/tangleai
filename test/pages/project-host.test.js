...
async function site(t, extra) {
  const { container, document } = createStubHost(); let registered;
  const host = mountPageHost({ root: container, assistantNode: document.createElement('aside'), hash: '#/project', schedule: f => f(), debounceMs: 0,
    slots: { settings: { read: () => ({ provider: 'openrouter', baseUrl: '', model: 'test', apiKey: 'test-key' }), write() {} } },
    webmcp: { context: { provideContext: ({ tools }) => { registered = tools; } } }, ...extra });
  t.after(() => host.dispose()); await host.webmcp.ready;
  const app = { getState: () => ({ project: host.project().read().document }), destroy: host.dispose,
...
it('the assistant model runner uses the injected private worker and app teardown closes it', async (t) => {
  let terminated = 0;
  const releases = [];
  const { app, tool } = await site(t, { projectWorker: () => {
