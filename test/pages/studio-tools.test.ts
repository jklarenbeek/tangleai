...
import { createStubHost, serialize } from '../assistant/dom.stub.ts';
const mounted: any[] = [];
afterEach(async () => { for (const host of mounted.splice(0)) await host.dispose(); });
async function mountSite({ hash = '#/project', modelContext, stored = null }: { hash?: string; stored?: any; modelContext?: any; } = {}) {
  const { container, document } = createStubHost(), assistantNode = document.createElement('aside');
  let data = stored;
  const host = mountPageHost(({
    root: container, assistantNode, hash, schedule: (fn: any) => fn(), debounceMs: 0,
    slots: { projects: { read: () => data, write: (value: any) => { data = structuredClone(value); } } },
    webmcp: { context: modelContext }, onError: (error: any) => { throw error; }
  } as any));
  mounted.push(host); await host.webmcp.ready;
  const app = {
    getState: () => { const snapshot = host.project().read(); return { route: { page: host.page() }, project: { ...snapshot.document, revision: snapshot.stageRevision } }; },
...
it('rides along on the studio write/patch tool results', async function () {
  ...
  assert.ok(patched.renderProblems.some((p: any) => /props\.schema did not resolve/.test(p)),
...
describe('website — the Studio assistant tools (WebMCP)', async function () {
  it('registers the four studio tools and they author, patch and read the project app file', async function () {
    ...
    const appFileDoc = () => JSON.parse(
      app.getState().project.files.find((f: any) => f.kind === 'app').text);

    // the seed library: list, then by name
    const list = (await tool('jaren_get_templates').execute({}));
    assert.deepStrictEqual(list.map((t: any) => t.name), ['form', 'dashboard', 'minisite']);
  ...
  it('the project tools see the WHOLE file tree — a project is not just its app document', async function () {
    ...
    assert.deepStrictEqual(listed.files.map((f: any) => f.name),
      ['app.json', 'stats.query', 'stats.data'], 'every file is listed, not just the app');
    assert.strictEqual(listed.active, 'app.json');
    assert.ok(listed.files.every((f: any) => f.valid), 'each file reports its own validity');
  ...
  it('jaren_get_state answers with the project files when the project is on screen', async function () {
    ...
    assert.deepStrictEqual(onProject.files.map((f: any) => f.name),
  ...
  it('a write into a project WITHOUT an app file adds one (and activates it)', async function () {
    ...
    assert.deepStrictEqual(JSON.parse(p.files.find((f: any) => f.name === 'app.json').text), doc);
  });
});
