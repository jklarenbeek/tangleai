...
import { mountGame as createGameHost } from '../../apps/pages/src/demos/game/mount.js';
import { createGameRuntime } from '../../apps/pages/src/demos/game/runtime.js';
import {
  PUZZLES, ITEMS, SCENERY, CHARACTERS, DIALOGUE,
} from '../../apps/pages/src/demos/game/gameContent.js';
import { createStubHost } from '../assistant/dom.stub.js';

/** A headless site parked on #/game. */
function mountGame() {
  const { container } = createStubHost();
  return createGameHost(container, { schedule: f => f(), onError: error => { throw error; } });
}
...
describe('received structured character dialogue', () => {
  ...
  const response = value => new Response(JSON.stringify({ choices: [{ message: { role: 'assistant', content: JSON.stringify(value) } }] }), { headers: { 'content-type': 'application/json' } });
  const begin = app => { app.dispatch('game/start'); app.dispatch('game/verb', 'talk'); app.dispatch('game/hotspot', 'gullbert'); app.dispatch('game/ask-draft', null, { target: { value: 'What is a receipt?' } }); app.dispatch('game/ask-send'); };
  const settle = async app => { for (let i = 0; i < 50 && app.read().thinking; i++) await new Promise(resolve => setTimeout(resolve, 1)); };
  it('uses the npc_line schema and renders a scripted reply', async t => {
    let request;
    const { container } = createStubHost();
    const app = createGameHost(container, { schedule: fn => fn(), settings: () => settings, aiFetch: async (_url, init) => { request = JSON.parse(init.body); return response({ line: 'A receipt keeps the gull honest.', mood: 'wry' }); } });
  ...
  it('keeps provider failures visible without changing the game position', async t => {
    const { container } = createStubHost();
    const app = createGameHost(container, { schedule: fn => fn(), settings: () => settings, aiFetch: async () => new Response('{"error":{"message":"scripted refusal"}}', { status: 400 }) });
    t.after(() => app.dispose()); begin(app); await settle(app);
  ...
  it('aborts disposal, ignores a late reply, and a remount clears transient thinking state', async () => {
    ...
    const { container } = createStubHost();
    const app = createGameHost(container, { schedule: fn => fn(), settings: () => settings, aiFetch: (_url, init) => { signal = init.signal; return pending; } });
    ...
    const resumed = createGameHost(container, { schedule: fn => fn(), state: snapshot });
