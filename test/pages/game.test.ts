...
import { mountGame as createGameHost } from '../../apps/pages/src/demos/game/mount.ts';
import { createGameRuntime } from '../../apps/pages/src/demos/game/runtime.ts';
import {
  PUZZLES, ITEMS, SCENERY, CHARACTERS, DIALOGUE,
} from '../../apps/pages/src/demos/game/gameContent.ts';
import { createStubHost } from '../assistant/dom.stub.ts';

/** A headless site parked on #/game. */
function mountGame() {
  const { container } = createStubHost();
  return createGameHost((container as any), { schedule: (f: any) => f(), onError: (error: any) => { throw error; } });
}
...
describe('website — the adventure game', function () {
  ...
  it('exports the admiralty forms to CSV and round-trips a save via JSONX', function () {
    let saved: any = null; const downloads: any[] = [];
    ...
    const calls: any[] = [];
    const dispatch = (a: any, p: any) => calls.push({ a, p });
    ...
    const restore = calls.find((c: any) => c.a === 'game/restore');
  ...
  it('the dynamic tier degrades gracefully without a key', function () {
    ...
    const rt = createGameRuntime({
      getApp: () => ({ getState: () => state }),
      isConfigured: (s: any) => s.provider === 'openrouter' && s.apiKey.trim() !== '',
    });
    rt.effects['game-ai-say']({}, (action: any, payload: any) => calls.push({ action, payload }));
    assert.match(JSON.stringify(calls), /would answer live/i);
  });
});

describe('received structured character dialogue', () => {
  const settings = { provider: 'openrouter', model: 'scripted', baseUrl: 'https://fixture.invalid/v1', apiKey: 'scripted-fixture' };
  const response = (value: any) => new Response(JSON.stringify({ choices: [{ message: { role: 'assistant', content: JSON.stringify(value) } }] }), { headers: { 'content-type': 'application/json' } });
  const begin = (app: any) => { app.dispatch('game/start'); app.dispatch('game/verb', 'talk'); app.dispatch('game/hotspot', 'gullbert'); app.dispatch('game/ask-draft', null, { target: { value: 'What is a receipt?' } }); app.dispatch('game/ask-send'); };
  const settle = async (app: any) => { for (let i = 0; i < 50 && app.read().thinking; i++) await new Promise(resolve => setTimeout(resolve, 1)); };
  it('uses the npc_line schema and renders a scripted reply', async t => {
    let request;
    const { container } = createStubHost();
    const app = createGameHost((container as any), { schedule: (fn: any) => fn(), settings: () => settings, aiFetch: async (_url: any, init: any) => { request = JSON.parse(init.body); return response({ line: 'A receipt keeps the gull honest.', mood: 'wry' }); } });
    t.after(() => app.dispose()); begin(app); await settle(app);
    assert.strictEqual(request!.response_format.json_schema.name, 'npc_line');
    assert.match(JSON.stringify(app.read().log), /A receipt keeps the gull honest/); assert.strictEqual(app.read().thinking, false);
  });
  it('keeps provider failures visible without changing the game position', async t => {
    const { container } = createStubHost();
    const app = createGameHost((container as any), { schedule: (fn: any) => fn(), settings: () => settings, aiFetch: async () => new Response('{"error":{"message":"scripted refusal"}}', { status: 400 }) });
    t.after(() => app.dispose()); begin(app); await settle(app);
    assert.match(JSON.stringify(app.read().log), /briefly speechless/); assert.strictEqual(app.read().room.current, 'wharf'); assert.strictEqual(app.read().thinking, false);
  });
  it('aborts disposal, ignores a late reply, and a remount clears transient thinking state', async () => {
    let release, signal;
    const pending = new Promise(resolve => { release = resolve; });
    const { container } = createStubHost();
    const app = createGameHost((container as any), { schedule: (fn: any) => fn(), settings: () => settings, aiFetch: (_url: any, init: any) => { signal = init.signal; return pending; } });
    begin(app); await Promise.resolve(); const snapshot = app.read(); app.dispose(); app.dispose();
    assert.strictEqual(signal!.aborted, true); release!(response({ line: 'must not land' })); await new Promise(resolve => setTimeout(resolve, 10));
    assert.deepStrictEqual(app.read(), snapshot);
    const resumed = createGameHost((container as any), { schedule: (fn: any) => fn(), state: snapshot });
    assert.strictEqual(resumed.read().thinking, false); assert.strictEqual(resumed.read().room.current, snapshot.room.current); resumed.dispose();
  });
});
