...
const sse = content => new Response(`data: ${JSON.stringify({ choices: [{ delta: { role: 'assistant', content } }] })}\n\ndata: ${JSON.stringify({ choices: [{ delta: {}, finish_reason: 'stop' }] })}\n\ndata: [DONE]\n\n`);
function fixture(t, options = {}) {
  let app, settings = options.settings ?? configured, transcript = options.transcript ?? null;
  const requests = [], ledger = createLedger();
  const controller = createAssistantController({ getApp: () => app, ledger, toolbox: createToolbox(),
    system: options.system ?? 'Help this host finish its task.', headers: options.headers,
    aiStorage: { read: () => settings, write: value => { settings = structuredClone(value); } },
    aiChat: { read: () => transcript, write: value => { transcript = structuredClone(value); } },
    aiFetch: (url, init) => { requests.push({ url, init }); return options.fetch ? options.fetch(url, init) : Promise.resolve(sse('Finished.')); },
    ...options.controller,
  });
...
it('streams with injected host instructions, preserves request settings and resumes its transcript', async t => {
  for (const name of ['Research desk', 'Workshop']) {
    const host = fixture(t, { system: name, headers: { 'X-Title': name } }), states = [];
    host.app.subscribe(state => states.push(state.ai.pending));
...
it('cancels a turn and prevents late results from restoring cleared conversation state', async t => {
  ...
  const host = fixture(t, { fetch: (_url, init) => new Promise(resolve => { finish = () => resolve(sse('too late')); entered(init.signal); }) });
...
it('disposal aborts provider probes, releases owned storage once and ignores later effects', async t => {
  let entered, closed = 0;
  const ready = new Promise(resolve => { entered = resolve; });
  const host = fixture(t, { controller: { onDispose: () => { closed++; } }, fetch: (_url, init) => new Promise((_resolve, reject) => {
...
it('a non-cooperating late provider cannot execute host tools after cancellation', async t => {
  let start, finish, calls = 0, requests = 0;
  ...
  const host = fixture(t, { controller: { toolbox }, fetch: () => ++requests === 1 ? new Promise(resolve => {
