...
import { createStubHost, fire, serialize } from './dom.stub.ts';
const settings = { provider: 'custom', baseUrl: 'https://fixture.invalid/v1', model: 'fixture', apiKey: '' };
const find = (node: any, predicate: any) => predicate(node) ? node : (node.childNodes ?? []).map((child: any) => find(child, predicate)).find(Boolean);
const settle = () => new Promise(resolve => setTimeout(resolve, 0));
function host(t: any, options: Record<string, any> = {}) {
  const { container, document } = createStubHost();
  const handle = mountAssistant((container as any), { schedule: fn => fn(), onError: error => { throw error; }, ...options });
...
it('mounts different host copy and capabilities without shared settings or datalist IDs', async t => {
  ...
  const aid = find(a.container, (n: any) => n.tagName === 'datalist').attributes.get('id');
  const bid = find(b.container, (n: any) => n.tagName === 'datalist').attributes.get('id');
  assert.notEqual(aid, bid);
  const model = find(a.container, (n: any) => n.attributes?.get('list') === aid);
...
it('updates branding while preserving the composer node, draft and stream display', async t => {
  ...
  const textarea = find(container, (n: any) => n.tagName === 'textarea');
  textarea.value = 'Keep my draft'; fire(textarea, 'input');
  handle.update({ title: 'Renamed host' });
  assert.equal(find(container, (n: any) => n.tagName === 'textarea'), textarea);
...
it('cancels with the visible control and removes DOM and listeners on repeated disposal', async t => {
  let start: any, signal, closed = 0;
  const ready = new Promise(resolve => { start = resolve; });
  const { handle, container } = host(t, {
    settings, open: true, onDispose: () => { closed++; }, aiFetch: (_url: any, init: any) => new Promise((_resolve, reject) => {
      signal = init.signal; start(); signal.addEventListener('abort', () => reject(new Error('cancelled')), { once: true });
    })
  });
  const draft = find(container, (n: any) => n.tagName === 'textarea');
  draft.value = 'Wait for me'; fire(draft, 'input'); handle.dispatch('ai/send'); await ready;
  const cancel = find(container, (n: any) => n.tagName === 'button' && serialize(n).includes('Cancel'));
...
it('assistant reply heading ids remain outside the host page namespace', async t => {
  const { container } = createStubHost();
  const host = mountAssistant((container as any), { open: true, transcript: { messages: [{ role: 'assistant', content: '## md\n\nSee [below](#md).\n' }] }, schedule: fn => fn() });
