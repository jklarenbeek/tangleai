...
function sseBody(chunks: any) {
  return chunks.map((c: any) => `data: ${typeof c === 'string' ? c : JSON.stringify(c)}\n\n`).join('')
    + 'data: [DONE]\n\n';
}

/** A Map-backed adapter; `async: true` answers promises, as a store would. */
function mapCache({ async = false }: { async?: boolean; } = {}) {
  const map = new Map();
  return {
    map,
    get: (key: any) => (async ? Promise.resolve(map.get(key)) : map.get(key)),
    set: (key: any, value: any) => { map.set(key, value); return async ? Promise.resolve() : undefined; },
  };
}

/** A chat client over a counting fetch that streams one fixed reply. */
function chatClient(cache: any, options: Record<string, any> = {}) {
  const calls: any[] = [];
...
describe('ai — the replay seam on the chat client', function () {
  ...
  it('an adapter that fails open is the adapter\'s own decision', async function () {
    ...
    const open = {
      ...
      set: (key: any, value: any) => { failing.map.set(key, value); },
    };
  ...
  it('refuses a cache that is not the seam at construction', function () {
    assert.throws(() => createChatClient({ provider: 'ollama', model: 'm', cache: ({ get: 1 } as any) }),
...
function embedClient(cache: any, options: Record<string, any> = {}) {
  const calls: any[] = [];
  const client = createEmbeddingClient({
    ...
    fetch: (url: any, init: any) => {
      ...
      calls.push(input);
      const data = input.map((text: any, index: any) => ({ object: 'embedding', index, embedding: VECTORS[text as keyof typeof VECTORS] })).reverse();
...
describe('ai — the replay seam on the embeddings client', function () {
  ...
  it('a stored entry is JSON-only: the vector is a plain array of numbers', async function () {
    ...
    assert.ok(Array.isArray(entry.vector) && entry.vector.every((x: any) => typeof x === 'number'));
  ...
  it('probeEmbeddings never consults a cache', async function () {
    const probe = await probeEmbeddings({
      ...
      ... ({ cache: { get: () => { throw new Error('must not be asked'); }, set: () => { } } } as any),
    });
