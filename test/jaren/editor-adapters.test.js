...
function scripted(values, onCall = () => {}) {
  const requests = [];
  return { requests, endpoint: { provider: 'openrouter' }, complete: async request => {
    requests.push(request); await onCall(request); return { message: { content: JSON.stringify(values.shift()) } };
  } };
}
function flow(t, kind = 'fsm', document = fsm) {
  let app;
  const runtime = createFlowRuntime({ getFlow: () => app.getState().flow, tasks: { double: async ({ input }) => input * 2 } });
  const editor = createFlowController({ getApp: () => app, runtime });
  app = createApp({ state: { flow: createFlowState({ kind, document }) }, actions: FLOW_ACTIONS, view: [] },
...
function project(t) {
  ...
  const document = { project: '0.1', files: [
...
async function data(t) {
  ...
  const request = async (name, input) => { const outcome = await client.invoke(name, input); if (!outcome.ok) throw new Error(outcome.error.message); return outcome.value; };
  const runtime = createDataRuntime({ model, query, seeds: [{ id: 'one', value: 7 }], lifecycleTarget: {}, transport: () => ({
    boot: () => request('data.init', null), request, bounded: (_stage, work) => work(),
    subscribe: (input, handlers) => client.subscribe('data.live', input, handlers),
...
it('Studio candidates preserve assembled imports, source order and concurrent manual work', async t => {
  ...
  const client = scripted([{ n: 2 }], () => app.dispatch('project/files-set', {
    files: editor.read().document.files.map(f => f.name === 'state' ? { ...f, text: '{"n":3}' } : f),
  }));
  ...
  assert.equal(accepted.ok, true); assert.deepEqual(editor.read().document.files.map(f => f.name), ['app', 'state']);
...
it('Flow proposals repair compiler failures, publish through shared history, and execute the real task registry', async t => {
  const { editor } = flow(t, 'dag', dag), bad = structuredClone(dag);
...
it('Flow refuses stale proposals, preserves run results and keeps template/write/patch/check operations', async t => {
  ...
  const adapter = createFlowAdapter({ editor, client: scripted([fsm], () => app.dispatch('flow/state-minted', 'manual')),
    templates: { machine: { kind: 'fsm', document: fsm } } });
  const proposal = await adapter.propose({ prompt: 'Use this machine.' });
  assert.equal((await adapter.accept(proposal)).conflict, true); assert.equal(editor.read().document.states.at(-1), 'manual');
...
it('Data repairs a query, explicitly executes through SQLite and reports the contract plan', async t => {
  ...
  assert.equal(result.ok, true, JSON.stringify(result)); assert.deepEqual(result.result, [{ id: 'one', value: 7 }]);
  assert.match(result.explain.sql, /SELECT/); assert.deepEqual(adapter.inspect().explain, result.explain);
  assert.deepEqual((await adapter.run({ externals: { limit: 9 } })).result, []);
});
it('Data conflicts preserve manual buffers, while accepting a model never recreates its store', async t => {
  const { editor, app, opens } = await data(t), changed = structuredClone(model);
  ...
  assert.equal((await adapter.accept(proposal)).conflict, true); assert.equal(editor.read().buffers.queryText, '{unfinished');
  assert.equal((await adapter.write(proposal.candidate, { expectedRevision: editor.read().revision })).ok, true);
  assert.equal(opens(), 0); assert.deepEqual((await adapter.run()).result, [{ id: 'one', value: 7 }]);
