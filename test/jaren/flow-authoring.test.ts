...
const load = (p: any) => JSON.parse(readFileSync(new URL(p, import.meta.url), 'utf8'));
const fsmSchema = load(import.meta.resolve('@jarenjs/flow/schemas/jaren-fsm.schema.json'));
const querySchema = load(import.meta.resolve('@jarenjs/json/schemas/jaren-query.schema.json'));
...
describe('recipe 2 — authoring a machine with semantic repair', function () {
  it('a schema-valid but compile-broken machine repairs; the JF code + docPath reach the prompt', async function () {
    ...
    const result = (await out.generate([{ role: 'user', content: 'a review machine' }]) as any);
  ...
  it('the reliable recipe as a one-liner: refs resolve the composed schema, gate adds compile', async function () {
    ...
    const result = (await out.generate([{ role: 'user', content: 'a machine' }]) as any);
    assert.deepStrictEqual(result.value, fixed);
...
describe('recipe 3 — a model as a dataflow node', function () {
  ...
  const llmTask = (client: any, pick: any) => ({ with: w, input }: { with?: any; input?: any; }, signal: any) =>
  ...
  it('runs the model as an ordinary task node and threads its output downstream', async function () {
    ...
    const compiled = compileDag(dag, {
      tasks: { llm: llmTask(client, (r: any) => JSON.parse(r.message.content)) },
    });
  ...
  it('a run abort reaches the model client through the shared signal', async function () {
    ...
    const client = {
      ...
      complete: (req: any) => new Promise((_resolve, reject) => {
    ...
    const compiled = compileDag(dag, {
      tasks: { llm: llmTask((client as any), (r: any) => r) },
    });
    const controller = new AbortController();
    const run = compiled.run(null, { signal: controller.signal });
    await new Promise((r) => setImmediate(r));
    controller.abort();
    await assert.rejects(run, (err) => (err as any).code === 'JF2007');
