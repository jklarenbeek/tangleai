...
import { measureAuthoring, legacyAuthoringEvidence } from '../../benchmark/lib/authoring.ts';
import { authoringScorecard } from '../../benchmark/lib/authoring-probes.ts';
import { AUTHORING_ATTEMPT_SCHEMA } from '../../benchmark/lib/authoring-schema.ts';
import { JarenValidator } from '@jarenjs/validate';
import { readFileSync } from 'node:fs';

describe('authoring evidence', () => {
  const base = {
    profile: { name: 'fixture', provider: 'scripted', model: 'test', deadlineMs: 10 },
    grammar: 'program', schema: { type: 'object' }, messages: [{ role: 'user', content: 'test' }],
    measuredAt: 'fixture'
  };
  const client = (reply: any) => ({ endpoint: { provider: 'openrouter' }, complete: async () => reply });
  ...
  it('reproduces fixture evidence byte for byte through the benchmark entry', async () => {
    ...
    assert.ok(a.rows.every((row: any) => row.outcome === 'correct'), JSON.stringify(a.rows));
    ...
    for (const name of ['authoring-fixture', 'authoring-live', 'authoring-remeasured-live']) {
      ...
      assert.ok(artifact.rows.every((row: any) => check(row) === true));
    }
  });
});
