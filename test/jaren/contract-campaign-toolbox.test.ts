...
it('the contract campaign client projects and executes tools over the real SQLite-backed HTTP boundary', async t => {
  ...
  const dispatcher = serveHttp(contract, {
    ...
    'product.save': async (input: any) => { await rows.put(input.product, input.id); return input.product; },
    'product.search': async (input: any) => (await rows.execute(scan) as any[]).filter((row: any) => input.q === undefined || row.name.includes(input.q)),
    'product.remove': () => true,
  ...
  const client = openHttpClient(contract, { baseUrl: `http://127.0.0.1:${withMember(server.address()!, 'port').port}` });
