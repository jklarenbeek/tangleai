...
import { createStylesheetAuthor, stylesheetGates, compileGate, literalBodyGate, nonEmptyGate, runGate, operatorArities, operatorNames, operatorCrib, describePaths, unknownOperatorGate, grammarKeywords, STYLESHEET_EXAMPLE } from '@tangleai/jaren/stylesheet';
...
describe('ai — the operator crib the profile displaces', function () {
  it('reads the vocabulary off the injected grammar, grouped by arity', function () {
    ...
    const find = (name: any) => groups.find((g) => g.names.includes(name))?.arity;
...
describe('ai — grounding a question in the data it runs on', function () {
  it('describes a sample as the JSONPaths that address it', function () {
    ...
    const many = describePaths({ rows: Array.from({ length: 50 }, (_: any, i) => ({ n: i })) });
  ...
  it('collapses an array to one entry, so the digest does not grow with the data', function () {
    ...
    const large = describePaths({ rows: Array.from({ length: 500 }, (_: any, i) => ({ a: i })) });
...
describe('ai — the author', function () {
  ...
  it('puts the crib, the example and the path digest in the request — and nothing else', async function () {
    ...
    const big = { ...SAMPLE, records: Array.from({ length: 400 }, (_: any, i) => ({ id: `R${i}`, class: 'upper', probability: i / 400 })) };
  ...
  it('does not escalate a model that answered badly', async function () {
    ...
    assert.deepStrictEqual(client.sent.map((r: any) => r.model), ['first', 'first', 'second', 'second'],
