...
import { createSpatialAuthor, spatialGates, prefixProximityGate, planarArithmeticGate, spatialOperatorGate, spatialCrib, spatialIntent, spatialSystemMessage, describeSpatialPaths, coordinateMembers, SPATIAL_OPERATORS, SPATIAL_EXAMPLE } from '@tangleai/jaren/spatial';
import { createStructuredOutput } from '@tangleai/models/structured';
import { operatorCrib, runGate, compileGate, unknownOperatorGate, STYLESHEET_EXAMPLE, stylesheetSystemMessage } from '@tangleai/jaren/stylesheet';
...
import { sectionOf } from '../format-sections.ts';
...
const PLACES = parseCsv(readFileSync(new URL('../json/fixtures/places.csv', import.meta.url), 'utf8'),
  { headers: true, typed: true }).map((row) => { const place = row as { name: string; lon: number; lat: number; }; return { name: place.name, at: [place.lon, place.lat] }; });
...
function replaying(...replies: any[]) {
  const sent: any[] = [];
  return {
    sent,
...
describe('ai — the prefix refusal (D7): bucketing offered as proximity', function () {
  it('is the profile\'s most valuable refusal: the document compiles, runs, and is wrong', function () {
    ...
    assert.ok(typeof outcome === 'object' && outcome !== null && 'errors' in outcome);
...
describe('ai — the planar refusal (D5): arithmetic over a coordinate member', function () {
  ...
  it('knows a coordinate by the sample, and by the names the world uses', function () {
    ...
    assert.strictEqual(withMember(planarArithmeticGate({ members: coordinateMembers({ counts: [3, 4] }) })(counts), 'errors').errors[0].code,
      'AI0231');
    const named = { $jslt: '0.1', rules: [{ match: '$', body: { $sub: ['$.a.lat', '$.b.latitude'] } }] };
    assert.strictEqual(withMember(planarArithmeticGate()(named), 'errors').errors[0].code, 'AI0231', 'lat/latitude need no sample');
    const geometry = { $jslt: '0.1', rules: [{ match: '$', body: { $mul: ['$.g.coordinates[1]', 2] } }] };
    assert.strictEqual(withMember(planarArithmeticGate()(geometry), 'errors').errors[0].code, 'AI0231', 'nor does coordinates[i]');
  });
...
describe('ai — the spatial author (recorded fixtures, no model run)', function () {
  ...
  it('answers the question it was asked, once the gates have passed', function () {
    ...
    assert.deepStrictEqual(run(SPATIAL_EXAMPLE).map((row: any) => row.name), ['Amsterdam', 'Utrecht', 'Rotterdam']);
  });
});
