...
import { createGeoToolbox, geoToolDefs, overlayRefusal, GEO_TOOL_NAMES, OVERLAY_TOOL_NAMES, GEOJSON_SCHEMA_ID } from '@tangleai/jaren/geo-tools';
...
describe('ai — the geo toolbox is exactly the seven tools of the design table', function () {
  ...
  it('refuses an artifact that is not the meta-schema', function () {
    ...
    assert.throws(() => createGeoToolbox(({} as any)), TypeError);
  });

  it('publishes the same seven over WebMCP', async function () {

    let registered: any[] = [];
    const binding = registerModelContext(toolbox(), {
      provideContext: ({ tools }: { tools?: any; }) => { registered = tools; },
    });
    assert.strictEqual((await binding.ready).status, 'registered');
    await binding.dispose();
    assert.deepStrictEqual(registered.map((t: any) => t.name), [...GEO_TOOL_NAMES]);
  });
});
...
describe('ai — every geo tool delegates to @jarenjs/core/geo (D1)', function () {
  ...
  it('has no arithmetic of its own: the kernel imports are the whole of its geometry', function () {
    const source = readFileSync(new URL('../../packages/jaren/src/geo-tools.ts', import.meta.url), 'utf8');
    ...
    const dir = new URL('../../packages/jaren/src/', import.meta.url);
...
describe('ai — a malformed geometry is refused by the validator, not caught', function () {
  it('names the offending position, with the schema to re-read', function () {
    ...
    assert.ok(rejected.errors.some((e: any) => e.instancePath === '/a/coordinates/0' && e.keyword === 'maximum'),
