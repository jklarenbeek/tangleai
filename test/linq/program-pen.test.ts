...
import * as p from '@tangleai/linq/program';
import { program as rootProgram, LinqBuildError } from '@tangleai/linq';
import { LinqBuildError as JarenLinqBuildError } from '@jarenjs/linq';
import { JarenValidator } from '@jarenjs/validate';
import { compileJsonQuery } from '@jarenjs/json/query';
import { PROGRAM_SCHEMA, PROGRAM_OPS, programSchema } from '@tangleai/agents/schemas/program';
import { PROGRAM_EXAMPLE, compileProgram, programGate, createProgramRunner } from '@tangleai/agents/program';
...
import { pairwiseProgram, needleProgram, extractingClient } from '../../benchmark/lib/horizon.ts';
...
const hands: { [K in p.Operation]: p.Step<K> } = {
...
function factory({ op, from, as, ...extra }: { [x: string]: any; op: any; from: any; as: any; }) {
  ...
  if (op === 'grep') return p.grep(from, as, extra as p.GrepOptions);
  return p[op as 'chunk' | 'stat' | 'peek'](from, as, extra);
}
function rebuild(hand: any) {
  let pen: any = p.program(['corpus', 'data']);
  for (const { op, from, as, ...extra } of hand.steps) {
    if (op === 'reduce') pen = pen.reduce((from as any), as, extra.query, extra.outputSchema ? { outputSchema: extra.outputSchema } : {});
...
function completeAround(op: p.Operation): { steps: p.StepDocument[]; } {
...
const withoutTiming = ({ ms: _ms, ...result }: { ms?: any; result?: any; }) => result;
...
describe('AI program pen', () => {
  it('covers every schema operation and every member of each step', () => {
    ...
    for (const schema of branches) assert.deepEqual(Object.keys(hands[schema.properties.op.const as p.Operation]), Object.keys(schema.properties));
  ...
  for (const [op, handStep] of Object.entries(hands)) {
    it(`${op}: exact factory/fluent output, grammar, compile, gate and scripted runner`, async () => {
      assert.equal(JSON.stringify(factory((handStep as any))), JSON.stringify(handStep));
      const hand = completeAround(op as p.Operation);
  ...
  it('captures pure queries without binding an environment into the document', async () => {
    ...
    const pen = start.select('data', 'n', (v: any) => v.get('n').add(1)).answer('n');
    ...
    assert.throws(() => p.select('data', 'n', (((_v: any, x: any) => x.unknown) as any)), { code: 'JL0104' });
    assert.deepEqual(p.reduce('found', 'n', (v: any) => v.count()).query, { $count: '$' });
  });
  it('preserves input and builder snapshots and refuses work after answer', () => {
    ...
    const b = a.step(p.chunk('corpus', 'pieces', (options as any)));
    options.size = 500;
    assert.equal(withMember(b.schema.steps[0], 'size').size, 200);
    ...
    assert.throws(() => (done as unknown as p.ProgramBuilder<any, false>).peek('corpus', 'x'), { code: 'JL0102' });
    assert.deepEqual(p.from(done.schema).schema, done.schema);
    for (const make of [() => p.program(([1] as any)), () => p.stat('', 'x'), () => p.chunk('corpus', ''),
    () => p.chunk('corpus', 'x', ({ content: 'payload' } as any)), () => a.step((null as any)), () => a.step(({ op: 'unknown' } as any)),
    () => a.step(({ op: 'answer', from: 'corpus', as: 'x' } as any)), () => p.select('data', 'x', (undefined as any))])
  ...
  for (const [op, patch] of Object.entries(invalid)) {
    it(`${op}: the target schema/compiler refuses invalid raw emission before any execution`, async () => {
      const hand = structuredClone(completeAround(op as p.Operation));
      const target = hand.steps.find((step: any) => step.op === op);
      assert.ok(target);
  ...
  it('leaves duplicate names, missing answer and size limits to the real compile/shape boundaries', () => {
    const duplicate = p.program(['data']).stat('data', 'x').peek('data', ('x' as any)).answer('x');
    assert.equal(withMember(gate(duplicate.schema), 'errors').errors[0].code, 'AI0202');
    assert.equal(withMember(gate(p.program(['data']).stat('data', 'x').schema), 'errors').errors[0].code, 'AI0203');
    const fat = p.program(['data']).select('data', 'x', { $const: 'x'.repeat(4001) }).answer('x');
    assert.equal(withMember(gate(fat.schema), 'errors').errors[0].code, 'AI0205');
  });
});

it('the root exposes the program namespace and preserves the shared Jaren error identity', () => {
  assert.equal(rootProgram.program, p.program);
  assert.equal(LinqBuildError, JarenLinqBuildError);
  assert.deepEqual(rootProgram.program(['data']).stat('data', 'meta').answer('meta').schema,
    p.program(['data']).stat('data', 'meta').answer('meta').schema);
  assert.throws(() => p.program(['']), error => error instanceof LinqBuildError && error.code === 'JL0101');
});
