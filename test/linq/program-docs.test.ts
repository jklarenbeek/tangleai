...
import * as program from '@tangleai/linq/program';

const root = fileURLToPath(new URL('../../', import.meta.url));
const markdown = readFileSync(join(root, 'packages/linq/docs/PROGRAM-PEN.md'), 'utf8');
function bodyOf(heading: any) {
...
const names = (section: any, name: any) => section.includes(`\`${name}(`) || section.includes(`\`.${name}(`) || section.includes(`\`${name}\``) || section.includes(`\`.${name}\``);
...
it('the received program worked example exists and emits its exact following JSON fence', async t => {
  ...
  for (const [index, [js, json]] of pairs.entries()) {
    ...
    const emitted = Object.values(module)[0] as { schema?: unknown; toJSON(): unknown; };
...
it('the mapping table names every callable export and builder method', () => {
  ...
  assert.deepEqual(vocabulary.filter((name: any) => !names(bodyOf('## 2. The mapping table'), name)), []);
});
...
it('documented refusal codes equal the program source codes in both directions', () => {
  const source = readFileSync(join(root, 'packages/linq/src/program.ts'), 'utf8');
