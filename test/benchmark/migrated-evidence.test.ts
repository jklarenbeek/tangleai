...
const loadBench = (name: any) => JSON.parse(readFileSync(new URL((name === 'long-horizon' ? '../../benchmark/jaren-long-horizon.json' : '../../benchmark/historical/' + name + '.json'), import.meta.url), 'utf8'));
it('derives the retrieval suite, with the oracle proof and the synthetic-corpus note in view', function () {
  ...
  for (const size of data.meta.sizes) {
    const oracle = data.rows.find((r: any) => r.size === size && r.policy === 'oracle');
    ...
    for (const key of ['random', 'recency', 'tag+recency', 'near', 'floor'])
      assert.ok(data.rows.some((r: any) => r.size === size && r.policy === key), `${key} row at ${size}`);
  }
  ...
  assert.ok(!data.rows.some((r: any) => r.policy === 'near-live'));
});
it('derives the long-horizon suite, keeping both tasks and both shapes', function () {
  ...
  const compacting = data.rows.filter((r: any) => r.task === 'pairwise' && r.compacted);
  ...
  assert.ok(compacting.some((r: any) => r.ceiling === 0),
