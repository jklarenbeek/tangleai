...
import { liveClients, scriptedEnv, scriptedFetch, scriptedProgram, completion, listedMemories } from '../fixtures/scripted-wire.ts';
...
describe('the LoCoMo answer-path instrument', { skip: missing }, () => {
  ...
  it('live progress says what was ANSWERED, never what was planned', async () => {
    // conv-30 holds no category-3 question, so one per category is exactly
    // three; the built-in embedder keeps the plan at those three calls
    const env = scriptedEnv({ TANGLE_AI_EMBEDDING_MODEL: '', TANGLE_AI_MAX_CALLS: '3', TANGLE_AI_MAX_CONCURRENCY: '1' });
    let call = 0;
    const { fetch } = scriptedFetch({
      chat: () => {
        call++;
        // q1 answers after one repair (two calls); q2's wire is down; q3
        // spends the last turn and its repair is refused by the budget
        if (call === 1 || call === 4) return { content: 'just prose' };
        if (call === 3) return new Response('nope', { status: 500 });
        return { content: JSON.stringify({ answer: 'yes', citations: [] }) };
      },
    });
    const lines: string[] = [];
    const live = await runLocomoQaLive(available!, {
      env, ...liveClients(env, fetch), rows: ['near-raw'],
      samples: ['conv-30'], perCategory: 1, adversarial: 0,
      onProgress: (message) => lines.push(message),
    });
    const row = rowOf(live, 'near-raw');
    assert.equal(row.questions.planned, 3);
    assert.equal(row.questions.answered, 1);
    assert.deepEqual(row.questions.unanswered, { wire: 1, budget: 1 });
    const line = lines.find((l) => l.startsWith('near-raw conv-30:'))!;
    assert.equal(line, 'near-raw conv-30: planned 3 / answered 1 / wire 1 / budget 1 / invalid 0');
    assert.ok(!lines.some((l) => /3 sampled questions answered|3 answered/.test(l)),
      'the plan was never printed as an answer count');
    assert.equal(validateLive(live).valid, true, JSON.stringify(validateLive(live).errors?.slice(0, 5)));
  });
