...
describe('the worked recursive program at actual depth', () => {
  for (const depth of [0, 1, 2]) {
    it(`preserves failures, nulls and complete large values at depth ${depth}`, async () => {
      ...
      const agent = createLongHorizonAgent({
        environment, client, ...queryOptions, depth,
      ...
      const leaf = result.trajectory.filter((event: any) => event.kind === 'program' && event.depth === depth);
      assert.equal(leaf.reduce((sum: any, event: any) => sum + event.failed, 0), 1);
    });
  }
  ...
  it('keeps an oversized child as a diagnostic under an explicit answer limit', async () => {
    ...
    const result = await createLongHorizonAgent({
      environment, client, ...queryOptions, depth: 1,
    ...
    assert.equal(result.trajectory.find((event: any) => event.kind === 'program' && event.depth === 0).failed, 1);
  });
});
