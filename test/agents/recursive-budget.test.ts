...
import { createLongHorizonAgent, createBudgetAccount } from '@tangleai/agents/recursive';
...
function corpusText(docs: number = 6) {
  return Array.from({ length: docs }, (_: any, d) =>
    Array.from({ length: 4 }, (_: any, i) =>
      `{"id":"D${d}R${i}","value":${100 + d * 10 + i}}`).join(' ; ')).join('\n');
}

function counting(tokensPerCall: number = 10) {
  const state = { calls: 0 };
  return {
    state,
