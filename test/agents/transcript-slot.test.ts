...
import { createAgent, transcriptText } from '@tangleai/agents/agent';
...
function fetchBox() {
  ...
  toolbox.add({
    ...
    execute: ({ index }: any) => record(index),
  });
  return toolbox;
}
 ...
 * @param afterGathering - tool calls to make once gathering ends
 */
function gatheringClient(afterGathering: any[] = []) {
...
const carries = (request: any, text: any) => JSON.stringify(request.messages).includes(text);
...
describe('ai — the transcript as a slot', function () {
  it('keeps the request flat while the conversation grows', async function () {
    ...
    const sizes = client.requests.map((request: any) => JSON.stringify(request.messages).length);
    const early = sizes[3];
    ...
    const pointer = client.requests.at(-1).messages
      .find((message: any) => String(message.content).includes('slot "transcript"'));
  ...
  it('answers a question about an early round that a compacted run cannot', async function () {
    ...
    const grepStep = result.steps.find((step: any) => step.name === 'env_grep');
    ...
    const readStep = result.steps.find((step: any) => step.name === 'env_read');
  ...
  it('registers the environment tools without taking a host name', async function () {
    ...
    const names = client.requests.at(-1).tools.map((tool: any) => tool.function.name);
    assert.strictEqual(names.filter((name: any) => name === 'env_read').length, 1,
