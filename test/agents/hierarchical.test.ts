...
import { hierarchicalCorpus, hierarchicalScorecard, hierarchyProbe, scoreHierarchy } from '../../benchmark/lib/hierarchical.ts';
...
describe('hierarchical depth frontier', () => {
  it('has original deterministic leaf provenance, neutral expected totals and distractors', () => {
    ...
    assert.ok(corpus.sections.every((section: any) => section.leaves.length === 4));
  ...
  it('runs the same corpus at every depth, charges the full tree and reproduces byte for byte', async () => {
    ...
    assert.ok(a.rows.every((row: any) => row.correct && row.evidenceRecall === 1));
    const first = a.rows.slice(0, 4);
    assert.equal(new Set(first.map((row: any) => row.corpusHash)).size, 1);
    assert.deepEqual(first.map((row: any) => row.authorCalls + row.subcalls), [3, 8, 22, 51]);
    assert.deepEqual(first.map((row: any) => row.tokens), [150, 400, 1100, 2550]);
  });
