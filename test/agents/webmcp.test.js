...
function toolbox(names = ['add']) {
  const box = createToolbox();
  for (const name of names) box.add({ name, description: 'Add one', inputSchema: schema, execute: ({ value }) => ({ value: value + 1 }) });
  return box;
}
function host(method = 'registerTool', async = false) {
  const tools = [], removed = [];
  const context = { [method](definition) {
...
const bind = (box, realm, options = {}) => registerModelContext(box, undefined, undefined, { realm, ...options });
describe('toolbox WebMCP wrapper over the public browser contract', () => {
  ...
  for (const async of [false, true]) it(`preserves ${async ? 'rejected' : 'thrown'} registration failures and removes only owned tools`, async () => {
    const failure = Error('permission denied'), cleanup = Error('cleanup'), removed = [], tools = [], errors = [], n = host();
    ...
    }, unregisterTool(name) { removed.push(name); throw cleanup; }, provideContext() { assert.fail('no retry after mutation'); } };
  ...
  it('makes pending callbacks inert before asynchronous disposal finishes', async () => {
    let finish, tool; const removed = [], promise = new Promise(resolve => { finish = resolve; });
    const context = { registerTool(value) { tool = value; return promise; }, unregisterTool(name) { removed.push(name); } };
  ...
  it('supports headless and late mounts, root changes, abort and qualified legacy cleanup', async () => {
    const realm = {}; assert.equal((await bind(toolbox(), realm).ready).status, 'unavailable');
    ...
    let signal; const owned = registerModelContext(toolbox(), { registerTool(_, options) { signal = options.signal; } }, undefined, { lifecycle: 'abort' });
