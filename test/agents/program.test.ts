...
import { createProgramRunner, createProgramAuthor, compileProgram, programGate, PROGRAM_EXAMPLE } from '@tangleai/agents/program';
import { createStructuredOutput } from '@tangleai/models/structured';
import { PROGRAM_SCHEMA, PROGRAM_OPS, MAX_PROGRAM_CHARS } from '@tangleai/agents/schemas/program';
...
function lines(count: number = 12) {
  return Array.from({ length: count }, (_: any, i) =>
    `{"id":"R${String(i).padStart(4, '0')}","value":${100000 + i * 7}} ${'pad '.repeat(60)}`).join('\n');
}

/** An environment holding one corpus slot. */
async function env(text: string = lines()) {
...
const runner = async (options: Record<string, any> = {}) => createProgramRunner({
...
describe('ai — the program grammar cannot carry content (D2)', function () {
  ...
  it('names every operation the runner implements, and no others', function () {
    assert.deepStrictEqual(branches.map((b: any) => b.properties.op.const).sort(),
      [...PROGRAM_OPS].sort());
  });
  ...
  it('closes every step and bounds every string it accepts', function () {
    ...
    for (const branch of branches) {
      ...
      for (const [name, member] of Object.entries(branch.properties)) {
        ...
        if ((member as any).type === 'string') {
          assert.ok(typeof ((member as any).maxLength) === 'number',
  ...
  it('caps the whole document, so a query cannot become the payload', function () {
    ...
    assert.throws(() => compileProgram(fat, { compileQuery: compileJsonQuery }),
      (err) => (err as any).code === 'AI0205');
  });
});

describe('ai — the compile gate', function () {
  const compile = (steps: any, options: Record<string, any> = {}) =>
  ...
  it('rejects a name bound twice — AI0202', function () {
    ...
    ], { known: ['corpus'] }), (err) => (err as any).code === 'AI0202');
  });

  it('requires exactly one answer, last — AI0203', function () {
    assert.throws(() => compile([
      { op: 'chunk', from: 'corpus', as: 'pieces' },
    ], { known: ['corpus'] }), (err) => (err as any).code === 'AI0203');
    assert.throws(() => compile([
  ...
  it('is the same check the gate runs, in the shape structured output wants', function () {
    ...
    assert.strictEqual((bad as any).valid, false);
    assert.strictEqual((bad as any).errors[0].code, 'AI0201');
    assert.strictEqual((bad as any).errors[0].docPath, '/steps/0/from');
  });
});
...
describe('ai — the runner answers with metadata and stores content in slots', function () {
  ...
  it('puts every step\'s result in a slot, under its own name', async function () {
    ...
    const names = (await environment.ledger.listSlots()).map((s: any) => s.name);
    assert.ok(names.includes('program/values'), 'the reduce wrote no slot');
    assert.ok(names.filter((n: any) => n.startsWith('program/found/')).length === 12,
  ...
  it('runs twice with the same addresses and no duplicate storage', async function () {
    ...
    const after = (await environment.ledger.listSlots()).map((s: any) => s.name).sort();
    await run.run(program);
    assert.deepStrictEqual((await environment.ledger.listSlots()).map((s: any) => s.name).sort(), after,
...
it('keeps completed map accounting when a later query fails', async function () {
  ...
  assert.strictEqual(withMember(result.steps.find((step) => step.op === 'map')!, 'skipped').skipped, 10);
});
...
it('executes the actual recursive prompt example over mixed JSON leaf values', async function () {
  ...
  const author = createProgramAuthor({
    ...
    client: {
      endpoint: { provider: 'openrouter' }, complete: async ({ messages }: { messages?: any; }) => {
  ...
  assert.deepStrictEqual(JSON.parse(result.answer.text).map((item: any) => item.value), values);
...
describe('ai — authoring a program', function () {
  ...
  const author = async (client: any, environment?: any) => createProgramAuthor({
    client,
...
describe('ai — the question compaction cannot answer', function () {
  ...
  it('computes over what the map DID return when some sub-calls failed', function () {
    ...
    const record = (id: any, value: any) => ({ slot: id, value: { id, value } });
