...
import { slotAddressesIn, roundSlotName } from '@tangleai/context/recall';
...
describe('ai — the agent loop', function () {
  it('runs a tool round-trip and returns the full transcript', async function () {
    ...
    assert.deepStrictEqual(result.messages.map((m: any) => m.role),
  ...
  it('does not duplicate an existing system message, omits tools without a toolbox', async function () {
    ...
    assert.strictEqual(result.messages.filter((m: any) => m.role === 'system').length, 1);
  });
  ...
  it('streams the final reply through onDelta (passthrough to the client)', async function () {
    const client = {
      complete: async ({ onDelta }: { onDelta?: any; }) => {
        onDelta?.('par');
        onDelta?.('tial');
        return final('partial');
      },
    };
    const agent = createAgent({ client: (client as any) });
    const chunks: any[] = [];
  ...
  it('forwards onReasoning and exposes reasoning on the result, never the transcript', async function () {
    const client = {
      complete: async ({ onReasoning }: { onReasoning?: any; }) => {
        onReasoning?.('hmm');
        return {
...
describe('ai — history compaction', function () {
  ...
  function longHistory(rounds: any) {
    ...
    for (let i = 0; i < rounds; i++) {
      ...
      messages.push(({ role: 'tool', tool_call_id: `a${i}`, name: 'jaren_studio_patch', content: `{"ok":true,"revision":${i}}` } as any));
      messages.push(({ role: 'tool', tool_call_id: `b${i}`, name: 'jaren_studio_read', content: 'x'.repeat(400) } as any));
  ...
  it('under budget the history passes through by reference', async function () {
    ...
    const agent = createAgent({ client: (client as any), historyBudget: 1_000_000 });
  ...
  it('over budget: pairing survives, pins survive, budget is respected', async function () {
    ...
    const size = wire.reduce((n: any, m: any) => n + JSON.stringify(m).length, 0);
    ...
    const callIds = new Set(wire.flatMap((m: any) =>
      Array.isArray(m.tool_calls) ? m.tool_calls.map((c: any) => c.id) : []));
    for (const m of wire) {
      if (m.role === 'tool') assert.ok(callIds.has(m.tool_call_id), `orphan tool reply ${m.tool_call_id}`);
    }
    for (const id of callIds) {
      assert.ok(wire.some((m: any) => m.role === 'tool' && m.tool_call_id === id), `unanswered call ${id}`);
    }

    // the synopsis names the dropped tool activity
    const synopsis = wire.find((m: any) => typeof m.content === 'string' && m.content.startsWith('[Earlier context'));
    ...
    assert.ok(wire.some((m: any) => m.content === 'now add a sleep question'));
  ...
  it('a custom compaction hook receives exactly the dropped rounds', async function () {
    ...
    await agent.send(longHistory(12));
    const wire = wires[0];
    assert.ok(wire.some((m: any) => m.content === 'CUSTOM SUMMARY'));
    assert.ok(Array.isArray(seen as any[][] | null) && (seen as unknown as any[][]).length > 0);
    for (const unit of (seen as unknown as any[][])) {
      ...
      if (unit[0].role === 'assistant' && Array.isArray(unit[0].tool_calls)) {
        assert.ok(unit.slice(1).every((m: any) => m.role === 'tool'),
  ...
  it('with a ledger every dropped round is archived and its address travels', async function () {
    ...
    await agent.send(history);
    const wire = wires[0];
    const synopsis = wire.find((m: any) => typeof m.content === 'string'
    ...
    const first = JSON.parse((await ledger.readSlot(addresses[1]) as any));
  ...
  it('recall is registered, appears in steps, and answers { error } for an unknown slot', async function () {
    ...
    const ledger = createLedger();
    const client = {
      complete: async ({ messages, tools }: { messages?: any; tools?: any; }) => {
        seenTools.push(tools);
        const synopsis = messages.find((m: any) => typeof m.content === 'string'
          && m.content.startsWith('[Earlier context'));
    ...
    assert.ok(seenTools[0].some((t: any) => t.function.name === 'recall'),
      'a ledger registers recall — and only a ledger does');
    assert.deepStrictEqual(result.steps.map((s: any) => s.name), ['recall', 'recall'],
  ...
  it('no ledger means no recall tool, and a host that has its own keeps it', async function () {
    ...
    const seenTools: any[] = [];
    const client = {
      complete: async ({ tools }: { tools?: any; }) => {
        seenTools.push(tools ?? []);
        return final('ok');
      },
    };

    await createAgent({ client: (client as any), toolbox: mathToolbox() })
      .send([{ role: 'user', content: 'x' }]);
    assert.deepStrictEqual(seenTools[0].map((t: any) => t.function.name), ['add'],
      'without a ledger the tool list is exactly the host\'s');

    await createAgent({ client: (client as any), toolbox: bare, ledger: createLedger() })
      .send([{ role: 'user', content: 'x' }]);
    assert.deepStrictEqual(seenTools[1].map((t: any) => t.function.name), ['recall'],
  ...
  it('a custom compaction hook is given the addresses, and the header survives it', async function () {
    ...
    const agent = createAgent({
      ...
      compaction: (dropped, given) => {
        addresses = (given as any[]);
        return `HOST WROTE ${dropped.length}`;
      },
    });
    await agent.send(longHistory(12));
    const synopsis = wires[0].find((m: any) => typeof m.content === 'string'
      && m.content.startsWith('[Earlier context'));
    ...
    assert.ok(Array.isArray(addresses as any[] | null) && (addresses as unknown as any[]).length > 0);
    for (const address of (addresses as unknown as any[])) {
  ...
  it('a ledger that cannot store fails loudly instead of dropping the round', async function () {
    ...
    const agent = createAgent({
      client: (client as any), historyBudget: 4_000, ledger: refusing,
    });
    await assert.rejects(() => agent.send(longHistory(12)), (err: any) => {
      assert.strictEqual(err.code, 'AI0001');
      assert.match(err.message, /could not archive a round/);
      return true;
    });

    assert.throws(() => createAgent({ client: (client as any), ledger: {} }),
