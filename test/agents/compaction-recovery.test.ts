 ...
 *
 * The reachability walk is the benchmark's (`benchmark/lib/horizon.ts`),
...
const wireSize = (messages: any) => messages.reduce((n: any, m: any) => n + JSON.stringify(m).length, 0);
...
describe('ai — compaction moves instead of destroying (D4)', function () {
  it('refuses colliding archive addresses before losing a round or calling the model', async () => {
    for (const legacy of [false, true]) {
      ...
      ...Array.from({ length: 5 }, (_: any, i) => ({ role: 'user', content: `filler ${i}${'x'.repeat(300)}` }))];
      const before = structuredClone(history);
      await assert.rejects(createAgent({ client: (client as any), ledger: storage, historyBudget: 500 }).send(history), /collision/);
  ...
  it('a synopsis truncated to fit still carries the way back to everything it cut', async function () {
    ...
    for (const budget of [1600, 1200]) {
      ...
      const synopsis = result.messages.find((m: any) => typeof m.content === 'string'
  ...
  it('compacting the same history twice writes the same slots and does not duplicate them', async function () {
    ...
    assert.deepStrictEqual(
      first.messages.map((m: any) => m.content), second.messages.map((m: any) => m.content),
    ...
    const writes: string[] = [];
    const watched = {
      putSlot: (name: string, content: string, meta: any) => {
        writes.push(name);
        return ledger.putSlot(name, content, meta);
      },
      getSlot: (name: string) => ledger.getSlot(name),
      readSlot: (name: string) => ledger.readSlot(name),
    };
  ...
  it('a round the synopsis has nothing to say about still gets an address', async function () {
    ...
    const history = [{ role: 'user', content: 'go' }];
    for (let i = 0; i < 6; i++) {
      history.push(({ role: 'assistant', content: `turn ${i}`, tool_calls: [] } as any));
      history.push({ role: 'user', content: `filler ${i} ${'y'.repeat(200)}` });
    }
    const ledger = createLedger();
    await createAgent({ client: (client as any), historyBudget: 900, ledger })
      .send(history);
  ...
  it('the returned transcript stays full, and the ledger holds the parts the request lost', async function () {
    ...
    for (let i = 0; i < corpus.n; i++) {
      history.push(({ role: 'assistant', content: '', tool_calls: [{ id: `c${i}`, type: 'function', function: { name: 'fetch_record', arguments: JSON.stringify({ index: i }) } }] } as any));
      history.push(({ role: 'tool', tool_call_id: `c${i}`, name: 'fetch_record', content: JSON.stringify({ id: corpus.ids[i], notes: 'x'.repeat(300), value: corpus.values[i] }) } as any));
    }
    const agent = createAgent({
      client: (client as any), historyBudget: 1500, ledger,
    });
