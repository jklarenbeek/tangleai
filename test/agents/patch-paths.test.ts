...
import { refinementPatchSchema } from '@tangleai/context/schemas/patch';
...
it('an injected authoring validator cannot weaken the full patch schema', async () => {
  const { createLedger, createRefiner } = await Promise.all([import('@tangleai/context'), import('@tangleai/agents')]).then(parts => Object.assign({}, ...parts));
  const ledger = createLedger();
  let applied = false;
  const refiner = createRefiner(({
    ledger, validator: () => true,
    applyPatch: (document: any) => { applied = true; return document; }
  } as any));
