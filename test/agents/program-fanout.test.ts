...
async function env(count: number = 12) {
...
function measuringClient(options: {
  delayMs?: number;
  fail?: (index: number) => any;
} = {}) {
  const delayMs = options.delayMs ?? 5;
  const state = { calls: 0, peak: 0, inFlight: 0, signals: ([] as any[]) };
  return {
    state,
...
function hangingClient() {
  const state = { started: ([] as any[]) };
  return {
    state,
    complete: ({ signal }: { signal?: any; }) => new Promise((resolve, reject) => {
...
const runnerOver = async (client: any, options: Record<string, any> = {}) => createProgramRunner({
...
describe('ai — map is bounded', function () {
  ...
  it('completes twelve sequential timer waves in three waves at concurrency four', async function (t: import("node:test").TestContext) {
    const slow = 20;
    t.mock.timers.enable({ apis: ['setTimeout', 'Date'], now: 1_000 });
    async function measure(options: any) {
      const client = measuringClient({ delayMs: slow });
      const runner = await runnerOver(client, options);
      let completed = false;
      const running = runner.run(PROGRAM).then((result: any) => {
        completed = true;
        return result;
      });
  ...
  it('stops at maxSubcalls and says how many pieces it did not visit', async function () {
    ...
    const map = result.steps.find((s: any) => s.op === 'map');
    assert.ok('subcalls' in map!);
    assert.strictEqual(map.subcalls, 5);
    assert.ok('skipped' in map);
    assert.strictEqual(map.skipped, 7);
    // a cap that said nothing would read as a map over everything
    assert.ok('note' in map);
    assert.match((map.note as any), /maxSubcalls/);
  });
...
describe('ai — a failed sub-call is a result', function () {
  it('records the failure in its own slot and finishes the map', async function () {
    ...
    const slots = (await environment.ledger.listSlots()).map((s: any) => s.name);
