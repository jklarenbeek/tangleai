...
import { createProjectionIndex, createRankStorage, annDecision, annMetrics } from '../../benchmark/lib/relevance-ann.ts';
import { pressureFixture, runPressure, pressurePatch, representedUnits, pressureMetrics, guardedMerge, similarityBands } from '../../benchmark/lib/refinement-pressure.ts';
import { loadDataset } from '../../benchmark/lib/relevance.ts';
import { runRelevance, relevanceSubset } from '../../benchmark/recall-quality.ts';

const temp = (t: any) => { const path = mkdtempSync(join(tmpdir(), 'pressure-')); t.after(() => rmSync(path, { force: true, recursive: true })); return path; };
...
describe('optional ANN instrument', () => {
  ...
  it('exact adapter agrees with the four-method sweep after vector updates and deletion', async () => {
    ...
    for (const text of ['record a', 'record c']) {
      ...
      assert.ok('memories' in actual);
      assert.ok('memories' in expected);
      assert.deepEqual(actual.memories, expected.memories);
      assert.ok('scores' in actual);
      assert.ok('scores' in expected);
    ...
    assert.deepEqual(withMember((await ledger.recall({ near: 'updated record a' })), 'memories').memories,
      withMember((await sweep.recall({ near: 'updated record a' })), 'memories').memories);
  ...
  it('scores every size and contender without losing judgments or mislabelling a hash model', async () => {
    ...
    assert.ok(result.rows.every((row: any) => row.datasetClass === 'authored' && row.embeddingClass === 'hash'));
    assert.equal(result.rows.filter((row: any) => row.oracle).length, 2);
    assert.ok(result.rows.filter((row: any) => row.lifecycle).every((row: any) => row.lifecycle.deletedAbsent && row.lifecycle.updatedPresent));
    assert.match(result.annDecision, /retain exact/);
    const good = {
      oracle: { recall: { 10: 1 } }, recall: { 10: 1 }, mrr: 1, ndcg10: 1,
...
describe('repeated refinement quality', () => {
  ...
  it('distinguishes duplicates from equally similar independent and contradictory evidence', () => {
    ...
    const base = fixture.facts.find((f: any) => f.id === 'amber-base');
    const corroboration = fixture.facts.find((f: any) => f.id === 'amber-confirm');
    ...
    const record = ({ text, evidence, tags }: { text?: any; evidence?: any; tags?: any; }) => ({ text, evidence, tags });
    ...
    const conflict = fixture.facts.find((f: any) => f.id === 'amber-conflict');
    const unsafe = pressurePatch('guarded-model-merge', [{ ...record(base), id: 'stored' }], [record(conflict)],
      new Map(), (expected: any) => ({ ...expected, text: base.text }));
  ...
  it('replays through real refiner validation, persists/reopens, and rejects similarity-only loss', async (t) => {
    ...
    assert.deepEqual(result.results.find((r: any) => r.policy === 'runtime-exact-evidence').waves, exact.waves);
  });
  it('keeps injected generation unlabelled and names its model without inventing quality scores', async (t) => {
    ...
    const result = await runPressure({
      fixture, embedder: createHashEmbedder(), directory: temp(t), policies: ['none'],
      liveClient: {
        endpoint: { provider: 'scripted-wire', model: 'proposal-fixture' },
    ...
    assert.ok(result.results[0].waves.every((row: any) => row.labelled === false && row.recall === undefined));
  ...
  it('a lossless merge cites replaced ids, retains conflicts and rolls back on validation refusal', async () => {
    ...
    const facts = ['amber-base', 'amber-conflict'].map((id) => fixture.facts.find((f: any) => f.id === id));
    const proposal = ({ text, evidence, tags }: { text?: any; evidence?: any; tags?: any; }) => ({ text, evidence, tags });
    ...
    assert.ok(withMember(records[0].evidence, 'includes').includes(original.id));
