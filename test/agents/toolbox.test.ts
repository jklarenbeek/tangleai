...
describe('ai — the toolbox', function () {
  ...
  it('Jaren validates every input before the tool runs', function () {
    ...
    assert.ok(rejected.errors.every((e: any) => typeof e.message === 'string'
...
describe('ai — WebMCP registration', async function () {
  it('publishes through provideContext when available', async function () {

    let registered: any[] = [];
    const ok = registerModelContext(demoToolbox(), {
      provideContext: ({ tools }: { tools?: any; }) => { registered = tools; },
    });
    assert.strictEqual((await ok.ready).status, 'registered');
  ...
  it('falls back to per-tool registerTool', async function () {
    ...
    assert.strictEqual((await ok.ready).status, 'registered');
    assert.deepStrictEqual(registered.map((t: any) => t.name), ['add']);
  });

  it('reports unavailable for an absent or unusable context', async function () {
    assert.strictEqual((await registerModelContext(demoToolbox(), undefined).ready).status, 'unavailable');
    assert.strictEqual((await registerModelContext(demoToolbox(), {}).ready).status, 'unavailable');
  });

  it('reports failure and surfaces the error when registration throws', async function () {

    let seen: any = null;
    const ok = registerModelContext(demoToolbox(), {
      provideContext: () => { throw new Error('host refused'); },
    }, (err) => { seen = err; });
    assert.strictEqual((await ok.ready).status, 'failed');
