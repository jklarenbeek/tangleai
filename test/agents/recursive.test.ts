...
import { createLongHorizonAgent, resolveDepth, MAX_DEPTH, DEFAULT_DEPTH, childScope } from '@tangleai/agents/recursive';
import { createProgramAuthor, createProgramRunner, compileProgram } from '@tangleai/agents/program';
...
function corpusText() {
  return Array.from({ length: 6 }, (_: any, d) =>
    Array.from({ length: 4 }, (_: any, i) =>
...
function scripted(options: {
  program?: any;
  authorAt?: (depth: number) => any;
} = {}) {
  const state = { authorings: 0, leaves: 0, payloads: ([] as string[]) };
  return {
    state,
    endpoint: { provider: 'openrouter' },
    complete: async ({ messages, responseFormat }: { messages?: any; responseFormat?: any; }) => {
...
async function agentOver(client: any, extra: Record<string, any> = {}) {
...
describe('ai — depth is capped, and the cap is enforced', function () {
  ...
  it('runs no deeper than the cap however deep it is asked to go', async function () {
    ...
    const note = result.trajectory.find((e: any) => e.kind === 'note');
    assert.match(note.note, /99 was asked for/);
    const deepest = Math.max(...result.trajectory.map((e: any) => e.depth ?? 0));
...
describe('ai — a child is isolated', function () {
  it('sees a clean namespace and cannot name its way out of it', async function () {
    ...
    assert.deepStrictEqual((await a.digest()).slots.map((s: any) => s.name), ['mine']);
...
describe('ai — a child\'s failure is a value, not a silence', function () {
  it('surfaces a child that cannot author, with its depth and address', async function () {
    ...
    const client = {
      endpoint: { provider: 'openrouter' },
      complete: async ({ responseFormat }: { responseFormat?: any; }) => {
    ...
    const failures = result.trajectory.filter((e: any) => e.kind === 'subcall' && e.ok === false);
    ...
    assert.ok(parsed.some((p: any) => p.error !== undefined && p.depth === 1 && p.address !== undefined),
...
describe('ai — a program has to survive its own recursion', function () {
  ...
  it('refuses the formerly shallow-correct, deep-null reduce before execution', async function () {
    ...
    assert.throws(() => compileProgram(plan, {
      compileQuery: compileJsonQuery,
      analyzeQuery, annotateTypes, recursive: true
    }), (error) => error instanceof Error && 'code' in error && error.code === 'AI0208');
    for (const depth of [0, 1, 2]) {
...
describe('ai — D2 holds at every depth', function () {
  ...
  it('records a trajectory a host can persist and render', async function () {
    ...
    const kinds = new Set(result.trajectory.map((e: any) => e.kind));
    ...
    result.trajectory.forEach((entry: any, i: any) => assert.strictEqual(entry.seq, i));
  });
});
