...
import { createProgramRunner, compileProgram } from '@tangleai/agents/program';
import { compileJsonQuery, analyzeQuery, annotateTypes } from '@jarenjs/json/query';
import { questionStreamScorecard, simulateQuestionStream } from '../../benchmark/lib/question-stream.ts';
...
async function setup(extra: Record<string, any> = {}) {
  ...
  const options = {
    ...
    reuse: {
      environmentId: 'data-v1', schemaVersion: 'program-v1', embedder,
      check: ({ result }: { result?: any; }) => result.answer.text === '5', ...extra
    }
  };
...
describe('verified program reuse', () => {
  ...
  it('checks suitability before a trap and stores wrong-outcome evidence without editing a success', async () => {
    ...
    const result = await createProgramSession({
      ...state.options,
      runner: { run: async (...args: Parameters<typeof actual.run>) => { executions++; return actual.run(...args); } },
      reuse: { ...state.options.reuse, check: ({ reused }: any) => !reused }
    }).run('maximum');
    assert.equal(result.reuse.fallback, true);
  ...
  it('does not execute a similar question without host suitability proof', async () => {
    ...
    assert.equal(result.reuse.reused, false);
    assert.ok(result.reuse.events.some((event: any) => event.reason === 'unsuitable'));
  });
  it('rejects invalid stored program metadata at the ledger boundary', async () => {
    const { ledger } = await setup();
    const result = await ledger.addSkill(({ name: 'bad', when: 'bad', instructions: 'bad', tools: [], program: { version: 2 } } as any));
...
describe('question-stream economics', () => {
  it('is deterministic and charges wrong and rejected reuse through fallback', async () => {
    ...
    assert.ok(a.frontier.every((row: any) => row.accuracy === 1));
    assert.ok(a.frontier.some((row: any) => row.falseReuse > 0));
    for (const card of a.frontier) {
      for (const row of card.rows.filter((item: any) => item.falseReuse)) {
...
describe('recursive result contracts', () => {
  const plan = (query: any, outputSchema?: any) => ({
    steps: [
