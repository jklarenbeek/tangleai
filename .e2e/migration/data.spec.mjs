...
test.describe('a busy owner remains the owner', () => {
  ...
  for (const webLocks of [true, false]) {
    test(`a second tab attaches while the owner worker is blocked (Web Locks: ${webLocks})`, async ({ page: owner, context }) => {
      test.slow();
      if (!webLocks) await context.route('**/data-worker.js', async (route) => {
...
test.describe('the boot is terminal: a failed stage is named and a retry starts clean', () => {
  ...
  test('a worker module that never arrives fails the worker-start stage within its budget, and a retry recovers', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      sessionStorage.setItem('tangle-data-boot-budgets', JSON.stringify({ 'worker-start': 2000 }));
    });
    ...
    await page.unroute(WORKER);
    await page.evaluate(() => sessionStorage.removeItem('tangle-data-boot-budgets'));
    await failure.locator('.data-boot-retry').click();
    await expect(page.locator('.data-status .data-vfs')).not.toHaveText('—', READY);
    await expect(page.locator('.data-boot-error')).toHaveCount(0);
    expect(errors, 'no uncaught page errors').toEqual([]);
