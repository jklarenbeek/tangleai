...
const bootPage = async (page, noLocks = false, singleWriter = false) => {
  ...
  await page.goto('/');
  return page.evaluate(async ({ noLocks, singleWriter }) => {
    ...
    window.ledgerStorage = api.createSlotLedgerStorage(slot, { locks: noLocks ? null : navigator.locks, singleWriter });
...
test('two tabs retain ids, evidence and reported archives across eviction, quota failure and restart', async ({ page, context, browserName }, testInfo) => {
  const boot = tab => bootPage(tab, false, browserName === 'webkit');
...
test('a host without locks explicitly refuses atomic retention', async ({ page }) => {
  expect(await bootPage(page, true)).toBe('single-writer');
