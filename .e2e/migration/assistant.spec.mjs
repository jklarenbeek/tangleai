...
test('the transcript persists across a reload and clear wipes it', async ({ page }) => {
  await page.addInitScript(() => {
    localStorage.setItem('tangle-ai',
      JSON.stringify({ provider: 'ollama', baseUrl: '', model: 'qwen3:4b', apiKey: '' }));
    localStorage.setItem('tangle-ai-chat', JSON.stringify({
      messages: [
  ...
  const stored = await page.evaluate(() => localStorage.getItem('tangle-ai-chat'));
...
test('the objective survives a reload, in a real browser store', async ({ page }) => {
  ...
  const stored = await page.evaluate(() => localStorage.getItem('tangle-ai-ledger'));
