...
const decision = JSON.parse(await readFile(new URL('../benchmark/results/locomo-policy.json', import.meta.url), 'utf8'));
await page.waitForSelector(`[data-policy-cell="${decision.selection.decision.default}"]`);
if (!body.includes('100 requests') || !body.includes('500 requests')) await fail('inert policy lost a conflicting observation');
if (!(await page.locator('.report').first().innerText()).includes('6 live')) await fail('inert policy did not retain six records');
if (!(await page.locator('[data-policy-cell]').innerText()).includes('512 hash dimensions')) await fail('offline width differs from the measured default');
