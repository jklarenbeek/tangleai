...
export async function verifySite(url: string, root = ROOT) {
  ...
  const expected = { version: main.version, commit: git(root, 'rev-parse', 'HEAD'), packages: Object.fromEntries(names.map(name => [name, main.version])), dependencySources: await verifyPageSources(root) };
