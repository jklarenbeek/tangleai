 ...
 * @param registries @param text
 */
function mergeFacts(registries: Registry[], text: Map<string, string>) {
...
export function runDerivation(options: {
  ...
  const sources: Map<string, string> = new Map(docs.map((rel) => [rel, readFileSync(join(root, rel), 'utf8')]));
  ...
  for (const [rel, source] of sources) {
    ...
    const now = scanSourceDirectives((current.get(rel) as string), { ns: NS }).directives;
...
export function main({ root, registries, argv }: {
  ...
  if (report.unanswered.length > 0) {
    ...
    if (stale.length > 0) {
      ...
      console.error('run `npm run migration:docs` to rewrite it from the sources.');
    }
    else {
 ...
 * - what it derives from, for a collision message
 */
  name: string; /**
 * - repo-relative paths
 */
  docs: (root: string) => string[]; facts: (text: Map<string, string>) => Record<string, () => string>;
};
export type DeriveReport = { /**
 * - 0 green, 1 failed
 */
  code: number; /**
 * - markers no registry could answer
 */
  unanswered: string[]; /**
 * - baked text the sources no longer agree with
 */
  drift: string[]; /**
 * - derivations no document carries
 */
  unused: string[]; /**
 * - the derivations that resolved
 */
  seen: string[]; /**
 * - bake passes needed to settle
 */
  passes: number; /**
 * - documents written (write mode only)
 */
  rewritten: string[]; /**
 * - documents walked
 */
