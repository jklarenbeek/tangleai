...
const read = (name: any) => JSON.parse(readFileSync(new URL(`../../benchmark/recall-quality-${name}.json`, import.meta.url), 'utf8'));
const fmt = (value: any) => value === null || value === undefined ? '—' : value.toFixed(3);
...
export const recallFacts = {
  ...
  facts: () => ({
    'recall.quality': () => table('| documents | policy | recall@10 | MRR@10 | nDCG@10 | p95 ms |',
      read('scifact-live').rows.map((r: any) => `| ${r.size} | ${r.policy} | ${fmt(r.recall[10])} | ${fmt(r.mrr)} | ${fmt(r.ndcg10)} | ${fmt(r.latencyMs.p95)} |`)),
    'recall.reference': () => {
      const data = read('scifact-live'), row = data.rows.find((r: any) => r.size === data.dataset.documents && r.policy === 'exact');
    ...
    'recall.ann': () => table('| documents | candidates | exact-top-10 recall | index bytes | build ms | update p95 ms | delete ms | clears all bars |',
      read('scifact-live').rows.filter((r: any) => r.oracle).map((r: any) => `| ${r.size} | ${r.policy} | ${fmt(r.oracle.recall[10])} | ${r.storage.indexBytes} | ${fmt(r.buildMs)} | ${fmt(r.lifecycle.updateP95Ms)} | ${fmt(r.lifecycle.deleteMs)} | ${r.decision.pass ? 'yes' : 'no'} |`)),
    'recall.annDecision': () => {
      const data = read('scifact-live'), rows = data.rows.filter((r: any) => r.oracle);
      return `${rows.filter((r: any) => r.decision.pass).length}/${rows.length} contender rows cleared all bars; retain exact. Required exact-top-10 recall ≥ ${data.bars.exactRecall10}, p95 speedup ≥ ${data.bars.p95Speedup}×, and a measured exact p95 ≥ ${data.bars.minimumExactP95Ms} ms. The largest reference corpus contains ${data.dataset.documents} documents; scale beyond it remains unmeasured.`;
    },
    ...
    'recall.dedup': () => {
      const data = read('pressure-live'), baseline = data.results.find((r: any) => r.policy === 'none').waves.at(-1), runtime = data.results.find((r: any) => r.policy === 'runtime-exact-evidence').waves.at(-1);
    ...
    'recall.cost': () => {
      ...
      const sum = (key: any) => rows.reduce((n: any, row: any) => n + row[key], 0);
