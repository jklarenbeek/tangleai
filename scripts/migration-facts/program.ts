...
const read = (name: any) => JSON.parse(readFileSync(new URL(`../../benchmark/programmind-${name}.json`, import.meta.url), 'utf8'));
const table = (head: any, rows: any) => '\n\n' + [head, head.replace(/[^|]/g, '-'), ...rows].join('\n') + '\n\n';
...
export const programFacts = {
  ...
  facts: () => ({
    ...
    'program.depth': () => table('| depth | correct fixture tasks | author calls | subcalls | token proxy |',
      read('depth-fixture').rows.filter((row: any) => row.seed === 17).map((row: any) =>
    ...
    'program.profiles': () => table('| grammar | full closure bytes | profile bytes | full branches | profile branches |',
      read('profiles').rows.map((row: any) => `| ${row.grammar} | ${row.full.bytes} | ${row.profile.bytes} | ${row.full.branches} | ${row.profile.branches} |`)),
    'program.live': () => {
      ...
      return table('| instrument | profile / model | attempts | correct | timeout | other failures |', runs.flatMap((name) => {
        ...
        return [...new Set(rows.map((row: any) => row.profile))].map((profile: any) => {
          const group = rows.filter((row: any) => row.profile === profile);
          const correct = group.filter((row: any) => row.outcome === 'correct').length;
          const timeout = group.filter((row: any) => row.outcome === 'timeout').length;
    ...
    'program.depthLive': () => {
      ...
      return `${rows.filter((row: any) => row.correct).length}/${rows.length} live depth tasks correct; ${rows.reduce((sum: any, row: any) => sum + row.timeouts, 0)} timed-out calls and ${rows.flatMap((row: any) => row.routes).filter((row: any) => row.outcome === 'token-limit').length} provider token-ceiling violations. No deeper default is justified.`;
    },
    'program.reuseLive': () => table('| profile | mode | correct | author calls | reported tokens | reused answers |',
      ['primary', 'secondary'].flatMap((profile) => ['fresh', 'reuse'].map((mode) => {
        const rows = read('reuse-live').rows.filter((row: any) => row.profile === profile && row.mode === mode);
        return `| ${profile} | ${mode} | ${rows.filter((row: any) => row.correct).length}/${rows.length} | ${rows.reduce((sum: any, row: any) => sum + row.authorCalls, 0)} | ${rows.reduce((sum: any, row: any) => sum + row.tokens, 0)} | ${rows.filter((row: any) => row.reused).length} |`;
      }))),
  }),
};
