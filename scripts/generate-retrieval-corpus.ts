 ...
 * `benchmark/retrieval.ts` to score retrieval policies against.
 *
 ...
 *
 *   node scripts/generate-retrieval-corpus.ts           # check, exit 1 on drift
 *   node scripts/generate-retrieval-corpus.ts --write   # rewrite the fixture
 *   node scripts/generate-retrieval-corpus.ts --write --out PATH
 *
 * The shape:
...
export function tokens(text: any) {
  return text.toLowerCase().split(/[^a-z0-9]+/).filter((t: any) => t.length > 0);
}

/** A statement, capitalized and terminated. */
const sentence = (subject: any, predicate: any, object: any) =>
  `${subject.charAt(0).toUpperCase()}${subject.slice(1)} ${predicate} ${object}.`;

/** One uniform pick from a list. */
const pick = (random: any, list: any) => list[Math.floor(random() * list.length)];

/** Zero to two secondary tags, distinct. */
function secondaryTags(random: any) {
 ...
 * @param [seed]
 */
export function generateRetrievalCorpus(seed: number = CORPUS_SEED) {
  ...
  function sameTagDistractor() {
    for (; ;) {
      ...
      statements.add(text);
      return {
        text, evidence: evidenceFor(random, topic.tag),
  ...
  TOPICS.forEach((topic, index) => {
    ...
    for (const asked of groups.reverse()) {
      const subjects = asked.map((i: any) => facts[i].subject);
      ...
      questions.push({
        ...
        text,
        topic: topic.tag,
        facts: asked.map((i: any) => `f-${String(i + 1).padStart(3, '0')}`),
        gold: asked.map((i: any) => goldIds[i]),
      });
    }
  });
 ...
 */
export function serializeCorpus(corpus: ReturnType<typeof generateRetrievalCorpus>) {
  const lines = (list: any) => list.map((item: any) => `  ${JSON.stringify(item)}`).join(',\n');
  return '{\n'
    + ` "seed": ${corpus.seed},\n`
...
if (process.argv[1] !== undefined && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (argv.includes('--write')) {
    ...
    if (current === text) {
      ...
      console.error('Run `node scripts/generate-retrieval-corpus.ts --write` and read the diff before keeping it.');
      process.exitCode = 1;
    }
  }
}
