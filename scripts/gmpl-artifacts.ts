...
import {gmplPromptFiles,gmplPromptStage} from './gmpl-sources.ts';
...
const files=await gmplPromptFiles();
if(files.length!==14)throw Error(`expected fourteen in-scope packs, found ${files.length}`);
...
for(const file of files){
  const stage=gmplPromptStage(file);
