...
import { sha256, parseJsonl, loadDataset } from '../benchmark/lib/relevance.ts';

/** Import only the known archive entries, without extracting arbitrary ZIP paths.
 * @param directory @param [config] */
export async function importScifact(directory: string, config: any = {}) {
  ...
  const read = (name: any) => execFileSync('unzip', ['-p', zip, `scifact/${name}`], { maxBuffer: 20 * 1024 * 1024 }).toString('utf8');
  ...
  const queries = parseJsonl(read('queries.jsonl'), 'queries').filter((q: any) => ids.has(q._id));
  const files: Record<string, any> = {};
  for (const [name, rows] of Object.entries({ corpus, queries, qrels })) {
    const data = rows.map((row: any) => JSON.stringify(row)).join('\n') + '\n';
    const path = `${name}.jsonl`;
    writeFileSync(join(directory, path), data);
    files[name] = { path, sha256: sha256(data) };
  }
  const manifest = {
    schemaVersion: 1, id: 'beir-scifact-test', datasetClass: 'real-language',
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (!directory || extra.length) throw new Error('Usage: node scripts/import-scifact.ts DIRECTORY');
