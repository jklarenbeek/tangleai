import { PROVIDER_OPTIONS, isConfigured } from './settings.ts';
  ...
  return {
    ...
    goal: ai.goal === null ? null : {
      ...
      progress: [...ai.goal.progress].reverse().slice(0, PROGRESS_SHOWN)
        .map((entry: any) => ({ note: entry.note, evidence: entry.evidence })),
