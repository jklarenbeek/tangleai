 ...
 * @param doc - a jaren-app document
 * @param [name]
 */
export function singleAppProject(doc: any, name: string = 'Studio app') {
  return {
    project: '0.1',
    name,
 ...
 * @type
 */
export const PROJECT_TEMPLATES: ReadonlyArray<{
  id: string;
  title: string;
  lead: string;
  active: string;
  ...
  ...['fsm', 'dag'].map((kind) => ({
    id: kind, title: kind === 'fsm' ? 'State machine' : 'Dataflow',
...
export function projectTemplate(id: any) {
 ...
 * @param snapshot - a decoded share token
 */
export function sharedProject(snapshot: any) {
  ...
  const files = p.files
    ...
    .map((f: any) => ({ ...f, name: String(f.name ?? ''), kind: String(f.kind ?? 'data'), text: String(f.text ?? '') }))
    .filter((f: any) => f.name !== '');
  ...
  return {
    ...
    files,
    active: files.some((f: any) => f.name === p.active) ? p.active : files[0].name,
