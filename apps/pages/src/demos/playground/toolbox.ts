...
import { playComponent, legacyExperimentToSession } from './play.ts';
import { operatorRegistry, runValidation, projectHost, documentHost } from './services.ts';
import { STUDIO_TEMPLATES, studioTemplate } from './appTemplates.ts';
import { FLOW_TEMPLATES, flowTemplate } from './flowTemplates.ts';
export { SYSTEM_PROMPT } from './prompt.ts';
const parse = (text: any) => { try { return JSON.parse(text); } catch { return text; } };
export const renderedText = (nodes: any) => (nodes ?? []).map((n: any) => {
  ...
  if (n.kind === 'cards') return n.items.map((i: any) => `${i.title}: ${i.value}`).join(' · ');
  if (n.kind === 'p' || n.kind === 'callout') return n.text;
  return null;
}).filter((line: any) => line !== null).join('\n');
/** @param env */
export function createPageToolbox(env: any) {
  const toolbox = createToolbox(), go = (hash: any) => env.navigate(hash);
  const projectSummary = () => {
    const { document, revision, stageRevision } = env.project().read(), described = projectHost.projectComponent.describe(document);
    return {
      name: document.name ?? 'Untitled project', active: described.active, revision: stageRevision, documentRevision: revision,
      files: described.files.map(({ name, kind, role, size, valid, errors }: { name?: any; kind?: any; role?: any; size?: any; valid?: any; errors?: any; }) => ({ name, kind, role, size, valid, errors }))
    };
  };
  const client = () => {
  ...
  const studio = (editor: any) => createStudioAdapter({ editor, operators: operatorRegistry });
  const appFile = () => {
  ...
  function studioReceipt(result: any, doc: any) {
    const audit = documentHost.auditDocumentRender(doc), snapshot = env.project().read();
    return {
      ...result, revision: snapshot.stageRevision, documentRevision: snapshot.revision, widgets: audit.widgets,
  ...
  async function studioSwap(doc: any) {
    const checked = documentHost.validateAppDocument(doc);
    if (!checked.valid) return {
      ok: false as const, errors: checked.errors, total: checked.total,
  ...
  async function runProject(input: any) {
    const editor = env.project(), before = editor.read(), name = input.name ?? before.document.active;
    const file = before.document.files.find((f: any) => f.name === name);
    if (!file) return { error: `no file named '${name}'`, names: before.document.files.map((f: any) => f.name) };
  ...
  const handlers = {
    ...
    jaren_list_engines() {
      return Object.fromEntries(Object.entries(playComponent.engines).map(([id, e]) => [id, {
    ...
    jaren_navigate(input: any) {
      const page = input.page === 'studio' ? 'project' : input.page;
      if (!['home', 'project', 'play', 'flow', 'data', 'game'].includes(page)) return { ok: false as const, error: `The optional '${page}' page is not hosted here.`, pages: ['home', 'project', 'play', 'flow', 'data', 'game'] };
      go(`#/${page === 'home' ? '' : page}?${new URLSearchParams(input.params ?? {})}`); return { ok: true as const };
    },
    jaren_get_examples(input: any) {
    ...
    jaren_project_files(input: any) {
      const summary = projectSummary(); if (input.name === undefined) return summary;
      const file = env.project().read().document.files.find((f: any) => f.name === input.name);
      return file ? { ...summary.files.find((f: any) => f.name === input.name), text: file.text, revision: summary.revision } : { error: `no file named '${input.name}'`, names: summary.files.map((f: any) => f.name) };
    },
    async jaren_project_author(input: any) {
      ...
      if (signal?.aborted) return { ok: false as const, error: 'Authoring was cancelled.', candidate: proposal.candidate };
    ...
    async jaren_project_write(input: any) {
      const editor = env.project(), before = editor.read();
      if (!input.kind && !before.document.files.some((file: any) => file.name === input.name))
    ...
    jaren_studio_write: (input: any) => studioSwap(input.doc),
    async jaren_studio_patch(input: any) {
    ...
    jaren_studio_read(input: any) {
      ...
      catch (error: any) { return { error: `invalid JSON Pointer: ${error.message}` }; }
    },
    jaren_get_templates(input: any) {
      if (input.name === undefined) return STUDIO_TEMPLATES.map(({ name, title, lead }) => ({ name, title, lead }));
    ...
    jaren_flow_write: (input: any) => env.writeFlow(input.kind, input.doc),
    async jaren_flow_patch(input: any) {
      const editor = env.flow(), before = editor.read();
    ...
    jaren_flow_check(input: any) { const result = validateFlowDocument(input.kind, input.doc); return { ...result, ok: result.valid }; },
    jaren_flow_get_templates(input: any) {
      if (input.name === undefined) return FLOW_TEMPLATES.map(({ name, kind, title, lead }) => ({ name, kind, title, lead }));
    ...
    jaren_save_experiment: (input: any) => env.save(input.name),
    jaren_list_experiments: () => env.saved(),
    jaren_load_experiment: (input: any) => env.load(input.name),
  ...
  for (const definition of TOOL_DEFINITIONS) {
    ...
    toolbox.add({
      ...definition, inputSchema: schema, execute: (input: any) => {
        if (env.signal?.aborted) return { ok: false as const, error: 'This host is disposed.' };
        try { return handlers[definition.name as keyof typeof handlers](input); } catch (error: any) { return { ok: false as const, error: error.message, code: error.code }; }
      }
    });
  }
  for (const kind of ['flow', 'data']) toolbox.add({
    name: `jaren_${kind}_author`, description: `Propose and validate a ${kind} document, then apply it only if the editor revision is unchanged. Data publication changes buffers; opening or recreating a store is a separate explicit operation.`,
    ...
    execute: async (input: any) => {
      const provider = client(); if (!provider) return { error: 'Configure a provider and model in assistant settings first.' };
      go(`#/${kind}`);
      const editor = env[kind](), signal = env.requestSignal?.() ?? env.signal;
      if (kind === 'data') {
        const adapter = createDataAdapter({ editor, client: provider });
        const proposal = await adapter.propose({ member: input.kind, prompt: input.prompt }, { signal });
        if (signal?.aborted) return { ok: false as const, error: 'Authoring was cancelled.', candidate: proposal.candidate };
        return proposal.ok ? adapter.accept(proposal) : proposal;
      }
      const adapter = createFlowAdapter({ editor, client: provider });
      const proposal = await adapter.propose(input, { signal });
      if (signal?.aborted) return { ok: false as const, error: 'Authoring was cancelled.', candidate: proposal.candidate };
      return proposal.ok ? adapter.accept(proposal) : proposal;
    },
  ...
  toolbox.add({
    name: 'jaren_data_run', description: 'Run the current Data query and return real results and its query plan. Opening an edited model requires operation "open".',
    inputSchema: { type: 'object', properties: { operation: { enum: ['query', 'open'] } } },
    execute: async (input: any) => { go('#/data'); const editor = env.data(); await editor.ready; return editor.run({ operation: input.operation ?? 'query' }); },
  });
  return toolbox;
}
