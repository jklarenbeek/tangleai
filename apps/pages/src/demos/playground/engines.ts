...
import { cards, code, error, callout } from '@jarenjs/studio/component';
import { now, formatJson, formatMsUnscaled } from './format.ts';
...
function parseJson(text: any, label: any) {
  try {
    return { value: JSON.parse(text) };
  }
  catch (err: any) {
    return { node: error({ message: `${label}: ${(err as Error).message}` }, 'Invalid JSON') };
  }
}

function timed(fn: any) {
  const start = now();
  const value = fn();
...
export function runQuery(inputs: {
  ...
  try {
    ...
    let externals: Record<string, any> = {};
    if (inputs.externals !== '') {
    ...
    const unbound = fn.externals.filter((name: any) => !(name in externals));
    if (unbound.length > 0) {
      return [callout('Unbound externals',
        `This query needs externals not provided: ${unbound.join(', ')}. Add them to the externals input as JSON.`, undefined, undefined)];
    }
    ...
    return [
      ...
      code('Result', value === undefined ? '(empty sequence)' : formatJson(value), undefined),
    ];
  }
  catch (err: any) {
    return [error((err as any), 'Query error')];
  }
}
...
export function runJslt(inputs: {
  ...
  try {
    ...
    return [error((err as any), 'JSLT error')];
  }
}
