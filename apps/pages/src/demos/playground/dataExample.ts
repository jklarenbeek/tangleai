...
import { OUR_SCHEMA_OPTIONS } from '@jarenjs/studio/component';
 ...
 * is probed with.
 */
export const tripChain = (source: any) => from(source)
  .params({ region: TRIP_REGION })
  .where((f, q) => f.get('geometry').within(q.region));
 ...
 */
export function tripPipeline(csvText: string): {
  rows: any[];
  collection: any;
  valid: boolean;
  errors: any[];
  ...
  const outcome = (validateGeoJson(collection) as any);
  const valid = typeof outcome === 'object' && outcome !== null ? outcome.valid === true : outcome === true;
  const errors = valid ? [] : (outcome?.errors ?? []).slice(0, 8).map((e: any) =>
 ...
 * @type
 */
const PREVIEW_RENDERERS: Record<string, (features: any, title: string) => any> = {
  map: (features: any, title) => compileChart(
 ...
 * @param features - a `FeatureCollection`, `Feature` or geometry
 */
export function previewVnode(preview: {
  kind: string;
} | null | undefined, features: any, title: string) {
 ...
 */
export function tripMap(report: any) {
 ...
 */
export function tripSummary(report: any) {
...
export const DATA_EXAMPLE = {
  ...
  createRow: (title: any) => ({ id: `n${Math.random().toString(36).slice(2, 8)}`, title, points: Math.floor(Math.random() * 50) }),
  migration: (model: any, collection: any) => {
    const to = structuredClone(model), indexes = to.collections[collection].indexes ??= [];
    if (!indexes.some((index: any) => index.name === 'by_title')) indexes.push({ name: 'by_title', path: '$.title' });
