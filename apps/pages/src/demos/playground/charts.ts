...
function parseWithEvents(format: any, text: any, events: any) {
  const sink = (e: any) => events.push(e);
...
function dataFor(config: any, events: any) {
...
function normalFormat(value: any) {
 ...
 * @param [config]
 * @returns the chart's SVG vnode
 */
export function chartRenderer(source: string, config?: {
  format?: string;
}): any {
