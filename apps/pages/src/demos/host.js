...
import { mountPlay } from './playground/mount-play.js';
import { createPageToolbox, SYSTEM_PROMPT } from './playground/toolbox.js';
import { projectHost, operatorRegistry } from './playground/services.js';
import { projectTemplate, PROJECT_TEMPLATE_CARDS, sharedProject, singleAppProject } from './playground/projectTemplates.js';
import { flowTemplate, FLOW_TEMPLATES } from './playground/flowTemplates.js';
import { DATA_EXAMPLE } from './playground/dataExample.js';
import { sessionOf, deepLinkExample, legacyExperimentToSession } from './playground/play.js';
const volatile = () => { let data = null; return { read: () => structuredClone(data), write: value => { data = structuredClone(value); }, reliable: false }; };
const labels = { project: 'Studio', play: 'Playground', flow: 'Flow', data: 'Data', game: 'Adventure' };
/** @param {{ root: HTMLElement, assistantNode: HTMLElement, landingNode?: HTMLElement, [key: string]: any }} options */
export function mountPageHost(options) {
  let appliedToken = null;
  let page = 'home', disposed = false, closing, assistant, binding, bindingEpoch = 0;
  const lifetime = new AbortController(), document = options.root.ownerDocument, panes = {}, editors = {}, snapshots = {};
  ...
  const removals = [];
  const listen = (node, type, fn) => { node.addEventListener(type, fn); removals.push(() => node.removeEventListener(type, fn)); };
  const element = (tag, parent, text) => { const node = document.createElement(tag); if (text) node.textContent = text; parent.appendChild(node); return node; };
  ...
  const ensure = key => {
    ...
    if (key === 'project') editors[key] = mountStudioEditor(host, { ...common, host: projectHost,
    ...
    if (key === 'flow') {
      ...
      editors[key] = mountFlowEditor(host, { ...common, kind: saved?.kind ?? null, document: saved?.document ?? null,
        input: saved?.input, templates: FLOW_TEMPLATES, template: flowTemplate,
        tasks: { lookup: async ({ input }, signal) => {
          await new Promise((resolve, reject) => { const abort = () => { clearTimeout(timer); reject(new Error('aborted')); };
            const timer = setTimeout(() => { signal.removeEventListener('abort', abort); resolve(); }, 350);
            if (signal.aborted) abort(); else signal.addEventListener('abort', abort, { once: true }); });
          return (Array.isArray(input) ? input : []).map(row => ({ ...row, region: 'eu' }));
        } },
      });
    }
    if (key === 'data') {
      editors[key] = mountDataEditor(host, { ...DATA_EXAMPLE, ...common, operators: operatorRegistry,
        transport: options.transport, lifecycleTarget: options.lifecycleTarget ?? {},
      });
    }
    if (key === 'game') editors[key] = mountGame(host, { ...common, state: snapshots.game,
  ...
  function setRoute(hash) {
    ...
    if (token && token !== appliedToken) {
      ...
      if (page === 'project' && value) { const candidate = sharedProject(value); if (candidate) { const editor = ensure('project'); void editor.replace(candidate, { expectedRevision: editor.read().revision }).then(result => { if (result.ok && !disposed) void editor.run(); }); } }
    }
  ...
  function navigate(hash) { setRoute(hash); options.navigate?.(hash); }
  async function writeFlow(kind, doc) {
    const result = validateFlowDocument(kind, doc); if (!result.valid) return { ...result, ok: false };
    ...
    navigate('#/flow'); return { ok: true, ...ensure('flow').read() };
  }
  function save(name) {
    if (typeof name !== 'string' || !name.trim()) return { ok: false, error: 'Give the work a name first.' };
    if (page === 'project') { docStore.save(name, { engine: 'project', project: { ...ensure('project').read().document, name }, savedAt: new Date().toISOString() }); updateSaved(); return { ok: true, names: docStore.names() }; }
    playStore.save(name, { ...sessionOf(ensure('play').read()), savedAt: new Date().toISOString() });
    ensure('play').dispatch('play/saved', { name, names: playStore.names() }); return { ok: true, names: playStore.names() };
  }
  async function load(name) {
    ...
    if (record) {
      ...
      if (!candidate) {
        ...
        if (!session) return { ok: false, error: 'This record has no supported project or engine.' };
        navigate('#/play'); ensure('play').replace(session, name); return { ok: true };
      }
  ...
  async function share() {
    ...
    const token = encodeShare(value); if (token.length > 8000) return { ok: false, error: 'Too large for a share link; download the document instead.' };
    const hash = `#/${page === 'project' ? 'project' : 'play'}?s=${token}`;
    if (!options.share) return { ok: false, error: 'Clipboard sharing is unavailable here.', hash };
    const url = await options.share(hash); return url ? { ok: true, url, note: 'A share link was copied to the clipboard.' } : { ok: false, error: 'The share link could not be copied.', hash };
  }
  const toolbox = createPageToolbox({ project: () => ensure('project'), play: () => ensure('play'), flow: () => ensure('flow'), data: () => ensure('data'),
  ...
  assistant = mountAssistant(options.assistantNode, { ...common, title: 'Tangle assistant', launchTitle: 'Open the Tangle assistant',
  ...
  function updateSaved() { while (savedSelect.firstChild) savedSelect.removeChild(savedSelect.firstChild);
  ...
  updateSaved();
  const button = (label, run) => { const btn = element('button', toolbar, label); btn.type = 'button'; listen(btn, 'click', () => { void Promise.resolve().then(run).then(r => { status.textContent = r?.error ?? (r?.ok ? label + ' complete' : 'Unavailable'); }, e => { status.textContent = e.message; }); }); };
  ...
  button('Delete saved', () => { const name = savedSelect.value; if (!name) return { error: 'Choose a saved project.' }; docStore.remove(name); updateSaved(); return { ok: true }; });
  listen(savedSelect, 'change', () => { if (savedSelect.value) void load(savedSelect.value).then(r => { status.textContent = r.ok ? 'Project opened' : r.error ?? 'Project refused'; }); });
  setRoute(options.hash ?? '#/');
  return { toolbox, assistant, ledger, ledgerStorage, slots, navigate, setRoute, page: () => page,
    ...
    dispose() { if (disposed) return closing; disposed = true; bindingEpoch++; lifetime.abort();
