...
import { mountPageHost } from './host.js';
import { mountTransferControls } from './transfer-controls.ts';
export function bootDemos(window) {
  const document = window.document;
  const jsonSlot = key => ({ key, reliable: true,
    read: () => JSON.parse(window.localStorage.getItem(key) ?? 'null'),
    write: value => window.localStorage.setItem(key, JSON.stringify(value)),
  });
  const slots = { settings: jsonSlot('tangle-ai'), transcript: jsonSlot('tangle-ai-chat'), ledger: jsonSlot('tangle-ai-ledger'),
    projects: jsonSlot('tangle-projects'), play: jsonSlot('tangle-play') };
  const gameSave = { read: () => window.localStorage.getItem('tangle-game'), write: value => window.localStorage.setItem('tangle-game', value) };
  ...
  const openFile = () => new Promise(resolve => {
    ...
    const finish = value => { if (complete) return; complete = true; pickers.delete(cancel); input.remove(); resolve(value); };
    pickers.add(cancel);
    input.addEventListener('cancel', () => finish(null), { once: true });
    input.addEventListener('change', () => { const file = input.files?.[0]; if (!file) finish(null);
      else void file.text().then(text => finish({ name: file.name, text }), () => finish(null)); }, { once: true });
  ...
  const host = mountPageHost({ root: document.getElementById('demos'), assistantNode: document.getElementById('assistant'), landingNode: document.getElementById('app'),
    slots, gameSave, locks: window.navigator.locks, singleWriter: true, realm: window, lifecycleTarget: window,
    hash: window.location.hash || '#/', navigate: hash => { if (window.location.hash !== hash) window.location.hash = hash; },
    share: async hash => { const url = new URL(window.location.href); url.hash = hash; await window.navigator.clipboard.writeText(url.href); return url.href; },
    download, openFile, aiFetch: window.fetch.bind(window), onError: error => window.console.error('[tangle-demos]', error),
  ...
  let disposed = false;
  function stopHost() { if (disposed) return; disposed = true;
