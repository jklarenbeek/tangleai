...
import { NAME_FORM_ACTIONS } from './runtime.ts';
/**
...
const titleScreen =
  ['div', { class: 'game-title' },
    ['div', { class: 'game-title-card' },
      ...
      ['p', { class: 'game-hint' },
        'Plays fully offline. Configure a provider in the assistant settings and the whole crew answers you ',
...
const hotGroup = (title: any, path: any) => { return { $if: [{ $exists: path }, ['div', { class: 'hs-group' }, ['h4', {}, title], ['div', { class: 'hs' }, [{ $apply: path }]]], ''] }; };
...
export const GAME_RULES = [
  ...
  // exits (navigation through the flow engine)
  {
    ...
    body: ['button', {
      type: 'button', class: 'hs-btn exit',
      on: { click: { action: 'game/go', with: '$.id' } }
    }, '→ ', '$.name'],
  },
  // items / scenery / people — all resolve the armed verb
  {
    ...
    body: ['button', {
      type: 'button', class: 'hs-btn item',
      on: { click: { action: 'game/hotspot', with: '$.id' } }
    }, '$.name'],
  },
  {
    match: '$.ui.game.room.scenery[*]', mode: 'game',
    body: ['button', {
      type: 'button', class: 'hs-btn scenery',
      on: { click: { action: 'game/hotspot', with: '$.id' } }
    }, '$.name'],
  },
  {
  ...
  // dialogue options
  {
    ...
    body: ['button', {
      type: 'button', class: 'gd-option',
      on: { click: { action: 'game/say-pick', with: '$.index' } }
    }, '$.text'],
  },
  // learned comebacks in the insult duel
  {
    ...
    body: ['button', {
      type: 'button', class: 'gd-option duel-comeback',
      on: { click: { action: 'game/duel-say', with: '$.index' } }
    }, '“', '$.text', '”'],
  },
  // the validation-language picker on the title card
  {
    ...
    body: ['button', {
      type: 'button', class: { $if: ['$.active', 'game-loc on', 'game-loc'] },
      on: { click: { action: 'game/locale', with: '$.id' } }
    }, '$.label'],
  },
  ...
  ...createFormView({ root: '$.ui.game.nameForm', actions: NAME_FORM_ACTIONS }).map((rule: any) => ({ ...rule, mode: 'game' })),
];
