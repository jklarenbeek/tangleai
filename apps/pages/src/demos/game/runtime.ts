 ...
 * @tangleai/models, in the exact voice the content declares.
 */
import { createChatClient, createStructuredOutput } from '@tangleai/models';
...
import { sceneApp } from './gameFsms.ts';
import {
  TITLE, GOAL, VERBS,
...
function roomThings(room: any, inv: any) {
...
// ------------------------------------------------------------------
export function gamePageViewModel(game: any) {
  ...
  return {
    ...
    nameForm: buildFormViewModel(NAME_MODEL, game.nameForm, { validateFields: true, catalog: CATALOGS[game.locale as keyof typeof CATALOGS] }),
    ...
    room: {
      ...
      exits: room.exits.map((id: any) => ({ id, name: LOCATIONS[id].name, event: `scene/go-${id}` })),
      items: items.map((i) => ({ id: i.id, name: i.name })),
      scenery: scenery.map((s) => ({ id: s.id, name: s.name })),
      npcs: npcs.map((c) => ({ id: c.id, name: c.name })),
    },
    inv: game.inv.map((id: any) => ({ id, name: ITEMS[id]?.name ?? id, held: game.held === id })),
    forms: game.forms.length,
    log: game.log,
    dialogue: dlg && node ? {
      who: npc!.name,
      text: node.text,
      options: (node.options ?? []).map((o: any, i: any) => ({ index: i, text: o.text })),
    } : null,
    duel: game.duel ? {
      ...
      known: game.duel.known.map((i: any) => ({ index: i, text: INSULTS[i].comeback })),
...
function sceneMermaid(current: any) {
  const label = (id: any) => (id === current ? `📍 ${LOCATIONS[id].name}` : LOCATIONS[id].name);
...
export function createGameRuntime(env: {
  ...
  const say = (dispatch: any, text: any, kind = 'narrate') => dispatch('game/say', { kind, text });
  ...
  const rawEffects = {
    ...
    'game-move': (props: any, dispatch: any) => {
    ...
    'game-export-forms': (_props: any, dispatch: any) => {
      ...
      const rows = forms.map((f: any, i: any) => ({ 'Form No.': `${i + 1}/${i + 1}/${i + 1}`, 'Item surrendered': f.item, Recipient: f.to, Status: 'FILED IN TRIPLICATE' }));
    ...
    'game-save': (_props: any, dispatch: any) => {
      try {
        if (!env.saveSlot || env.saveSlot.write(stringifyJsonx(env.getApp().getState().game)!) === false) throw new Error('Save unavailable');
        say(dispatch, '💾 Voyage saved.', 'sys');
      }
      catch { say(dispatch, 'The parrot ate the save file. Try again.', 'sys'); }
    },
    'game-load': (_props: any, dispatch: any) => {
    ...
    'game-interact': (props: any, dispatch: any) => {
      const g = env.getApp().getState().game;
      ...
      if (verb === 'look') { say(dispatch, `${cap(t.name)}: ${t.examine ?? ('look' in t ? t.look : undefined) ?? 'Nothing remarkable.'}`); return; }
    ...
    'game-inv': (props: any, dispatch: any) => {
      const g = env.getApp().getState().game;
      const id = props.id;
    ...
    'game-talk': (props: any, dispatch: any) => {
    ...
    'game-duel-start': (_props: any, dispatch: any) => {
    ...
    'game-duel-say': (props: any, dispatch: any) => {
    ...
    'game-duel-learn': (_props: any, dispatch: any) => {
      const d = env.getApp().getState().game.duel;
      if (!d) return;
    ...
    'game-dialogue-pick': (props: any, dispatch: any) => {
      const g = env.getApp().getState().game;
    ...
    // the dynamic tier: an NPC answers free text live, in-character
    'game-ai-say': (props: any, dispatch: any) => {
      ...
      if (!env.isConfigured?.(s)) {
        say(dispatch, `(${npc.name} would answer live if you configured a provider in the assistant settings.)`, 'sys');
        return;
      }
      ...
      const client = createChatClient({
        ...
        fetch: env.aiFetch,
        headers: env.headers ?? { 'X-Title': 'Tangle adventure' },
      });
      ...
      return out.generate([{ role: 'system', content: sys }, { role: 'user', content: question }], { signal: lifetime.signal })
        .then((r) => {
        ...
        .catch((err: any) => dispatch('game/ai-reply', { kind: 'sys', text: `(${npc.name} is briefly speechless: ${String(err.message ?? err).slice(0, 80)})` }));
    },
  };

  /** advance the duel to the next insult and announce it. */
  function throwNext(dispatch: any, base: any, from: any) {
  ...
  function resolveUse(dispatch: any, g: any, held: any, targetId: any) {
  ...
  function resolveGive(dispatch: any, g: any, held: any, npcId: any) {
  ...
  return { effects, dispose() { if (disposed) return; disposed = true; lifetime.abort(); } };
}
