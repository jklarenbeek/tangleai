export interface Location { id: string; name: string; icon: string; look: string; exits: string[]; }
export interface Character { id: string; name: string; at: string; mood: string; voice: string; look: string; }
export interface Item { id: string; name: string; at?: string; portable: boolean; examine: string; combine?: Record<string, string>; }
export interface Scenery { id: string; name: string; at: string; examine: string; }
export interface Puzzle { id: string; at: string; clue: string; solve: { verb: string; item: string; target: string; }; done: string; reward?: string; }
export interface Dialogue { root: string; nodes: Record<string, { text: string; give?: string; options: { text: string; to: string | null; give?: string; }[]; }>; }
...
export const DIALOGUE: Record<string, Dialogue> = {
  gullbert: {
    ...
    nodes: {
      g0: {
        text: 'Gullbert regards you the way a cliff regards a lemming.', options: [
  ...
  miles: {
    ...
    nodes: {
      m0: {
        text: 'Miles wipes his hands on an apron that has seen things. "Chef! You’re on. What do you need?"', options: [
  ...
  marina: {
    ...
    nodes: {
      r0: {
        text: 'Dr. Barnacle inhales as if tasting your aura. "Ah. A base note of panic. Charming."', options: [
  ...
  crumble: {
    ...
    nodes: {
      c0: {
        text: 'Captain Crumble salutes a beat too long. "State your business, sailor. And please, no *slang*. It gives me hives."', options: [
