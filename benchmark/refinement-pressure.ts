...
import { pressureFixture, runPressure } from './lib/refinement-pressure.ts';
import { sha256 } from './lib/relevance.ts';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  const live = flags.live ? configureLiveEmbeddings({
    datasetHash: sha256(JSON.stringify(fixture)),
    directory: flags.cache, dims: Number(flags.dims), textPolicy: 'pressure-text-and-lossless-merges-v1'
  }) : null;
  ...
  if (flags['live-proposals']) {
    ...
    const client = createChatClient({
      provider: env.provider, baseUrl: env.baseUrl, model: env.model, apiKey: env.apiKey,
      retry: { attempts: 1 }
    });
    liveClient = {
      endpoint: client.endpoint, complete: (request: any) => {
        if (generationCalls >= env.maxCalls) throw new Error('generation request ceiling reached');
        generationCalls++;
        return client.complete({ ...request, signal: request.signal ? AbortSignal.any([request.signal, AbortSignal.timeout(60000)]) : AbortSignal.timeout(60000) });
      }
    };
  }
  let result;
  try {
    result = await runPressure({
      fixture, embedder, directory: flags.directory,
      policies: liveClient ? ['none'] : flags.policies?.split(','), liveClient, embeddingClass: flags.live ? 'live' : 'hash'
    });
  }
  catch (error: any) {
    if (flags.filepath) writeFileSync(flags.filepath, JSON.stringify({
      ...
      error: 'pressure run failed; previous batches retained'
    }, null, 2) + '\n');
    throw error;
  }
  if (live) Object.assign(result, { live: live.report() });
  if (liveClient) Object.assign(result, {
    generation: {
      ...result.generation, calls: generationCalls, maxCalls: env.maxCalls,
      retryAttempts: 1, timeoutMs: 60000
    }
  });
  if (flags.filepath) writeFileSync(flags.filepath, JSON.stringify(result, null, 2) + '\n');
  console.log(JSON.stringify(result.results.map((r: any) => ({
    policy: r.policy, pass: r.decision?.pass,
