...
import { loadDataset, scoreRelevance, sha256 } from './lib/relevance.ts';
import { configureLiveEmbeddings } from './lib/relevance-live.ts';
import { createRankStorage, createProjectionIndex, ANN_BARS, annDecision, annMetrics } from './lib/relevance-ann.ts';
import { loadLedger, recencyPolicy, tagPolicy, rankedPolicy } from './lib/retrieval.ts';
import { tokens } from '../scripts/generate-retrieval-corpus.ts';

/** Deterministic nested subsets keep every labelled answer; full-size is the reference.
 * @param dataset @param size @param seed */
export function relevanceSubset(dataset: any, size: number, seed: number) {
  const gold = new Set(dataset.questions.flatMap((q: any) => q.gold));
  ...
  const byHash = (a: any, b: any) => sha256(`${seed}:${a.id}`).localeCompare(sha256(`${seed}:${b.id}`));
  const selected = [...dataset.documents.filter((d: any) => gold.has(d.id)).sort(byHash),
  ...dataset.documents.filter((d: any) => !gold.has(d.id)).sort(byHash)].slice(0, size);
  return selected.sort((a: any, b: any) => a.id < b.id ? -1 : 1);
}

/** Update/delete probes operate through real ledger writes and rank reads, then restore.
 * @param ledger @param memories @param questions @param embedder */
async function lifecycle(ledger: any, memories: any[], questions: any[], embedder: any) {
  const times: any[] = [];
  ...
  const position = updated.error ? -1 : updated.memories.findIndex((m: any) => m.id === target.id);
  ...
  const deletedAbsent = !deleted.error && !deleted.memories.some((m: any) => m.id === target.id);
 ...
 * @param dataset @param embedder @param [options] */
export async function runRelevance(dataset: any, embedder: any, options: {
  sizes?: number[];
  seed?: number;
  ann?: boolean;
  ...
  const texts = [...dataset.documents, ...dataset.questions].map((d: any) => d.text);
  const vectors = await embedder.embed(texts);
  const byText = new Map(texts.map((text: any, i) => [text, vectors[i]]));
  const frozen = {
    model: embedder.model, dims: embedder.dims, embed: async (requested: any) => requested.map((text: any) => {
  ...
  for (const size of sizes) {
    ...
    const memories = selected.map((d: any) => ({
      id: d.id, text: d.text, evidence: `dataset:${dataset.hash}:${d.id}`,
      tags: [...new Set(tokens(d.text).filter((word: any) => word.length >= 5))].slice(0, 3), at: '2026-01-01T00:00:00.000Z'
    }));
    ...
    const annotate = (row: any) => ({
      ...row, size, datasetId: dataset.manifest.id,
    ...
    for (const fraction of [null, ...(options.ann ? [0.1, 0.5] : [])]) {
      const start = performance.now();
      const storage = createRankStorage({
        ...identity, ...(fraction === null ? {} : {
          indexFactory: ({ dims }: any) => createProjectionIndex({ dims, seed, projections: 32, fraction }),
        })
      });
      ...
      const row = annotate({
        ...measured, buildMs, storage: storage.stats(), config: storage.indexConfig,
        lifecycle: await lifecycle(ledger, embedded, dataset.questions, frozen)
      });
      if (fraction === null) exact = row;
      else {
        row.oracle = annMetrics(exact.rankings.map((r: any) => r.ids), row.rankings.map((r: any) => r.ids));
  ...
  return {
    schemaVersion: 1, instrument: 'recall-quality', date: new Date().toISOString(),
    ...
    config: {
      seed, sizes, ks: [1, 5, 10], ann: options.ann ?? false, scope: 'memories',
    ...
    embedding: {
      ...identity, class: options.embeddingClass ?? 'scripted',
      vectorHashes: texts.map((text: any, i) => ({ textHash: sha256(text), vectorHash: sha256(JSON.stringify(Array.from(vectors[i]))) }))
    },
    bars: ANN_BARS, rows,
    annDecision: options.ann ? (rows.some((r: any) => r.decision?.pass) ? 'eligible; review lifecycle before promotion' : 'retain exact; no contender cleared all bars') : 'not evaluated'
  };
}

/** CLI shared by the standalone runner and retrieval.js --dataset. @param [argv] */
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (flags.help) {
    console.log('node benchmark/recall-quality.ts --dataset MANIFEST [--live --dims N --cache DIR] [--ann] [--sizes 500,2000,5183] [--filepath FILE]');
    return;
  }
  ...
  const live = flags.live ? configureLiveEmbeddings({
    datasetHash: dataset.hash,
    directory: flags.cache, dims: Number(flags.dims), batch
  }) : null;
  ...
  try {
    if (flags['embed-only']) {
      ...
      result = {
        schemaVersion: 1, instrument: 'recall-quality-embedding', date: new Date().toISOString(),
    ...
    if (live) Object.assign(result, { live: live.report() });
  ...
  console.log(JSON.stringify({
    instrument: result.instrument, rows: result.rows?.length ?? 0,
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url))
  main().catch((error: any) => { console.error(error.message); process.exitCode = 1; });
