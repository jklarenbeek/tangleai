 ...
 *
 *   node benchmark/long-horizon.ts                        # ceilings only
 *   node --env-file-if-exists=.env benchmark/long-horizon.ts --live
 *   node benchmark/long-horizon.ts --output json --filepath out.json
 *
...
import { readAiEnv, describeAiEnv, AI_ENV } from './lib/env.ts';
import { REPLAY_CACHE_DIR, createFileReplayCache } from './lib/replay-cache.ts';
import {
...
function parseArgs(argv: any) {
  ...
  for (let i = 2; i < argv.length; i++) {
    switch (argv[i]) {
      ...
      case '--budgets': options.budgets = argv[++i].split(',').map((s: any) => parseInt(s.trim(), 10)); break;
      case '--output': case '-o': options.output = argv[++i]; break;
      case '--filepath': case '-f': options.filepath = argv[++i]; break;
      case '--help': case '-h':
        console.log('Usage: node [--env-file-if-exists=.env] benchmark/long-horizon.ts [options]\n');
...
const pct = (x: any) => `${(x * 100).toFixed(1)}%`;
const SHAPE_KEYS = (['front', 'late'] as const);
const TASK_KEYS = (['needle', 'pairwise'] as const);
...
const VARIANT_KEYS = (['synopsis', 'ledger'] as const);
 ...
 * @returns one entry per variant+shape+budget
 */
async function runCeilings(corpus: any): Promise<any[]> {
  const out: any[] = [];
 ...
 */
async function runPrograms(corpus: any) {
  const out: any[] = [];
  for (const shape of SHAPE_KEYS) {
 ...
 */
async function runScheduling(corpus: any) {
 ...
 * @param [ledger]
 */
async function ask(client: any, context: any[], question: string, ledger?: any) {
  ...
  const agent = createAgent({
    client: {
      complete: (call: any) => client.complete({ ...request, ...call }).then((completion: any) => {
        add(completion);
        return completion;
      }),
    },
 ...
 */
async function runLive(corpus: any, ceilings: any, config: any): Promise<{
  rows: Map<string, any>;
  asked: number;
  recalls: number;
  failed: number;
  ...
  const results = await mapConcurrent(work, config.maxConcurrency, async (item) => {
    ...
    try {
      ...
      return { key: item.key, correct, ok: true as const, recalls: answer.recalls };
    }
    catch (error: any) {
      const message = (error as Error).message;
      failed += 1;
      if (errors.length < 5) errors.push(`${item.key}: ${message}`);
      return { key: item.key, correct: false, ok: false as const, recalls: 0 };
    }
  });
 ...
 */
async function fetchPricing(config: any) {
  try {
    ...
    const entry = (payload?.data ?? []).find((m: any) => m?.id === config.model);
 ...
 * here.
 */
async function runDepths(corpus: any) {
  const factories = {
    createLedger,
    createEnvironment,
  ...
  for (const depth of DEPTHS) {
    ...
    const nearest = (values: any, p: any) => quantile(values, p, { method: 'nearest-rank' }) ?? 0;
    rows.push({
      depth,
      tasks: runs.length,
 ...
 * @param client - the counting wrapper the program tier built
 */
async function runLiveDepths(corpus: any, config: any, client: any) {
  const rows: any[] = [];
  for (const depth of [1, 2]) {
    ...
    for (const index of targets) {
      ...
      try {
        ...
        errors.push(`depth ${depth}: ${(err as Error).message}`.slice(0, 160));
      }
    }
 ...
 */
async function runLivePrograms(corpus: any, config: any) {
  const client = createChatClient({
    provider: config.provider,
    baseUrl: config.baseUrl,
    apiKey: config.apiKey,
  ...
  const authored = { trials: 0, compiled: 0, attempts: 0, errors: [] as string[] };
  ...
  for (let trial = 0; trial < config.trials; trial++) {
    ...
    try {
      ...
      authored.errors.push(`authoring failed: ${(err as Error).message}`);
    }
  }
...
function printTables(corpus: any, ceilings: any, live: any, config: any) {
  ...
  for (const shape of SHAPE_KEYS) {
    for (const variant of VARIANT_KEYS) {
      ...
      for (const row of ceilings.filter((r: any) => r.shape === shape && r.variant === variant)) {
        const actual = (task: any) => {
  ...
  const determined = ceilings.filter((row: any) => row.compacted && row.ceilings.pairwise === 1);
  ...
  const above = live === null ? [] : ceilings.flatMap((row: any) => TASK_KEYS.flatMap((task) => {
 ...
 */
function printPrograms(corpus: any, programs: any[], scheduling: any, livePrograms: any) {
 ...
 * what it cost.
 */
function printDepths(depths: any[]) {
  ...
  const base = depths.find((r: any) => r.depth === 0);
  const deeper = depths.filter((r: any) => r.depth > 0 && r.callsMedian > 0);
...
function jsonRows(corpus: any, ceilings: any, live: any, config: any) {
  const rows: any[] = [];
  for (const row of ceilings) {
    for (const task of TASK_KEYS) {
      const slot = live?.rows.get(`${task}|${row.variant}|${row.shape}|${row.budget}`);
 ...
 */
function programRows(corpus: any, programs: any[], livePrograms: any, config: any) {
  const rows: any[] = [];
  for (const row of programs) {
    for (const task of TASK_KEYS) {
...
function displayTables(corpus: any, ceilings: any, live: any, config: any) {
  ...
  return SHAPE_KEYS.flatMap((shape) => VARIANT_KEYS.map((variant) => {
    ...
    return {
      ...
      rows: ceilings.filter((r: any) => r.shape === shape && r.variant === variant).map((row: any) => {
        const actual = (task: any) => {
          const slot = live?.rows.get(`${task}|${variant}|${shape}|${row.budget}`);
          return slot === undefined || slot.trials === 0
 ...
 */
function programTable(corpus: any, programs: any[], scheduling: any, livePrograms: any) {
...
async function main() {
  if (flags.authoring || flags.questionStream || flags.hierarchical) {
    let result;
    if (flags.authoring) result = await (await import('./lib/authoring-probes.ts')).authoringScorecard(flags);
    else if (flags.questionStream) result = await (await import('./lib/question-stream.ts')).questionStreamScorecard(flags);
    else result = await (await import('./lib/hierarchical.ts')).hierarchicalScorecard(flags);
  ...
  if (flags.output === 'json' && flags.filepath !== null) {
    ...
    const worst = ceilings
      .filter((r: any) => r.shape === 'late' && r.compacted && r.variant === 'synopsis')
      .reduce((a: any, b: any) => (a === null
    ...
    const paired = worst === null ? null : ceilings.find((r: any) => r.variant === 'ledger'
    ...
    const determined = ceilings.filter((r: any) => r.compacted && r.ceilings.pairwise === 1)
      .map((r: any) => `${r.variant}/${r.shape}@${r.budget}`);
