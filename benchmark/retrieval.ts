 ...
 * `scripts/generate-retrieval-corpus.ts`) it scores, per policy, whether
 ...
 *
 *   node benchmark/retrieval.ts                           # both sizes
 *   node benchmark/retrieval.ts --sizes 1000              # one size
 *   node benchmark/retrieval.ts --output json --filepath out.json
 *   node --env-file-if-exists=.env benchmark/retrieval.ts --live
 *
 * `--live` scores a SECOND ranked row, `near-live`, through the provider
 * `lib/env.ts` resolves (`readAiEnv`) and the `/embeddings` model in
...
import { readAiEnv, describeAiEnv } from './lib/env.ts';
import { formatNs } from './lib/fmt.ts';
import { KS, LIVE_BATCH, POLICY_SEED, loadCorpus, runSize } from './lib/retrieval.ts';
import { createDbStorage } from './lib/ledger-db.ts';
...
function parseArgs(argv: any) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    switch (argv[i]) {
      ...
      case '--sizes': options.sizes = argv[++i].split(',').map((s: any) => parseInt(s.trim(), 10)); break;
      ...
      case '--help': case '-h':
        console.log('Usage: node benchmark/retrieval.ts [options]\n');
...
const fmt3 = (x: any) => x.toFixed(3);
const pct = (x: any) => `${(x * 100).toFixed(1)}%`;
const ms = (x: any) => formatNs(x * 1e6);
const thousands = (n: any) => n.toLocaleString('en-US');

const HEAD = ['policy', ...KS.map((k) => `recall@${k}`), 'MRR', 'median', 'p95'];

/** The console table for one size. */
function printSize(result: any) {
  ...
  const width = Math.max(30, ...result.policies.map((policy: any) => policy.label.length + 1));
...
function tableFor(result: any) {
  const rows = result.policies.map((policy: any) => ({
...
function printMisses(result: any) {
  const incumbent = result.policies.find((p: any) => p.key === 'tag+recency');
 ...
 * @param fresh - ignore the replay store (still remember what is bought)
 */
async function liveEmbedder(fresh: boolean): Promise<{
  embedder: any;
  provider: string;
  maxCalls: number;
  reason: null;
...
async function main() {
  ...
  for (const size of sizes) {
    ...
    if (live.embedder !== null) {
      ...
      if (calls > ('maxCalls' in live ? live.maxCalls : 0)) {
        liveNotes.push(`${thousands(size)} memories skipped: ${calls} embedding calls exceed JAREN_AI_MAX_CALLS=${('maxCalls' in live ? live.maxCalls : 0)}`);
      }
      else liveAt = { embedder: live.embedder, label: `near-live (${live.embedder.model}, ${('provider' in live ? live.provider : undefined)})` };
    }
    const result = await runSize(corpus, size, {
      seed: flags.seed, live: liveAt,
  ...
  const liveSkipped = live.embedder === null
    ...
    : liveRan.length === 0
      ? liveNotes.join('; ')
      : `not skipped — near-live scored through ${('provider' in live ? live.provider : undefined)} · ${live.embedder.model} at`
  ...
  if (flags.output === 'json' && flags.filepath !== null) {
    ...
    const at = (result: any, key: any) => result.policies.find((p: any) => p.key === key);
    ...
    const liveMeta = liveRan.length === 0 ? {} : {
      live: {
        provider: ('provider' in live ? live.provider : undefined),
        model: (live.embedder as any).model,
...
(labelled ? import('./recall-quality.ts').then((runner) => runner.main()) : main()).catch((error: any) => {
  console.error(error);
  process.exit(1);
});
