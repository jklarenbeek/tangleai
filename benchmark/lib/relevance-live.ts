...
import { createRelevanceCache, meteredFetch } from './relevance-cache.ts';
...
export function configureLiveEmbeddings({ datasetHash, directory, dims, batch = 64, textPolicy }: {
  ...
  const wire = createEmbeddingClient({
    provider: env.provider, baseUrl: env.baseUrl, model, apiKey: env.apiKey,
    dims, retry: { attempts: 1 }, timeoutMs: 60000, fetch: meter.fetch
  });
  const cache = createRelevanceCache({
    directory, embedder: wire, batch, maxCalls: env.maxCalls,
    usage: () => meter.usage,
    identity: {
      provider: env.provider, model, dims, datasetHash,
  ...
  return {
    ...
    report: () => {
      ...
      if (meter.usage.requests) {
        ...
        const index = history.findIndex((entry: any) => entry.runId === runId);
        if (index < 0) history.push(row); else history[index] = row;
        writeFileSync(path, JSON.stringify(history, null, 2) + '\n');
      }
      const batches = cache.history();
      return {
        identity: cache.identity, batch, maxBatchChars: 100000, maxCalls: env.maxCalls,
        cache: cache.stats, batches, usage: history, wallMs: performance.now() - start,
        unmeteredHistoricalRequests: Math.max(0, batches.length - history.reduce((sum: any, row: any) => sum + row.requests, 0)),
