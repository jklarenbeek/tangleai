...
// The measuring instrument behind `benchmark/retrieval.ts`: the corpus
...
// so `test/context/retrieval-corpus.test.ts` can drive the same functions
 ...
 */
export function corpusAt(corpus: any, size: number): {
  size: number;
  memories: any[];
  questions: any[];
  tags: Set<string>;
  ...
  const present = new Set(memories.map((m: any) => m.id));
 ...
 */
export function questionTags(text: string, tagSet: Set<string>): string[] {
  const out: any[] = [];
 ...
 * @param [options]
 */
export async function loadLedger(memories: any[], options: {
  embedder?: import('@tangleai/models/embed').Embedder;
  storage?: any;
} = {}) {
  ...
  const results = await Promise.all(memories.map((m: any) => ledger.addMemory(m)));
  const rejected = results.find((r: any) => (r as any).error !== undefined);
 ...
 */
export const fourMethods = (adapter: any) => ({
 ...
 * not the policy.
 * @param count - how many records the sweep must embed
 * @param [batch] - texts per seam call (the ledger's default when absent)
 */
export async function sweepLedger(ledger: any, count: number, batch?: number): Promise<{
  embedded: number;
  ms: number;
}> {
 ...
 * @param [via] - the path this row must have taken
 */
export function rankedPolicy(ledger: any, key: string, label: string, via?: 'sweep' | 'adapter'): {
  key: string;
  label: string;
  run: (question: any, k: number) => Promise<string[]>;
} {
  return {
    key, label,
    run: async (question: any, k) => {
      ...
      return result.memories.map((m: any) => m.id);
    }
  };
}

/** The default recall, capped — recency, and nothing else. */
export function recencyPolicy(ledger: any, key: any, label: any) {
  return {
    key, label,
    run: async (_question: any, k: any) => {
      const records = await ledger.recall({ limit: k });
      return Array.isArray(records) ? records.map((r: any) => r.id) : [];
    }
  };
}

/** The incumbent: the question's words that are tags, then recency. */
export function tagPolicy(ledger: any, tags: any, key: any, label: any) {
  return {
    key, label,
    run: async (question: any, k: any) => {
  ...
  return [
    {
      ...
      run: async (question: any) => [...question.gold]
    },
    {
      ...
      run: async (_question: any, k) => drawDistinct(random, ids.length, k).map((i) => ids[i])
    },
 ...
 * @param [ks]
 * @returns `misses` names the questions with no gold id in the top k
 */
export async function scorePolicy(policy: {
  run: (question: any, k: number) => Promise<string[]>;
}, questions: any[], ks: number[] = KS): Promise<{
 ...
 */
export function randomFloor(questions: any[], n: number, k: number) {
  let sum = 0;
  for (const question of questions) {
 ...
 */
export function randomBand(questions: any[], n: number, k: number): {
  floor: number;
  low: number;
  high: number;
} {
 ...
 * @param [ks]
 */
export function assertScorer(result: {
  n: number;
  questions: any[];
  policies: Array<{
    key: string;
 ...
 * @param open - the adapter factory
 */
export async function durablePolicies(at: {
  memories: any[];
  tags: Set<string>;
}, embedder: import('@tangleai/models/embed').Embedder, open: (dims: number) => Promise<any>) {
  const adapter = await open((embedder.dims as number));
  const start = process.hrtime.bigint();
 ...
 * @param [ks]
 */
export function assertDurable(result: {
  n: number;
  policies: Array<{
    key: string;
    recall: Record<number, number>;
  ...
  const at = (key: any) => result.policies.find((p) => p.key === key);
 ...
 * @param [options]
 */
export async function runSize(corpus: any, size: number, options: {
  seed?: number;
  ks?: number[];
  live?: {
    embedder: import('@tangleai/models/embed').Embedder;
  ...
  store?: ((dims: number) => Promise<any>) | null;
} = {}) {
  ...
  if (options.live != null) {
    ...
    live = {
      model: options.live.embedder.model, dims: (options.live.embedder.dims as number),
  ...
  const untagged = at.questions.filter((q: any) => questionTags(q.text, at.tags).length === 0).length;
  const result = {
    size,
