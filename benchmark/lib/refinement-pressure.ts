...
import { createDbStorage } from './ledger-db.ts';
import { sha256 } from './relevance.ts';
...
export const DEDUP_BARS = Object.freeze({
  recallTolerance: 0, mrrTolerance: 0, ndcgTolerance: 0,
  conflictRetention: 1, complementRetention: 1, minimumByteReduction: 0.1,
  minimumDuplicateReduction: 0.1, defaultEligible: false
});

const normalized = (text: any) => text.trim().replace(/\s+/g, ' ').toLowerCase();
const sameEvidence = (a: any, b: any) => a.evidence === b.evidence;
const exactEvidence = (a: any, b: any) => a.text === b.text && sameEvidence(a, b)
 ...
 * @param expected @param suggested */
export function guardedMerge(expected: any, suggested: any) {
 ...
 * @param policy @param existing @param proposals @param vectors
 * @param [suggestMerge] */
export function pressurePatch(policy: string, existing: any[], proposals: any[], vectors: Map<string, any>, suggestMerge: (expected: any) => any = (expected: any) => expected) {
  const next = structuredClone(existing);
  for (const proposal of proposals) {
    if (policy === 'none' || policy === 'runtime-exact-evidence') { next.push(proposal); continue; }
    if (policy === 'exact-evidence') {
      if (!next.some((record: any) => exactEvidence(record, proposal))) next.push(proposal);
      continue;
    }
    if (policy === 'normalized-text') {
      if (!next.some((record: any) => normalized(record.text) === normalized(proposal.text))) next.push(proposal);
      continue;
    }
    if (policy === 'similarity-only') {
      if (!next.some((record: any) => cosineSimilarity(vectors.get(record.text), vectors.get(proposal.text)) >= 0.95)) next.push(proposal);
      continue;
    }
    if (policy === 'identical-evidence-merge' || policy === 'guarded-model-merge') {
      ...
      const target = next.find((record: any) => record.evidence === proposal.evidence
        || record.evidence.startsWith(proposal.evidence + '\nSource ids: '));
      if (!target) { next.push(proposal); continue; }
      if (target.text.split('\n').includes(proposal.text) && proposal.tags.every((tag: any) => target.tags.includes(tag))) continue;
      const expected = {
 ...
 * @param fixture @param records */
export function representedUnits(fixture: any, records: any[]) {
  return [...new Set(records.flatMap((record: any) => fixture.facts
    .filter((fact: any) => record.text.split('\n').includes(fact.text) && record.evidence.includes(fact.evidence))
    .map((fact: any) => fact.unit)))];
}
 ...
 * @param questions @param rankings @param available */
export function pressureMetrics(questions: any[], rankings: string[][][], available: string[][]) {
  const recall = { 1: 0, 5: 0, 10: 0 };
  let mrr = 0, ndcg10 = 0;
  const count = (mask: any) => { let n = 0; while (mask) { n += mask & 1; mask >>>= 1; } return n; };
  for (let i = 0; i < questions.length; i++) {
    ...
    const mask = (units: any) => gold.reduce((bits: any, id: any, j: any) => units.includes(id) ? bits | (1 << j) : bits, 0);
    const masks = rankings[i].map(mask);
    for (const k of [1, 5, 10]) recall[k as keyof typeof recall] += count(masks.slice(0, k).reduce((a: any, b: any) => a | b, 0)) / gold.length;
    const first = masks.findIndex((m: any) => m !== 0);
  ...
  return {
    recall: Object.fromEntries(Object.entries(recall).map(([k, value]) => [k, value / questions.length])),
 ...
 * @param records */
export function similarityBands(records: any[]) {
...
export async function runPressure({ fixture = pressureFixture(), embedder, directory,
  ...
  const sourceTexts = [...new Set(fixture.facts.map((f: any) => f.text))];
  ...
  for (const evidence of new Set(fixture.facts.map((f: any) => f.evidence))) {
    const seen: any[] = [];
    for (const wave of fixture.waves) for (const id of wave) {
      const fact = fixture.facts.find((f: any) => f.id === id);
      if (fact.evidence !== evidence || seen.includes(fact.text)) continue;
      seen.push(fact.text);
      sourceTexts.push(seen.join('\n'));
    }
  }
  const texts = [...new Set([...sourceTexts, ...fixture.questions.map((q: any) => q.text)])];
  const embedded = await embedder.embed(texts);
  const vectors = new Map(texts.map((text: any, i) => [text, embedded[i]]));
  const frozen = {
    model: embedder.model, dims: embedder.dims,
    embed: async (values: any) => values.map((text: any) => {
  ...
  for (const fact of fixture.facts) if (fact.class !== 'valuable') {
    const base = fixture.facts.find((f: any) => f.class === 'valuable' && f.tags[0] === fact.tags[0]);
    similarities.push({
      id: fact.id, class: fact.class,
  ...
  for (const policy of policies) {
    ...
    try {
      ...
      for (let wave = 0; wave < fixture.waves.length; wave++) {
        ...
        const proposals = fixture.waves[wave].map((id: any) => {
          const fact = fixture.facts.find((f: any) => f.id === id);
        ...
        const refiner = createRefiner({
          ledger, client, applyPatch: applyJSONPatch,
          maxOps: 100, maxRepairs: 0, now: () => at,
          ...(policy === 'runtime-exact-evidence' ? { deduplicate: 'exact-evidence' } : {})
        });
        const trajectory = proposals.map((p: any) => ({ role: 'tool', content: `${p.text}\n${p.evidence}` }));
        ...
        if (liveClient) {
          waves.push({
            wave: wave + 1, proposals: proposals.length, records: (await ledger.listMemories()).length,
        ...
        const appeared = new Set(fixture.waves.slice(0, wave + 1).flat().map((id: any) => fixture.facts.find((f: any) => f.id === id).unit));
        const questions = fixture.questions.map((q: any) => ({ ...q, gold: q.gold.filter((id: any) => appeared.has(id)) })).filter((q: any) => q.gold.length);
        const rankings: any[] = [];
        for (const q of questions) {
          const answer = await ledger.recall({ near: q.text, limit: 10 });
          if ('error' in answer) throw new Error('pressure recall failed');
          rankings.push(answer.memories.map((record: any) => representedUnits(fixture, [record])));
        }
        const metrics = pressureMetrics(questions, rankings, records.map((record: any) => representedUnits(fixture, [record])));
        const retained = (kind: any) => {
          const expected = [...new Set(fixture.facts.filter((f: any) => f.class === kind && appeared.has(f.unit)).map((f: any) => f.unit))];
          return expected.length ? expected.filter((u: any) => units.includes(u)).length / expected.length : 1;
        };
        const represented = records.map((record: any) => representedUnits(fixture, [record]));
        const duplicates = represented.reduce((sum, list) => sum + list.length, 0) - units.length;
        const keys = await storage.keys();
        const bytes = async (selected: any) => (await Promise.all(selected.map(async (key: any) => Buffer.byteLength(JSON.stringify(await storage.get(key)))))).reduce((a: any, b: any) => a + b, 0);
        waves.push({
          wave: wave + 1, proposed, additions: records.length - before.length,
          records: records.length, valuableUnits: units.length, duplicates, bytes: await bytes(keys.filter((k: any) => k.startsWith('ai/state/'))),
          similarity: similarityBands(records),
          snapshotBytes: await bytes(keys.filter((k: any) => k.startsWith('ai/snap/'))),
          classCounts: Object.fromEntries(['valuable', 'duplicate', 'complement', 'conflict', 'irrelevant'].map((kind) => [kind,
            fixture.waves.slice(0, wave + 1).flat().filter((id: any) => fixture.facts.find((f: any) => f.id === id).class === kind).length])),
  ...
  for (const result of results) if (baseline && !liveClient) {
    ...
    const checks = {
      relevance: result.waves.every((row: any, i: any) => row.recall[10] >= baseline.waves[i].recall[10]
        && row.mrr >= baseline.waves[i].mrr && row.ndcg10 >= baseline.waves[i].ndcg10),
      conflict: result.waves.every((row: any) => row.conflictRetention === 1),
      complement: result.waves.every((row: any) => row.complementRetention === 1),
  ...
  return {
    schemaVersion: 1, instrument: 'refinement-pressure', date: new Date().toISOString(), node: process.version,
    ...
    ...(liveClient ? {
      generation: {
        provider: liveClient.endpoint?.provider ?? 'host-injected',
        model: liveClient.endpoint?.model ?? null, maxRepairs: 0, labelled: false
      }
    } : {}),
    config: {
      seed: 0, waves: fixture.waves.length, ledger: 'one SQLite database per policy, reopened after each wave',
