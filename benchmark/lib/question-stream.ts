...
import { createProgramRunner, programGate, createProgramAuthor } from '@tangleai/agents/program';
import { createProgramSession } from '@tangleai/agents/program-session';
 ...
 * @param [seed] */
export function questionStream(seed: number = 20260909) {
  let state = seed >>> 0;
  ...
  for (let round = 0; round < 5; round++) {
    ...
    for (const { family: [family, words, query, expected] } of order) {
      rows.push({
        id: `q${rows.length}`, family, expected,
...
export async function simulateQuestionStream(options: {
  ...
  for (const item of questionStream(seed)) {
    const row = {
      id: item.id, family: item.family, class: item.class, selectedFamily: null as string | null,
      score: null as number | null, eligible: families.has(item.family), correctReuse: 0, falseReuse: 0, rejectedReuse: 0,
    ...
    if (row.correct && !families.has(item.family)) {
      ...
      const stored = await ledger.addSkill({
        name: item.family, when: item.question,
  ...
  const sum = (key: any) => rows.reduce((total: any, row: any) => total + Number(row[key]), 0);
  const correctReuse = sum('correctReuse');
  const falseReuse = sum('falseReuse');
  return {
    threshold, dims, mode: 'deterministic', costModel: { authorTokens, authorMs, retrievalTokens: 5, retrievalMs: 2, executionMs: 1 },
...
export async function questionStreamScorecard(options: Record<string, any> = {}) {
  ...
  return {
    version: 1, seed: 20260909, baseline, frontier,
    runtime: await runtimeQuestionStream({ threshold: selected.threshold, dims: selected.dims }),
    selected: {
      threshold: selected.threshold, dims: selected.dims, requiresOutcomeChecker: true,
 ...
 * @returns */
export async function liveQuestionStream(): Promise<any> {
  const env = readAiEnv();
  ...
  for (const [name, model] of [['primary', env.model], ['secondary', env.modelStrong]]) {
    for (const mode of ['fresh', 'reuse']) {
      ...
      const routes: any[] = [];
      const options = {
        environment, compileQuery: compileJsonQuery, createStructuredOutput,
        ...
        onRoute: (event: any) => routes.push(event),
      ...
      let current: any;
      const session = createProgramSession({
        ...options, author: createProgramAuthor(options),
        ...(mode === 'reuse' ? {
          reuse: {
            environmentId: 'question-stream-live-v1', schemaVersion: 'program-v1',
            embedder, threshold: DEFAULT_LIVE_THRESHOLD,
            accept: ({ skill }: any) => labels.get(skill.program.question) === current.family,
            check: ({ result }: any) => result.ok && JSON.parse(result.answer.text) === current.expected
          }
        } : {}),
      });
      for (const [index, item] of stream.entries()) {
        ...
        rows.push({
          profile: name, provider: env.provider, providerVersion: null, model, mode, index,
          ...
          authorCalls: calls.filter((call: any) => call.purpose === 'author').length,
          tokens: calls.reduce((sum: any, call: any) => sum + (call.usage?.total_tokens ?? 0), 0),
          usageComplete: calls.every((call: any) => call.usage !== null), ms: performance.now() - started, routes: calls,
        });
      }
    }
  }
  return {
    version: 1, mode: 'live', threshold: DEFAULT_LIVE_THRESHOLD,
...
export async function runtimeQuestionStream(policy: {
  ...
  let current: any;
  const session = createProgramSession({
    environment, compileQuery: compileJsonQuery,
    author: { author: async () => ({ value: current.program, attempts: 1 }) },
    reuse: {
      ...policy, embedder, environmentId: 'question-stream-v1', schemaVersion: 'program-v1',
      accept: ({ skill }: any) => families.get(skill.program.question) === current.family,
      check: ({ result }: any) => result.ok && JSON.parse(result.answer.text) === current.expected
    },
  });
  for (const item of corpus) {
    ...
    rows.push({
      id: item.id, correct: result.ok, reused: result.reuse.reused,
      authorCalls: result.reuse.authorCalls, fallbackCalls: result.reuse.fallback ? 1 : 0,
      rejectedReuse: result.reuse.events.filter((event: any) => event.kind === 'rejected').length
    });
  }
  const authorCalls = rows.reduce((sum: number, row) => sum + row.authorCalls, 0);
  return {
    mode: 'deterministic', ...policy, questions: rows.length,
