 ...
 * `benchmark/long-horizon.ts`. It lives in its own module because the
 * probe's self-tests (`test/agents/benchmark-probe.test.ts`) drive exactly
 ...
 * own self-test; it is asserted in `test/agents/`, not only here.
 */

import { createAgent } from '@tangleai/agents/agent';
import { createToolbox } from '@tangleai/agents/toolbox';
import { slotAddressesIn } from '@tangleai/context/recall';
...
export const recordId = (i: any) => `REC${String(i).padStart(4, '0')}`;
 ...
 */
function closestPair(values: number[]): {
  a: number;
  b: number;
  gap: number;
} | null {
...
export function makeCorpus({ n = DEFAULTS.rounds, seed = DEFAULTS.seed }: {
  ...
  for (let attempt = 0; attempt < 64; attempt++) {
    ...
    if (pair !== null) {
      const ids = Array.from({ length: n }, (_: any, i) => recordId(i));
 ...
 */
export function needleTargets(corpus: {
  n: number;
  seed: number;
}, count: number, salt: number): number[] {
  const random = mulberry32(corpus.seed + salt * 7919);
  const pool = Array.from({ length: corpus.n }, (_: any, i) => i);
 ...
 */
export function recordingClient(rounds: number) {
 ...
 */
export function recordToolbox(corpus: {
  ids: string[];
  values: number[];
}, padding: number, shape: 'front' | 'late') {
 ...
 */
export function retention(messages: any[], corpus: {
  n: number;
  ids: string[];
  values: number[];
}): {
...
export function transcriptText(messages: any) {
 ...
 */
export function retentionIn(text: string, corpus: {
  n: number;
  ids: string[];
  values: number[];
}) {
 ...
 */
function slotText(content: any): string {
  try {
 ...
 * @param messages - the request the client received
 */
export async function reachable(messages: any[], ledger: {
 ...
 */
export function pairSurvived(messages: any[], corpus: {
  values: number[];
  pair: {
    a: number;
    b: number;
  ...
  return pairSurvivedIn(messages.map((m: any) => String(m.content ?? '')).join('\n'), corpus);
}
 ...
 */
export function pairSurvivedIn(text: string, corpus: {
  values: number[];
  pair: {
    a: number;
    b: number;
...
export async function probe({
  ...
  full: number;
  compacted: boolean;
  pairSurvived: boolean;
  idRecoverable: number;
 ...
 * @returns 0..1
 */
export function ceilingFor(task: 'needle' | 'pairwise', { valuePresent, n }: {
  valuePresent: number;
  n: number;
}): number {
  return task === 'needle'
 ...
 */
export function needleQuestion(corpus: {
  ids: string[];
}, index: number): string {
 ...
 */
export function pairwiseQuestion(): string {
 ...
 */
export function scoreNeedle(text: string, corpus: {
  values: number[];
 ...
 */
export function scorePairwise(text: string, corpus: {
  ids: string[];
  pair: {
    a: number;
    b: number;
 ...
 * @param [shape]
 */
export function corpusText(corpus: {
  ids: string[];
  values: number[];
  n: number;
}, padding: number = DEFAULTS.padding, shape: 'front' | 'late' = 'front'): string {
  const notes = 'x'.repeat(padding);
  const rows: any[] = [];
  for (let i = 0; i < corpus.n; i++) {
 ...
 * @param [options]
 */
export function needleProgram(corpus: {
  ids: string[];
...
export function extractingClient(options: {
  ...
  return {
    state,
    complete: async ({ messages, signal }: { messages?: any; signal?: any; }) => {
      state.calls += 1;
      state.inFlight += 1;
...
export async function programProbe({
  ...
  const mapped = slots.filter((s: any) => /^program\/[^/]+\/\d+$/.test(s.name));
  let reduceInput = '';
 ...
 * @param program */
export function recursiveProgram(program: any) {
  return {
    ...program, steps: program.steps.map((step: any) => step.op === 'reduce'
      ? {
