...
/** Stable content identity. @param value */
export const contentHash = (value: any) => createHash('sha256').update(JSON.stringify(value)).digest('hex');

/** Preserve historical aggregate evidence without inventing missing per-call provenance.
 * @param artifact */
export function legacyAuthoringEvidence(artifact: any) {
  return {
    version: 1,
...
export async function measureAuthoring(options: {
  ...
  const measured = {
    ...
    async complete(request: any) {
      const row = {
        ...
        responseHash: null as string | null, outcome: 'provider', codes: [],
      };
      rows.push(row);
  ...
  try {
    const result = await createStructuredOutput({
      ...
      onAttempt(event: { attempt: number; outcome: string; errors: any[]; }) {
        const row = rows.at(-1);
        row.codes = event.errors.map((error: any) => error.code).filter(Boolean);
        row.outcome = event.outcome === 'gate' && row.codes.some((code: any) => /^(AI02|JQ)/.test(code))
