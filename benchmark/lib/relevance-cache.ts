 ...
 * @param value @param dims */
export function verifiedVector(value: any, dims: number) {
  if ((!Array.isArray(value) && !(value instanceof Float32Array)) || value.length !== dims
    || !Array.from(value).every((x: any) => typeof x === 'number' && Number.isFinite(x)))
...
export function createRelevanceCache({ directory, identity, embedder, batch = 64, maxCalls = 200, maxBatchChars = 100000, usage }: {
  ...
  const bound = {
    datasetHash: identity.datasetHash, provider: identity.provider,
  ...
  const stats = { cacheHits: 0, embedded: 0, calls: 0, wallMs: 0, failures: [] as any[] };
  const journalPath = join(directory, 'batches.jsonl');
  const journal = (row: any) => writeFileSync(journalPath, JSON.stringify(row) + '\n', { flag: 'a' });
  /** @param texts */
  async function embed(texts: string[]) {
    ...
    while (pending.length) {
      ...
      stats.calls++;
      const row = { date: new Date().toISOString(), identityHash, textHashes: part.map((e: any) => e.textHash) };
      ...
      try {
        const vectors = await embedder.embed(part.map((e: any) => e.text));
        ...
        const verified = vectors.map((v: any) => verifiedVector(v, bound.dims));
        ...
        const failure = {
          ...row, outcome: 'failed', code: typeof error?.code === 'string' && /^AI\d{4}$/.test(error.code) ? error.code : 'EMBED_FAILED',
  ...
  return {
    embed, model: bound.model, dims: bound.dims, identity: bound, stats,
 ...
 * @param [fetchFn] */
export function meteredFetch(fetchFn: typeof fetch = globalThis.fetch) {
  const usage = { requests: 0, failures: 0, reportedTokens: 0, tokenReports: 0, reportedCost: 0, costReports: 0 };
  return {
    usage, fetch: async (url: any, init: any) => {
      usage.requests++;
      try {
      ...
      catch (error: any) { usage.failures++; throw error; }
    }
  };
}
