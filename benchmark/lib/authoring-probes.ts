...
import { programGate, PROGRAM_EXAMPLE, createProgramRunner } from '@tangleai/agents/program';
import { PROGRAM_SCHEMA } from '@tangleai/agents/schemas/program';
import { compileGate, STYLESHEET_EXAMPLE, stylesheetSystemMessage } from '@tangleai/jaren/stylesheet';
...
import { measureAuthoring, legacyAuthoringEvidence } from './authoring.ts';
...
export const AUTHORING_PROBES = [
  ...
  },
  {
    ...
    gate: [(doc: any) => new JarenValidator().compile(jslt)(doc), compileGate({ compile: compileJsltStylesheet })],
    ...
    check: (doc: any) => compileJsltStylesheet(doc)({ prices: [3, 7, 2] })?.total === 12
  },
];
...
export async function authoringScorecard(flags: {
  ...
  const legacy = JSON.parse(readFileSync(new URL('../jaren-long-horizon.json', import.meta.url), 'utf8'));
  const rows: any[] = [];
  let calls = 0;
  for (const route of profiles) {
    ...
    for (const probe of AUTHORING_PROBES) {
      for (let trial = 0; trial < (flags.trials ?? env.trials); trial++) {
        ...
        const client = flags.live
          ...
          : {
            endpoint: { provider: 'openrouter' }, complete: async () => ({
              message: {
                content: JSON.stringify(probe.grammar === 'jslt'
        ...
        let tick = 0;
        const attempts = await measureAuthoring({
          ...probe, client, profile: flags.live ? spec
        ...
        rows.push(...attempts.map((row: any) => ({ trial, ...row })));
      }
    }
  }
  return {
    version: 1, mode: flags.live ? 'live' : 'deterministic',
