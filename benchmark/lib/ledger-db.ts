...
// The `@tangleai/context` ledger's storage contract over one `@jarenjs/db`
...
// because both need exactly this one: `test/context/ledger-db.test.ts` runs
// the ledger's whole suite of shapes over it, and
// `benchmark/retrieval.ts --store=db` scores the instrument through it.
//
// The listing between the markers below is the recipe in `packages/context`'s
// README, verbatim, and the test is what keeps them one thing. Nothing
// here imports `@tangleai/context`: the storage contract is the whole
// interface between a ledger and where it lives, and an adapter that
// reached for the ledger would be a fork of it.

// —— the recipe (packages/context/README.md, "A durable ledger over @jarenjs/db") ——
