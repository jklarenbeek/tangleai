 ...
 */
export function readAiEnv(env: Record<string, string | undefined> = process.env): {
  live: boolean;
  reason: string | null;
  provider: string;
 ...
 * anything.
 */
export function describeAiEnv(config: ReturnType<typeof readAiEnv>): string {
