...
import { REFINEMENT_PATCH_SCHEMA } from '@tangleai/context/schemas/patch';
...
const conditional = {
  ...full, items: {
    ...
    anyOf: full.items.oneOf.map((branch: any) => ({ properties: { op: branch.properties.op, path: branch.properties.path } })),
    allOf: full.items.oneOf.map((branch: any) => ({ if: { properties: { op: branch.properties.op, path: branch.properties.path } }, then: branch })),
  }
};
...
for (const [syntax, schema] of [['oneOf', full], ['if-then', conditional]]) {
  ...
  const client = createChatClient({
    provider: live ? env.provider : 'openrouter', model: live ? env.model : 'fixture',
  ...
  try {
    ...
    rows.push({
      syntax, outcome: valid ? 'valid' : 'rejected-full-schema', acceptedRequest: true, schemaSent,
...
const result = {
  version: 1, mode: live ? 'live' : 'scripted', provider: live ? env.provider : 'fixture',
