...
import { retentionFixture, measureRetention } from './lib/retention.ts';
const { values } = parseArgs({ options: { filepath: { type: 'string' } } });
...
const result = {
  version: 1, instrument: 'ledger-retention', seed: 73, rounds: 48,
  bars: { referencedResolution: 1, resumedCorrectness: 1, retrievalRecall10: 1 },
  decision: {
    archive: 'unreferenced', goal: 'lossless-dictionary',
...
writeFileSync(values.filepath ?? new URL('./retention-result.json', import.meta.url), JSON.stringify(result, null, 2) + '\n');
