...
| consolidation lexical routing | `@jarenjs/core/search`: `compileLexical`, bounded postings/index/search and compatibility scoring, `reciprocalRankFusion` with code point id ties | source/artifact selection, NFKC word normalization, stable input-order lexical ties, one fusion vote per id per lane and exact-evidence expansion; frozen BM25 reference is benchmark-only |
...
Current Jaren 0.91.3 adoption: [integration audit](JARENJS_INTEGRATION.md).
