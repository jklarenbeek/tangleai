...
| consolidation lexical routing | `@jarenjs/core/search`: `compileLexical`, bounded postings/index/search and compatibility scoring | source/artifact selection, NFKC word normalization, stable input-order ties and exact-evidence expansion; frozen BM25 reference is benchmark-only |
...
Current Jaren 0.90.6 adoption: [integration audit](JARENJS_INTEGRATION.md).
...
| A skill as a directory, used with no retrieval index | nothing anchored: `SKILL_SCHEMA` is a closed flat record, `json/patch` is RFC 6902 over JSON | **Landed** — `@tangleai/trace2skill` owns `SkillBundle`, the anchored directory-patch compiler and the format validator; `composeSkillSystem` + `skillReadTool` put the active directory into a request as data. A ledger skill record may be derived for display and carries no authority. The k-means-cluster-then-retrieve shape is the paper's retrieval baseline and survives only as the `retrieval-bank` ablation row. |
| Self-modification that cannot go rogue | `createGuardedRefiner` in `@jarenjs/core/guarded` (`read`/`validateProposal`/`apply`/`validateCandidate`/`planCommit`/`commit`, synchronous validators); `@tangleai/agents` `createRefiner` specializes the same idea for the context ledger's arrays | **Landed** — the skill loop's one application to the frozen directory is a `createGuardedRefiner` consumer, with evaluator-backed checks run BEFORE it as separately recorded stages; outcome artifact refinement uses the generic engine directly. Repository evolution is still open. |
| An isolated worktree, and a child process that cannot outlive its deadline | nothing: `@jarenjs/core` has no process runner and no git vocabulary, and `@jarenjs/flow`'s `createExternalEffects` fences an effect without knowing how to perform one | **Landed, Tangle-owned** — `@tangleai/evolve/host` owns the worktree lifecycle, the git allow-list of whole argument vectors, and a bounded runner whose deadline signals the child's whole process GROUP (a gate is rarely one process). It is Tangle's because a repository experiment is Tangle's problem, not the suite's. The upstream ask, if one is ever made, is a bounded process runner in `@jarenjs/core` — argv arrays only, time and output caps, group kill — and nothing git-shaped above it. |
...
| Geography | `@jarenjs/core/geo`; `@tangleai/jaren` owns the spatial gates and the geo toolbox | The place lane. |
