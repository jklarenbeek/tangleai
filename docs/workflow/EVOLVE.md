...
| stopping one, and cleaning up after it | exists — `cancelExperiment` is a host call and not a wire operation, and the cleanup is a reconciler rather than a node, because a cancelled run executes no further segment. A cancel records `abandoned`/`command`; an expiry records `abandoned`/`over-budget`; the decision vocabulary is not widened |
