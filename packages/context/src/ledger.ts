...
import { validateClaimEvidence } from './evidence.ts';
import { AiError } from '@tangleai/models/errors';
import { retainArchives } from './archive.ts';
import { jsonBytes, checkpointProgress, validateCheckpoint, goalPrompt } from './retention.ts';
import { atomicTask, isAtomicView } from './storage/transaction.ts';
import { createMemoryStorage } from './storage/memory.ts';
import { LEDGER_SCHEMAS } from './schemas/ledger.ts';
...
const seq = (n: any) => String(n).padStart(6, '0');
 ...
 */
function nextSequence(keys: string[]): number {
  let highest = -1;
 ...
 */
function defined(record: Record<string, any>): any {
 ...
 * unreproducible.
 */
function byRecency(records: any[]) {
 ...
 */
function recencyOrder(a: any, b: any) {
  return a.at === b.at
...
export function createLedger(options: {
  ...
  const storage: NonNullable<typeof options.storage> = options.storage ?? createMemoryStorage();
  for (const [limits, names] of [[options.archiveLimits, ['maxItems', 'maxBytes']],
  [options.goalLimits, ['maxEntries', 'maxBytes', 'maxChars']]] as const) {
    for (const [name, value] of Object.entries(limits ?? {})) {
      if (!(names as readonly string[]).includes(name)) throw new TypeError(`unknown ledger budget '${name}'`);
  ...
  }).valid) throw new TypeError('artifacts must be unique, valid admitted records');
  const compileQuery = typeof options.compileQuery === 'function' ? options.compileQuery : null;

  const embedder: Embedder | null = options.embedder ?? null;
  if (embedder !== null) {
   ...
   * @param [scope]
   */
  function enqueue<T>(task: (view: any) => Promise<T>, scope: import('./storage/transaction.ts').StorageScope = 'ai/'): Promise<T> {
   ...
   */
  function validate(kind: 'goal' | 'memory' | 'skill' | 'slot', record: any): null | {
    error: string;
    code?: string;
    errors: any[];
    inputSchema: any;
    ...
    if (kind === 'memory' && typeof record.evidence !== 'string') {
      ...
      if (!references.valid) return invalidInput(kind, {
        errors: references.errors.map((error: any) => ({
   ...
   */
  async function embedThrough(texts: string[]): Promise<{
    vectors: Float32Array[];
    identity: LedgerEmbeddedBy;
    error?: undefined;
  } | {
    error: string;
    code?: string;
   ...
   * @template {object} T
   */
  const withEmbedding = <T extends object>(record: T, vector: Float32Array, identity: LedgerEmbeddedBy): T & {
    embedding: number[];
   ...
   */
  async function embedOnWriteStep<T extends {
    embedding?: number[];
  }>(record: T, text: string): Promise<{
    record: T;
    embedError?: string;
  }> {
  ...
  async function readAll(prefix: any, view: { get: (key: string) => Promise<any>; set: (key: string, value: any) => Promise<void>; delete: (key: string) => Promise<void>; keys: (prefix?: string) => Promise<string[]>; mutate: import("./storage/transaction.ts").StorageMutation; } | { get: (key: string) => Promise<any>; set: (key: string, value: any) => Promise<void>; delete: (key: string) => Promise<void>; keys: (prefix?: string) => Promise<string[]>; status?: () => any; mutate?: import("./storage/transaction.ts").StorageMutation; rank?: (request: { prefix: string; vector: number[]; model: string; dims: number; limit?: number; minScore?: number; }) => Promise<{ hits: { key: string; score: number; }[]; skipped: number; identities: LedgerEmbeddedBy[]; ranking?: LedgerRanking; }>; } = storage) {
   ...
   */
  async function mintId(kind: any, prefix: any, view: any) {
  ...
  function setGoal(input: {
    ...
    return enqueue(async (storage: any) => {
      let goal = defined({
   ...
   */
  async function getGoal(): Promise<LedgerGoal | null> {
   ...
   */
  async function listArchivedGoals(): Promise<LedgerGoal[]> {
   ...
   */
  function setGoalStatus(status: 'active' | 'done' | 'abandoned' | 'superseded'): Promise<LedgerGoal | LedgerRejection | {
    error: string;
    code?: string;
  }> {
    return enqueue(async (storage: any) => {
  ...
  async function boundGoal(goal: any, view: any) {
    ...
    const ids = new Set([...(next.checkpoint?.sources ?? []), ...next.progress].map((entry: any) => entry.id));
    ...
    const fits = (candidate: any) => candidate.progress.length <= (limits.maxEntries ?? Infinity)
    ...
    if (!fits(next)) {
      ...
      catch (error: any) { return { error: `checkpoint reducer failed: ${error instanceof Error ? error.message : String(error)}`, code: 'GOAL_CHECKPOINT' }; }
      ...
      const compacted = {
        ...
        retention: { version: 1, reason: 'goal-budget', retired: next.progress.map((entry: any) => entry.id) }
      };
      const rejected = validate('goal', compacted);
      if (rejected) return rejected;
      if (!fits(compacted)) return {
   ...
   */
  async function getMemory(id: string): Promise<LedgerMemory | null> {
   ...
   */
  async function listMemories(): Promise<LedgerMemory[]> {
   ...
   */
  async function getSkill(id: string): Promise<LedgerSkill | null> {
   ...
   */
  async function listSkills(): Promise<LedgerSkill[]> {
   ...
   * `some`/`includes`.
   */
  const tagPredicate = (tags: string[]) => ({ $eq: ['$it.tags[*]', { $seq: tags }] });

  /** Compile once per distinct query document. */
  function compiled(document: any) {
   ...
   * straight face.
   */
  function filter(records: any, tags: any, where: any) {
    const parts: any[] = [];
    ...
    if (compileQuery === null) {
      ...
      return {
        value: records.filter((record: any) =>
          (record.tags ?? []).some((tag: any) => tags.includes(tag)))
      };
    }
    ...
    try {
      ...
      return { error: `recall: ${(err as any)?.reason ?? (err as any)?.message ?? err}` };
    }
  }
   ...
   */
  function retrieve(records: any[], query: LedgerQuery = {}) {
   ...
   */
  function refuseMixture(name: string, found: Map<string, LedgerEmbeddedBy>, identity: LedgerEmbeddedBy): {
    error: string;
    code?: string;
  } | null {
   ...
   * @param [ranking]
   */
  function rankedResult(records: any[], probe: any, query: LedgerQuery, member: 'memories' | 'skills', skipped: number, via: 'sweep' | 'adapter',
    ranking: LedgerRanking = { algorithm: 'exact-cosine', exhaustive: true, candidateCount: records.length }) {
    const ranked = records
      .map((record: any) => ({ record, score: cosineSimilarity(probe, record.embedding) }))
      .sort((a, b) => (b.score - a.score) || recencyOrder(a.record, b.record));
    const kept = typeof query.minScore === 'number'
      ? ranked.filter((entry) => entry.score >= (query.minScore as number))
      : ranked;
   ...
   */
  async function rankNear(query: LedgerQuery, name: 'recall' | 'recallSkills', member: 'memories' | 'skills', prefix: string): Promise<{
    error: string;
    code?: string;
  } | {
    [member: string]: any;
    ...
    if (delegated) {
      const hits = await (storage as any).rank({
      ...
      const found: Map<string, LedgerEmbeddedBy> = new Map();
      ...
      const records = await Promise.all(hits.hits.map((hit: any) => storage.get(hit.key)));
      ...
      return rankedResult(records.filter((record: any) => record?.embedding !== undefined),
    ...
    const found: Map<string, LedgerEmbeddedBy> = new Map();
    let skipped = 0;
    for (const record of (filtered as any).value) {
   ...
   */
  function recall(query: LedgerQuery & { near: string; }): Promise<LedgerRankedMemories | { error: string; }>;
  function recall(query?: LedgerQuery & { near?: undefined; }): Promise<LedgerMemory[] | { error: string; }>;
  function recall(query: LedgerQuery): Promise<LedgerMemory[] | LedgerRankedMemories | { error: string; }>;
  async function recall(query: LedgerQuery = {}): Promise<LedgerMemory[] | LedgerRankedMemories | {
    error: string;
    code?: string;
  }> {
    return query.near === undefined
      ? retrieve(await readAll(KEYS.memory), query)
      : (rankNear(query, 'recall', 'memories', KEYS.memory) as any);
  }
   ...
   * @param [query]
   */
  function recallSkills(query: LedgerQuery & { near: string; }): Promise<LedgerRankedSkills | { error: string; }>;
  function recallSkills(query?: LedgerQuery & { near?: undefined; }): Promise<LedgerSkill[] | { error: string; }>;
  function recallSkills(query: LedgerQuery): Promise<LedgerSkill[] | LedgerRankedSkills | { error: string; }>;
  async function recallSkills(query: LedgerQuery = {}): Promise<LedgerSkill[] | LedgerRankedSkills | {
    error: string;
    code?: string;
  }> {
    return query.near === undefined
      ? retrieve(await readAll(KEYS.skill), query)
      : (rankNear(query, 'recallSkills', 'skills', KEYS.skill) as any);
  }
  ...
  function embedMissing(options: {
    ...
    return enqueue(async (storage: any) => {
      ...
      const held: Map<string, LedgerEmbeddedBy> = new Map();
      ...
      const mixture = (identity: LedgerEmbeddedBy) => {
   ...
   */
  function putSlot(name: string, content: string, meta: {
    kind?: string;
    at?: string;
    count?: number;
    pinned?: boolean;
   ...
   */
  async function getSlot(name: string): Promise<LedgerSlot | null> {
   ...
   */
  async function readSlot(name: string): Promise<string | undefined | {
    status: 'evicted';
    name: string;
    bytes: number;
    reason: string;
   ...
   */
  async function listSlots(): Promise<LedgerSlot[]> {
  ...
  async function storeArchive(view: any, entries: any, protection: Record<string, any> = {}) {
    ...
    const current = Object.fromEntries(await Promise.all(keys.map(async (key: any) => [key, await view.get(key)])));
    ...
    return { ok: true as const, retention: planned.report, footprint: planned.footprint };
  }
  ...
  function putArchive(entries: any, protection: Record<string, any> = {}) {
    return enqueue(async (view: any) => {
      const records: any[] = [];
      for (const entry of entries) {
  ...
  function clearArchives(prefix: string = '') {
    return enqueue(async (view: any) => {
      ...
      else if (report) await view.set('ai/state/retention/archive', {
        ...report,
        evicted: report.evicted.filter((entry: any) => !entry.name.startsWith(prefix)),
        written: report.written.filter((name: any) => !name.startsWith(prefix))
      });
      return true;
    });
  }
  ...
  function snapshot(): Promise<string> {
    return enqueue(async (storage: any) => {
      ...
      const entries = await Promise.all(keys.map(async (key: any) => [key, await storage.get(key)]));
   ...
   */
  function rollback(token: string): Promise<true | {
    error: string;
    code?: string;
  }> {
    return enqueue(async (storage: any) => {
   ...
   */
  function transaction(expected: any, work: (ledger: any) => Promise<any>) {
    return enqueue(async (view: any) => {
...
export type LedgerRankedMemories = {
  memories: LedgerMemory[]; scores: number[]; skipped: number; /**
...
export type LedgerRankedSkills = {
  skills: LedgerSkill[]; scores: number[]; skipped: number; /**
...
export type LedgerRanking = { algorithm: string; exhaustive: boolean; candidateCount: number; };
export type LedgerQuery = { tags?: string[]; where?: any; limit?: number; near?: string; minScore?: number; };
