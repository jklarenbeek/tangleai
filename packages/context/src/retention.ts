...
import { GOAL_SCHEMA } from './schemas/ledger.ts';
...
export const jsonBytes = (value: any) => new TextEncoder().encode(JSON.stringify(value)).length;
...
export function ledgerFootprint(records: Record<string, any>) {
  ...
  return { bytes: 2 + Object.values(classes).reduce((n: any, entry: any) => n + entry.bytes, 0), overhead: 2, classes };
}

/** Lossless checkpoint: repeated note/evidence pairs share a dictionary entry. */
export function checkpointProgress(goal: any) {
  const records: any[] = [], sources: any[] = [];
  const add = (source: any, record: any) => {
    if (typeof source?.id !== 'string' || source.id === '' || !record) throw new TypeError('checkpoint needs identified source entries');
    let index = records.findIndex((held: any) => held.note === record.note && held.evidence === record.evidence);
...
export function validateCheckpoint(checkpoint: any, goal: any) {
  ...
  if (!shape.valid) return {
    valid: false as const, errors: shape.errors.map((error: any) => ({
      ...error,
  ...
  catch { return { valid: false as const, errors: [{ code: 'GOAL_CHECKPOINT', docPath: '/goal', message: 'invalid checkpoint source goal' }] }; }
  const errors: any[] = [];
  const add = (path: any, message: any) => errors.push({ code: 'GOAL_CHECKPOINT', docPath: path, instancePath: path, message });
  if (checkpoint?.version !== 1 || !Array.isArray(checkpoint?.sources) || !Array.isArray(checkpoint?.records))
    ...
    const sources = new Map(expected.sources.map((source: any) => [source.id, source]));
    const seen = new Set();
    checkpoint.sources.forEach((source: any, index: any) => {
    ...
    if (seen.size !== sources.size || [...sources.keys()].some((id: any) => !seen.has(id)))
      add('/checkpoint/sources', 'every retired source id must remain covered');
    if (checkpoint.records.some((_: any, index: any) => !checkpoint.sources.some((source: any) => source.record === index)))
...
export function goalPrompt(goal: any) {
