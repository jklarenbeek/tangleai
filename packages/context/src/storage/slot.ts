...
export function createSlotLedgerStorage(slot: {
  ...
  let ownership;
  let releaseOwner: any;
  let ownerRequest: any;
  const own = () => {
    active();
    if (!atomic || !singleWriter) return Promise.resolve();
    ownership ??= new Promise<void>((resolve, reject) => {
      const held = new Promise<void>((release) => { releaseOwner = release; });
      ownerRequest = locks.request(`ledger-owner:${name}`, { ifAvailable: true }, (lock: any) => {
        if (!lock) {
  ...
  const locked = (fn: any) => atomic ? locks.request(`ledger:${name}`, async () => {
    await taskBoundary();
    try { return fn(); }
    finally { await taskBoundary(); }
  }) : Promise.resolve().then(fn);
  const mutate = async (prefix: any, transform: any) => {
    await own();
    return locked(() => {
      const matches = (key: any) => typeof prefix === 'string' ? key.startsWith(prefix)
        : (prefix.keys ?? []).includes(key) || (prefix.prefixes ?? []).some((part: any) => key.startsWith(part));
      const current = load();
  ...
  return {
    ...
    get: async (key: any) => { await own(); return copy(load()[key]); },
    set: (key: any, value: any) => mutate('', (current: any) => ({ next: { ...current, [key]: copy(value) } })),
    delete: (key: any) => mutate('', (current: any) => {
      delete current[key];
      return { next: current };
    }),
