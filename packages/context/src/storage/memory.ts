 ...
 */
export function createMemoryStorage(backing: Map<string, string> = new Map()): {
  ...
  return {
    mutate: async (prefix, transform) => {
      ...
      if (!outcome || typeof (outcome as { then?: unknown; }).then === 'function') throw new TypeError('mutate callback must be synchronous');
