...
import { CLAIM_EVIDENCE_SCHEMA } from './evidence.ts';
 ...
 * RFC 3339
 */
  at: string; note: string; evidence: string;
};
export type LedgerGoal = {
  objective: string; /**
 * RFC 3339
 */
  createdAt: string; status: "active" | "done" | "abandoned" | "superseded"; progress: LedgerProgressEntry[]; checkpoint?: { version: 1; records: { note: string; evidence: string; }[]; sources: { id: string; at: string; record: number; }[]; }; retention?: { version: 1; reason: "goal-budget"; retired: string[]; };
};
export type LedgerEmbeddedBy = { model: string; dims: number; };
export type LedgerEmbeddingPair = { embedding?: undefined, embeddedBy?: undefined; }
  | { embedding: number[], embeddedBy: LedgerEmbeddedBy; };
export type LedgerMemoryFields = {
  id: string; text: string; evidence: string | import('./evidence.ts').ClaimEvidenceEnvelope; tags: string[]; /**
 * RFC 3339
 */
  at: string;
};
...
export type LedgerSkillFields = {
  id: string; name: string; when: string; instructions: string; tools: string[]; /**
 * RFC 3339
 */
  at: string; program?: { version: 1; question: string; fingerprint: string; environmentId: string; schemaVersion: string; document: any; evidence: string; checked: true; };
};
export type LedgerSkill = LedgerSkillFields & LedgerEmbeddingPair;
export type LedgerSlot = {
  name: string; kind: string; size: number; excerpt: string; /**
 * RFC 3339
 */
  at: string; count?: number; pinned?: boolean;
};
export type LedgerRejection = { error: string; code?: string; errors: any[]; inputSchema: any; };
