...
/**
 */
const record = (properties: Record<string, object>, required: string[]): {
  type: 'object';
...
export type ArtifactRecord = { id: string, kind: string, locator?: string, digest?: string, metadata?: Record<string, any>; };
export type EvidenceRecord = { id: string, artifact: string, selector?: string, quote?: string; };
export type ClaimRecord = { id: string, text: string, critical: boolean, status: 'supported' | 'unresolved', evidence: string[]; };
export type ClaimEvidenceEnvelope = { version: 1, artifacts: ArtifactRecord[], evidence: EvidenceRecord[], claims: ClaimRecord[], visibleEvidence: string[]; };
