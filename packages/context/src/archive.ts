...
export function retainArchives(current: Record<string, any>, added: any, limits: any, options: Record<string, any> = {}) {
  const next: Record<string, any> = structuredClone(current);
  const protectedNames = new Set([...(options.protectedNames ?? []), ...added.map((entry: any) => entry.slot.name)]);
  ...
  const report = {
    ...
    written: added.map((entry: any) => entry.slot.name)
  };
  next[REPORT] = report;
