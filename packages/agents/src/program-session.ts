...
import { createProgramAuthor, createProgramRunner, programGate } from './program.ts';
 ...
 * @param question */
export async function questionFingerprint(question: string) {
    ...
    async run(question: any, hooks: Record<string, any> = {}) {
      ...
      if (policy && !hooks.signal?.aborted) {
        ...
        for (const [index, skill] of (recalled.skills ?? []).entries()) {
          ...
          const names = (await ledger.listSlots()).map((slot: any) => slot.name);
      ...
      if (hooks.signal?.aborted) return { ok: false as const, stopped: 'aborted', reuse: { reused: false, authorCalls: 0, events } };
      ...
      if (authored.value === undefined) return { ok: false as const, errors: authored.errors, reuse };
      ...
      if (!checked) return { ...result, ok: false as const, error: 'fresh program failed outcome check', reuse };
      let pair: Record<string, any> = {};
      if (policy.embedder) {
        try {
