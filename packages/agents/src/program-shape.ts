 ...
 * @param value */
export function recursiveItems(value: any) {
  const items = value === null ? [] : Array.isArray(value) ? value : [value];
  return items.every((item: any) => item !== null && typeof item === 'object' && !Array.isArray(item)
...
export function recursiveSchema(schema: any, item: boolean = false): boolean {
  if (!schema || typeof schema !== 'object') return false;
  if (schema.type === 'array') return !item && recursiveSchema(schema.items, true);
  if (schema.anyOf) return schema.anyOf.every((branch: any) => recursiveSchema(branch, item));
...
export function recursiveShape(node: any, item: boolean = false): 'compatible' | 'incompatible' | 'unknown' {
  ...
  if (node.kind === 'array') {
    ...
    const shapes = node.elements.map((element: any) => recursiveShape(element, true));
  ...
  if (node.kind === 'object') {
    const slot = node.entries.find((entry: any) => entry.name === 'slot')?.expr;
    const value = node.entries.find((entry: any) => entry.name === 'value');
