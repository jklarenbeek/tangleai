...
import { recursiveItems } from './program-shape.ts';
import { createProgramSession } from './program-session.ts';
import { readProgramAnswer } from './program-result.ts';
 ...
 */
export const BUDGET_DIMENSIONS = (['turns', 'tokens', 'ms'] as const);
...
export function createTrajectory(): {
  ...
  return {
    ...
    summary: () => ({
      steps: entries.filter((e: any) => e.kind === 'program').length,
      calls: entries.filter((e: any) => e.kind === 'subcall' || e.kind === 'author').length,
      slots: entries.filter((e: any) => e.slot !== undefined).length,
      depths: [...new Set(entries.map((e: any) => e.depth ?? 0))].sort((a: any, b: any) => a - b),
    }),
  };
}
...
export const childScope = (depth: any, index: any) => `child/${depth}/${index}/`;
...
export function createLongHorizonAgent(options: {
   ...
   * @param [signal]
   */
  async function level(depth: number, env: any, question: string, signal?: AbortSignal) {
    const stop = account.stop();
    if (stop !== null) return { ok: false as const, depth, stopped: stop, answer: null };
    ...
    const author = authorFactory({
      ...
      depth, onRoute: (event: any) => { trajectory.add({ kind: 'route', depth, ...event }); options.onRoute?.(event); },
      environment: env,
    ...
    const runner = runnerFactory({
      ...
      subcall: depth >= maxDepth ? undefined : (name: any, prompt: any, sig: any, index: any) =>
    ...
    try {
      ...
      trajectory.add({ kind: 'author', depth, ok: false as const });
      return { ok: false as const, depth, stopped: account.stop(), error: (error as Error).message, answer: null };
    }
    trajectory.add({
   ...
   * @param [index]
   */
  async function child(depth: number, parentEnv: any, name: string, prompt: string, signal?: AbortSignal, index: number = 0) {
  ...
  return {
    run(question: string, hooks: { signal?: AbortSignal; } = {}) {
