...
import { MAX_PROGRAM_CHARS, NAME_PATTERN, PROGRAM_SCHEMA, programSchema } from './schemas/program.ts';
import { checkOutcome } from '@jarenjs/core/check';
import { unfence } from '@tangleai/models/structured';
import { recursiveShape, recursiveSchema, recursiveItems } from './program-shape.ts';
...
export function compileProgram(doc: any, options: {
  ...
  for (let i = 0; i < doc.steps.length; i++) {
    ...
    const step: any = { op, from, as, index: i };
  ...
  if (steps.filter((s: any) => s.op === 'answer').length > 1)
...
export function programGate(options: {
  ...
  return (doc: any) => {
    try {
      ...
      return { valid: false as const, errors: [errorRecord(err)] };
    }
  };
}
...
const resultSlot = (as: any) => `${RESULT_PREFIX}${as}`;

/** The family a map's per-piece results live under. */
const mapFamily = (as: any) => `${RESULT_PREFIX}${as}/`;
...
export function createProgramRunner(options: {
  ...
  async function compileHere(doc: any) {
   ...
   */
  async function membersOf(target: any): Promise<string[]> {
    if (target.matches !== undefined) {
      ...
      const names = [...new Set((listing.matches ?? []).map((m: any) => m.slot))];
      return (names as string[]);
    }
    if (target.slot !== undefined) return [target.slot];
    const slots = await environment.ledger.listSlots();
    return slots
      .filter((s: any) => s.name.startsWith(target.family))
      .map((s: any) => s.name)
      .sort((a: any, b: any) => {
   ...
   * @param [signal]
   */
  async function subcall(name: string, prompt: string, signal?: AbortSignal) {
    ...
    try {
      ...
      return { slot: name, error: `the sub-call failed: ${(err as Error).message}` };
    }
    ...
    try {
      ...
      return {
        slot: name,
        error: `the reply is not JSON: ${(err as Error).message}`,
   ...
   * @param [hooks]
   */
  async function run(doc: any, hooks: {
    signal?: AbortSignal;
  } = {}): Promise<ProgramRunResult> {
    ...
    try {
      ...
      return {
        ok: false as const, ran: 0, steps: [], subcalls: 0, failed: 0, concurrency,
     ...
     */
    async function store(step: any, data: any) {
    ...
    for (const step of plan.steps) {
      ...
      if (step.op === 'grep') {
        ...
        report.push({
          ...
          slots: new Set(result.matches.map((m: any) => m.slot)).size, size: stored.size,
        });
        continue;
      }
      ...
      if (step.op === 'select') {
        ...
        try { data = JSON.parse(String(raw)); }
          ...
          return failure(step,
            { error: `slot '${name}' is not JSON: ${(err as Error).message}` });
        }
      ...
      if (step.op === 'map') {
        ...
        try {
          ...
          if (!Boolean(signal?.aborted)) throw err;
          stopped = 'aborted';
          break;
        }
        ...
        report.push({
          ...
          as: step.as,
          subcalls: budgeted.length,
          failed: results.filter((r: any) => r.error !== undefined).length,
    ...
    function failure(step: any, result: any): ProgramRunResult {
      return {
        ok: false as const,
        answer: null,
        ran: report.length,
    ...
    if (answered.error !== undefined) return {
      ...metrics, ok: false as const, answer: null, error: answered.error,
      ...(stopped === null ? {} : { stopped }),
    };
    return {
      ...metrics, ok: true as const,
...
export function createProgramAuthor(options: {
  ...
  async function author(question: any, hooks: Record<string, any> = {}) {
    const digest = await environment.digest();
    const known = (await environment.ledger.listSlots()).map((s: any) => s.name);
