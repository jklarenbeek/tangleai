 ...
 */
function normalizeErrors(raw: any[]) {
  return raw.slice(0, MAX_ERRORS).map((e: any) => {
 ...
 * @param [refs]
 */
function compileWithRefs(schema: any, refs?: any[]): (value: any) => any {
...
export function createStructuredOutput(options: {
  ...
  const gates = options.gate === undefined ? [] : ([] as Array<(value: any) => any>).concat(options.gate);
   ...
   * @param [hooks]
   */
  async function generate(messages: any[], hooks: {
    signal?: AbortSignal;
  } = {}) {

    const request: any = { stream, signal: hooks.signal };
    let turn = [...messages];
    if (tier === 'json_schema') {
    ...
    for (let attempt = 1; attempt <= 1 + maxRepairs; attempt++) {
      ...
      try {
        ...
        errors = [{ instancePath: '', keyword: 'parse', message: `the reply is not JSON: ${(err as Error).message}` }];
