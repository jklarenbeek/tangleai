 ...
 */
const refuse = (code: any, reason: any, docPath: any) => ({
  valid: false as const, errors: [{ code, docPath, message: reason }],
});
 ...
 * @param operand - the phrase's additionalProperties schema
 * @returns a stable key ('1', '2', '3', '2..3', '1+', '0+')
 */
function arityOf(operand: any): string {
 ...
 *
 * @param schema - a grammar artifact (the query or JSLT schema)
 * @returns in grammar order
 */
export function operatorArities(schema: any): Array<{
  arity: string;
  names: string[];
}> {

  const byArity: Map<string, Set<string>> = new Map();
  /** @param node */
 ...
 */
export function operatorNames(schema: any): string[] {
 ...
 */
export function grammarKeywords(schema: any): Set<string> {
...
export function operatorCrib(schema?: any, extra: string[] = []): string {
  ...
  return 'Every operator is a one-member object whose value is its operands. Operands are'
    ...
    + groups
      .map((g) => `  ${ARITY_PHRASE[g.arity as keyof typeof ARITY_PHRASE] ?? `${g.arity} operand(s)`}: ${g.names.join(' ')}`)
      .join('\n')
 ...
 */
function rulesOf(doc: any): Array<{
  body: any;
  docPath: string;
}> {
  ...
  return rules.map((rule: any, i) => ({ body: rule?.body, docPath: `${base}/${i}/body` }));
}
 ...
 *
 * @param [options] - `allow: true` returns a gate
 ...
 * every other check.
 */
export function nonEmptyGate(): (doc: any) => true | {
  valid: false;
  errors: any[];
} {
  return function gate(doc: any) {
...
export function compileGate(options: {
  ...
  return function gate(doc: any) {
    try { compile(doc); return true; }
      ...
      const e = (err as any);
      return { valid: false as const, errors: [{ code: e.code ?? 'AI0222', docPath: e.docPath ?? '', message: e.reason ?? e.message }] };
    }
  };
}
...
export function unknownOperatorGate(options: {
  ...
  return function gate(doc: any) {
    ...
    /** @param node @param at */
    function walk(node: any, at: string) {
      if (found !== null) return;
      if (Array.isArray(node)) { node.forEach((item: any, i) => walk(item, `${at}/${i}`)); return; }
      if (node === null || typeof node !== 'object') return;
      for (const key of Object.keys(node)) {
    ...
    const matched = found as {
      name: string;
      at: string;
    } | null;
    if (matched === null) return true;
    return refuse('AI0225',
      `'${matched.name}' is not an operator in this grammar. The vocabulary is closed — rewrite `
 ...
 *
 * @param value - a sample of the document the stylesheet will run on
 * @param [options]
 * @returns one `path  type  examples` line per address
 */
export function describePaths(value: any, options: {
  maxPaths?: number;
} = {}): string {
  ...
  function walk(node: any, path: string) {
    ...
    if (type !== 'object' && type !== 'array' && entry.samples.length < MAX_SAMPLES
      && !entry.samples.some((s: any) => s === node)) entry.samples.push(node);
  ...
  for (const [path, entry] of seen) {
    ...
    const samples = entry.samples.length === 0 ? '' : `  e.g. ${entry.samples.map((s: any) => JSON.stringify(s)).join(', ')}`;
 ...
 * COMPILES — `test/jaren/stylesheet.test.ts` asserts that, because an
 ...
 * @param crib - {@link operatorCrib}'s text, or `''`
 * @param [options]
 ...
 */
export function stylesheetSystemMessage(crib: string, options: {
  example?: any;
  exampleIntro?: string;
  extra?: string;
} = {}): string {
...
export function createStylesheetAuthor(options: {
  ...
  maxRepairs?: number;
  system?: string;
  gates?: Array<(doc: any) => any> | ((call: {
    question: string;
    sample?: any;
    externals?: any;
  }) => Array<(doc: any) => any>);
  describe?: (sample: any) => string;
}): {
  author: (question: string, options?: {
    sample?: any;
    signal?: AbortSignal;
  }) => Promise<any>;
} {
   ...
   * @param [hooks]
   */
  async function author(question: string, hooks: {
    sample?: any;
    externals?: any;
    signal?: AbortSignal;
  } = {}) {
    ...
    for (const model of models) {
      ...
      const bounded = {
        ...
        /** @param request */
        complete: (request: any) => client.complete({
          ...request,
      ...
      try {
        ...
        if (result.value !== undefined)
          return { ...result, model: model ?? client.endpoint.model, tried: [...attempts, { model, ok: true as const }] };
        attempts.push({ model, ok: false as const, ms: Date.now() - started, errors: result.errors });
      }
      catch (err: any) {
        attempts.push({ model, ok: false as const, ms: Date.now() - started, error: (err as any).message });
      }
    }
    ...
    return {
      ...
      errors: [{
        ...
        message: `no configured model authored a stylesheet that passed the gates `
          + `(${attempts.map((a: any) => a.model ?? 'default').join(', ')})`,
      }],
    };
  }
 ...
 * every repair round.
 */
function canonicalGate(canonical: any) {

  let check: ((value: any) => any) | null = null;
  return function gate(doc: any) {
    if (check === null) {
