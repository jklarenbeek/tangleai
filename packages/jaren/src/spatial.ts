 ...
 * §8.14 queries, on the pattern `stylesheet.ts` measured into shape.
 *
...
import { createStylesheetAuthor, describePaths, operatorCrib, stylesheetSystemMessage } from './stylesheet.ts';

/**
 * The refusal shape the gate seam speaks (see `stylesheet.ts`):
 *
 ...
 */
export function spatialCrib(): string {
 ...
 */
export function spatialIntent(question: string): {
  proximity: boolean;
  spatial: boolean;
} {
 ...
 */
export function coordinateMembers(sample: any): Set<string> {
 ...
 */
export function describeSpatialPaths(sample: any): string {
...
const isPhrase = (node: any, name: any) => node !== null && typeof node === 'object' && !Array.isArray(node)
 ...
 */
function isCoordinateComponent(path: string, members: Set<string>) {
...
function coordinateOperand(node: any, members: any): string | null {
...
export function planarArithmeticGate(options: {
  members?: Set<string> | Iterable<string>;
} = {}): (doc: any) => true | {
  valid: false;
  errors: any[];
} {
  ...
  return function gate(doc: any) {
    ...
    return refuse('AI0231',
      `'${matched.operator}' over '${matched.operand}' is arithmetic on a raw coordinate: a PLANAR `
 ...
 *
 * @param options - whether the question asked
...
export function prefixProximityGate(options: {
  ...
  return function gate(doc: any) {
    ...
    function walk(node: any, at: string) {
      ...
      for (const key of Object.keys(node)) {
        ...
        if (key === '$starts-with' && Array.isArray(operands)
          && operands.some((operand: any) => isPhrase(operand, '$geohash'))) {
          found = { operator: key, at: `${at}/${key}` };
          return;
        }
        if ((key === '$eq' || key === '$ne') && Array.isArray(operands)
          && operands.length === 2 && operands.every((operand: any) => isPhrase(operand, '$geohash'))) {
          found = { operator: key, at: `${at}/${key}` };
          return;
        }
    ...
    return refuse('AI0230',
      `'${matched.operator}' over a $geohash is a PREFIX test, which is bucketing, not proximity: `
 ...
 *
 * @param options - whether the question was a
 ...
 * @param crib - the operator crib read off the grammar
 */
export function spatialSystemMessage(crib: string): string {
 ...
 */
export function createSpatialAuthor(options: Parameters<typeof createStylesheetAuthor>[0]): ReturnType<typeof createStylesheetAuthor> {
