 ...
 */
const geoValue = (description: string) => ({
  description,
  anyOf: [
...
const args = (properties: any, required: any) => ({
 ...
 */
export function geoToolDefs(): import('@tangleai/agents/toolbox').ToolDef[] {
  return [
    {
      name: 'geo_distance',
    ...
    },
    {
      ...
      inputSchema: args({
        ...
        precision: {
          type: 'integer', minimum: 1, maximum: 12, default: 9,
          description: 'cell length in characters, 1..12'
        },
      }, ['value']),
      execute: ({ value, precision }: any) => {
        const at = representative(value);
        return at === null
    ...
    },
    {
      ...
      execute: ({ cell }: any) => (isValidGeohash(cell)
 ...
 *
 * The artifact is injected so this adapter uses the host's canonical
 * GeoJSON grammar. The meta-schema belongs to `@jarenjs/json`. Pass
 * `import geojson from '@jarenjs/json/schemas/geojson.schema.json' with { type: 'json' }`.
 *
 * @param options - `geojson` is the
 ...
 * @returns a toolbox whose `execute`
...
export function createGeoToolbox(options: {
  geojson: any;
  validator?: any;
}): ReturnType<typeof createToolbox> {
  ...
  return {
    ...
    /**
     */
    execute: (name: string, input: any) => (overlay.has(name)
